  ...
  >
    ...
    >
      <header class="modal-header">
        <h5 class="modal-title">{{ $t("password.change") }}</h5>
        <button
          type="button"
          aria-label="Close"
          class="close"
          @click="toggleModal('changePasswordModal', false)"
        >
          ×
        </button>
      </header>
      <div class="modal-body">
        <b-col sm="12" md="12">
          <div class="mb-1 row">
            ...
            <div class="col-sm-8">
              <CInput type="password" v-model="Password.oldpassword">
          ...
          <div class="mb-3 row">
            ...
            <div class="col-sm-8">
              <b-input-group>
                <template #prepend>
                  <b-input-group-text> <b-icon-key /> </b-input-group-text>
                </template>
                <b-form-input
                  type="password"
          ...
          <div class="form-group m-auto text-center form-actions">
            <CButton
              color="danger"
              size="sm"
              class="mr-2"
            ...
            <CButton
              :disabled="!nameState || !checkpass || loading"
              type="button"
              size="sm"
              color="success"
              @click="ChangePassword"
            >
              <b-icon-check2 v-if="!loading" />
              <b-spinner v-if="loading" small></b-spinner> {{ $t("Save") }}
            </CButton>
          </div>
        </b-col>
      </div>
...
export default {
  ...
  created() {
    AccountService.GetAllUserRole().then(res => {
  ...
  computed:{
    roleid() {
      ...
      return JSON.parse(userInfo)?.roleId
    }
  },
  methods: {
    ChangeRole(item) {
      AccountService.GetUserInfo(item.roleid).then(res => {
        localStorage.setItem("user_info",JSON.stringify(res.data));
        this.$store.dispatch("auth/signIn", res.data); 
        this.$store.commit('auth/loginSuccess', { userinfo : res.data });
    ...
    ChangePassword() {
      ...
      AccountService.changePassword(this.Password)
        .then((res) => {
          ...
          var vm = this;
          setTimeout(() => {
            vm.toggleModal("changePasswordModal", false);
          }, 400);
          this.loading = false;
        })
        .catch((error) => {
    ...
    CheckPassword() {
      ...
      this.CheckPassword1();
      return !strongRegex.test(this.Password.newpassword)
    ...
    //     self.OrganizationChange = res.data;
    //   });
    // },
