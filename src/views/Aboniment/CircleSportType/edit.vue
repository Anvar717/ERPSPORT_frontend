    ...
    <CCol>
      <CCard accent-color="primary" class="pb-4">
        <CCardHeader>
          <CRow>
            <CCol>
              <h4 class="text-center">{{ $t("Sport turi") }}</h4>
            </CCol>
          </CRow>
        </CCardHeader>
        <CCardBody>
              ...
              <CCol sm="6">
                <ValidationProvider v-slot="v" name="code" rules="required">
                  <CInput
                    ...
                    horizontal
                    :label="$t('shortname')"
                    v-model="CircleSportType.shortname"
                    :addInputClasses="{
                      'is-invalid': v.classes.invalid,
                      'is-valid': v.classes.valid,
                    }"
                  >
                    <template #append>
                      <CButton
                        v-if="
                          CircleSportType.TranslatableColumns.includes(
                            'shortname'
                          )
                        "
              ...
              <CCol sm="6">
                <ValidationProvider v-slot="v" name="code" rules="required">
                  <CInput
                    ...
                    horizontal
                    :label="$t('fullname')"
                    v-model="CircleSportType.fullname"
                    :addInputClasses="{
                      'is-invalid': v.classes.invalid,
                      'is-valid': v.classes.valid,
                    }"
              ...
              <CCol sm="6">
                <div class="form-group form-row">
                  ...
                  <v-select
                    class="col-sm-9"
                    :options="statelist"
                    v-model="CircleSportType.stateid"
                    :reduce="(item) => item.id"
          ...
          <CCol>
            <div class="text-left">
              <CButton
                ...
                size="sm"
                color="danger"
                @click="$router.push({ name: 'CircleSportType' })"
              >
...
export default {
  data() {
    return {
      ...
      CircleSportType: { TranslatableColumns: [] },
      selectedrowid: 0,
      translitefields: [
  ...
  created() {
    ...
    CircleSportTypeService.Get(self.selectedrowid).then((res) => {
      self.CircleSportType = res.data;
    });
  },
  methods: {
    check() {
      ...
      if (
        self.CircleSportType.code === 0 ||
        self.CircleSportType.code === null ||
        self.CircleSportType.code === undefined ||
        self.CircleSportType.code === ""
      ) {
        this.makeToast(this.$t("codeNotCorrect"), this.$t("Error"), "danger");
        return false;
      }
      if (
        self.CircleSportType.shortname === 0 ||
        self.CircleSportType.shortname === null ||
        self.CircleSportType.shortname === undefined ||
        self.CircleSportType.shortname === ""
      ) {
        this.makeToast(
      ...
      if (
        self.CircleSportType.fullname === 0 ||
        self.CircleSportType.fullname === null ||
        self.CircleSportType.fullname === undefined ||
        self.CircleSportType.fullname === ""
      ) {
        this.makeToast(
      ...
      if (
        self.CircleSportType.stateid === 0 ||
        self.CircleSportType.stateid === null ||
        self.CircleSportType.stateid === undefined ||
        self.CircleSportType.stateid === ""
      ) {
        this.makeToast(
    ...
    SaveData() {
      ...
      var self = this;
      CircleSportTypeService.Update(self.CircleSportType)
        .then((res) => {
          self.makeToast(
          ...
          self.$router.push({ name: "CircleSportType" });
        })
        .catch((error) => {
          self.makeToast(
    ...
    translate(selectcolumnname) {
      ...
      self.translatelist = self.CircleSportType.Translates.filter(
    ...
    SaveTranslate() {
      var self = this;
      self.CircleSportType.Translates = self.CircleSportType.Translates.filter(
        (x) => x.columnname !== self.translatebyname
      );
      self.translatelist.forEach((item) => {
        self.CircleSportType.Translates.push(item);
      });
