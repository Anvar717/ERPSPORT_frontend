  ...
  <CRow>
    <CCol sm="12" md="12">
      <CCard accent-color="primary">
        <h3 class="text-center mt-3">{{ $t("PaymentFreeMemberShip") }}</h3>
        <CCardHeader>
...
export default {
  ...
  methods: {
    ...
    Delete(item) {
      PaymentFreeMemberShipService.Delete(item.id)
        .then((res) => {
          this.makeToast(
    ...
    Approve(item) {
      var self = this;
      PaymentFreeMemberShipService.Approve(item.id)
        .then((res) => {
          self.makeToast(
    ...
    CancelApproval(item) {
      var self = this;
      PaymentFreeMemberShipService.CancelApproval(item.id)
        .then((res) => {
          self.makeToast(
    ...
    Refresh() {
      this.isBusy = true;
      PaymentFreeMemberShipService.GetList(
        this.filter.Search,
