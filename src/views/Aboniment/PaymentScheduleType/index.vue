  ...
  <CRow>
    <CCol sm="12" md="12">
      <CCard accent-color="primary">
        <h3 class="text-center mt-3">{{ $t("PaymentScheduleType") }}</h3>
        <CCardHeader>
          <CRow class="form-group justify-content-between">
            ...
            <CCol lg="7" md="6" sm="12" class="text-lg-right mt-2" style="margin-right: -7px;">
              <CButton
                v-if="$can('PaymentScheduleTypeEdit', 'permissions')"
                @click="Create"
                color="primary"
                variant="outline"
                class="mr-2"
          ...
          >
            ...
            <template v-slot:cell(actions)="{ item }">
              <div class="text-center">
                ...
                <b-link
                  v-if="$can('PaymentScheduleTypeDelete', 'permissions')"
...
export default {
  ...
  methods: {
    ...
    Delete(item) {
      PaymentScheduleTypeService.Delete(item.id)
        .then((res) => {
          this.makeToast(
    ...
    Refresh() {
      this.isBusy = true;
      PaymentScheduleTypeService.GetList(
        this.filter.Search,
