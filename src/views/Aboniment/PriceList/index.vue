  ...
  <CRow>
    <CCol sm="12" md="12">
      <CCard accent-color="primary">
        <h3 class="text-center mt-3">{{ $t("PriceList") }}</h3>
        <CCardHeader>
        ...
        <CCardBody>
          ...
          >
            ...
            <template v-slot:cell(coachpayment)="{ item }">
              <span class="font-weight-bold">{{
                $options.filters.currency(item.coachpayment, {
                  symbol: "",
                  fractionCount: 2,
                })
              }}</span>
            ...
            <template v-slot:cell(actions)="{ item }">
              <div class="text-center">
                <b-link
                  v-if="$can('PriceListEdit', 'permissions')"
                  @click="Edit(item)"
                  class="mr-2"
                ...
                <b-dropdown variant="link" no-caret class="ml-n2 px-0">
                  ...
                  <div>
                    <b-dropdown-item
                      v-if="$can('PriceListApprove', 'permissions')"
                    ...
                    <b-dropdown-item
                      v-if="$can('PriceListDelete', 'permissions')"
...
export default {
  ...
  methods: {
    ...
    Delete(item) {
      PriceListService.Delete(item.id)
        .then((res) => {
          this.makeToast(
    ...
    Approve(item) {
      var self = this;
      PriceListService.Approve(item.id)
        .then((res) => {
          self.makeToast(
    ...
    CancelApproval(item) {
      var self = this;
      PriceListService.CancelApproval(item.id)
        .then((res) => {
          self.makeToast(
