  ...
  <CRow>
    <CCol sm="12" md="12">
      <CCard accent-color="primary">
        <h3 class="text-center mt-3">{{ $t("CloseMembership") }}</h3>
        <CCardHeader>
            ...
            <template v-slot:cell(actions)="{ item }">
              <div class="text-center">
                <b-link
                  @click="Edit(item)"
                  v-if="
                    $can('CloseMembershipEdit', 'permissions') &&
                    item.statusid != 2
                  "
                  class="mr-2"
              ...
              <b-modal :id="'AcceptModal' + item.id" hide-footer>
                ...
                <div class="d-block text-right">
                  ...
                  <CButton
                    @click="Approve(item)"
                    v-if="$can('CloseMembershipApprove', 'permissions')"
                    color="success"
                  >
              ...
              <b-modal :id="'CancelModal' + item.id" hide-footer>
                ...
                <div class="d-block text-right">
                  ...
                  <CButton
                    ...
                    v-if="$can('CloseMembershipCancelApproval', 'permissions')"
                    color="success"
                  >
                    <b-icon icon="check2"></b-icon>
                    {{ $t("Approve") }}
              ...
              <b-modal :id="'DeleteModal' + item.id" hide-footer>
                ...
                <div class="d-block text-right">
                  ...
                  <CButton
                    @click="Delete(item)"
                    v-if="$can('CloseMembershipDelete', 'permissions')"
                    color="success"
                  >
                    <b-icon icon="check2"></b-icon>
                    {{ $t("Accept") }}
...
export default {
  ...
  methods: {
    ...
    Delete(item) {
      CloseMembershipService.Delete(item.id)
        .then((res) => {
          this.makeToast(
    ...
    Refresh() {
      this.isBusy = true;
      CloseMembershipService.GetList(
        this.filter.Search,
    ...
    Approve(item) {
      var self = this;
      CloseMembershipService.Approve(item.id)
        .then((res) => {
          self.makeToast(
    ...
    CancelApproval(item) {
      var self = this;
      CloseMembershipService.CancelApproval(item.id)
        .then((res) => {
          self.makeToast(
