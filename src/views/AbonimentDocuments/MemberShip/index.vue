  ...
  <CRow>
    <CCol sm="12" md="12">
      <CCard accent-color="primary">
        <h3 class="text-center mt-3">{{ $t("MemberShip") }}</h3>
        <CCardHeader>
          <CRow class="form-group justify-content-between">
            ...
            <CCol lg="6" md="6" sm="12" class="text-lg-right mt-2" style="margin-right: -7px;">
              <CButton
                @click="Create"
                v-if="$can('MemberShipEdit', 'permissions')"
                color="primary"
                variant="outline"
                class="mr-2"
                size="sm"
          ...
          >
            ...
            <template v-slot:cell(actions)="{ item }">
              <div class="text-center">
                ...
                <!-- <b-link  @click="Printer(item)" v-c-tooltip="{ content: $t('Print') }" class="mr-2">
                ...
                <b-dropdown variant="link" no-caret class="ml-n2 px-0">
                  ...
                  <div>
                    <b-dropdown-item
                      v-if="$can('MemberShipApprove', 'permissions')"
                    ...
                    <b-dropdown-item
                      v-if="$can('MemberShipCancelApproval', 'permissions')"
                    ...
                    <b-dropdown-item
                      v-if="$can('MemberShipDelete', 'permissions')"
                ...
                <b-modal :id="'SendModal' + item.id" hide-footer>
                  <template v-slot:modal-title>{{ $t("commenttosend") }}</template>
                  <div class="d-block text-right">
                    ...
                    <CButton @click="SendAll(item)" color="success">
...
export default {
  ...
  data() {
    return {
      fields: [
        ...
        {
          key: "sportgroupcategorytypename",
          label: this.$t("sportgroupcategorytype"),
          thClass: "text-center",
          tdClass: "text-left",
          sortable: false,
        },
  ...
  methods: {
    Printer(item) {
      MemberShipService.PrintMemberShipPdf('pdf', item.id).then(res => {
    ...
    Delete(item) {
      MemberShipService.Delete(item.id)
        .then((res) => {
          this.makeToast(
    ...
    Approve(item) {
      var self = this;
      MemberShipService.Approve(item.id)
        .then((res) => {
          self.makeToast(
    ...
    SendAll(item) {
      MemberShipService.SendSms(item.id, 0)
        .then((res) => {
    ...
    CancelApproval(item) {
      var self = this;
      MemberShipService.CancelApproval(item.id)
        .then((res) => {
          self.makeToast(
