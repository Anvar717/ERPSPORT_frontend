<template>
    <CRow>
      <CCol sm="12" md="12">
        <CCard accent-color="primary">
          <h3 class="text-center mt-3">{{ $t("MissedDays") }}</h3>
          <CCardHeader>
  ...
  export default {
    ...
    methods: {
      ...
      Delete(item) {
        MissedDaysService.Delete(item.id)
          .then((res) => {
            this.makeToast(
      ...
      Approve(item) {
        var self = this;
        MissedDaysService.Approve(item.id)
          .then((res) => {
            self.makeToast(
      ...
      CancelApproval(item) {
        var self = this;
        MissedDaysService.CancelApproval(item.id)
          .then((res) => {
            self.makeToast(
