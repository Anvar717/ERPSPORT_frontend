<template>
    <CRow>
      <CCol sm="12" md="12">
        <CCard accent-color="primary">
          <h3 class="text-center mt-3">{{ $t("Payment") }}</h3>
          <CCardHeader>
            <CRow class="form-group justify-content-between">
              ...
              <CCol lg="7" md="6" sm="12" class="text-lg-right mt-2" style="margin-right: -7px;">
                <CButton
                  v-if="$can('PaymentEdit', 'permissions')"
                  @click="Create"
                  color="primary"
                  variant="outline"
                  class="mr-2"
          ...
          <CCardBody>
            ...
            >
              ...
              <template v-slot:head(personmembername)>
                <div style="cursor: pointer" @click="ChangeSort('personmembername')">
                  {{ $t("personmembername") }}
                  <b-icon
                    v-if="filter.SortColumn === 'personmembername'"
                    :icon="filter.SortIcon"
                  ></b-icon>
                </div>
              </template>
              ...
              <template v-slot:cell(actions)="{ item }">
                <div class="text-center">
                  ...
                  <b-dropdown variant="link" no-caret class="ml-n2 px-0">
                    ...
                    <div>
                      <b-dropdown-item
                        v-if="$can('PaymentApprove', 'permissions')"
                      ...
                      <b-dropdown-item
                        v-if="$can('PaymentCancelApproval', 'permissions')"
                      ...
                      <b-dropdown-item
                        v-if="$can('PaymentDelete', 'permissions')"
  ...
  export default {
    ...
    methods: {
      ...
      Approve(item) {
        var self = this;
        PaymentService.Approve(item.id)
          .then((res) => {
            self.makeToast(
      ...
      CancelApproval(item) {
        var self = this;
        PaymentService.CancelApproval(item.id)
          .then((res) => {
            self.makeToast(
