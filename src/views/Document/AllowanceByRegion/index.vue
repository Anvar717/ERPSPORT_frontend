  ...
  <CRow>
    <CCol sm="12" md="12">
      <CCard accent-color="primary">
        <h3 class="text-center mt-3">{{ $t("AllowanceByRegion") }}</h3>
        <CCardHeader>
          <CRow class="form-group">
            ...
            <CCol lg="7" md="6" sm="12" class="text-lg-right mt-2">
              <CButton
                v-if="$can('AllowanceByRegionEdit', 'permissions')"
                @click="Create"
                color="primary"
                variant="outline"
                class="mr-2"
          ...
          >
            ...
            <template v-slot:head(calculationkind)>
              ...
              >
                {{ $t("calculationkind") }}
                <b-icon
                  v-if="filter.SortColumn === 'calculationkind'"
                  :icon="filter.SortIcon"
                ></b-icon>
              </div>
            </template>
            ...
            <template v-slot:cell(actions)="{ item }">
              <div class="text-center">
                <b-link
                  v-if="
                    $can('AllowanceByRegionEdit', 'permissions') &&
                    item.statusid != 2
                  "
                  @click="Edit(item)"
                  class="mr-2"
                ...
                <b-link
                  v-if="
                    $can('AllowanceByRegionDelete', 'permissions') &&
                    item.statusid != 2
                  "
                ...
                <b-link
                  v-if="
                    $can('AllowanceByRegionAccept', 'permissions') &&
                    item.statusid != 2
                  "
                ...
                <b-link
                  v-if="
                    $can('AllowanceByRegionCancel', 'permissions') &&
                    item.statusid != 3
                  "
...
export default {
  ...
  methods: {
    ...
    Delete(item) {
      AllowanceByRegionService.Delete(item.id)
        .then((res) => {
          this.makeToast(
    ...
    Approve(item) {
      var self = this;
      AllowanceByRegionService.Approve(item.id)
        .then((res) => {
          self.makeToast(
    ...
    CancelApproval(item) {
      var self = this;
      AllowanceByRegionService.CancelApproval(item.id)
        .then((res) => {
          self.makeToast(
            self.$t("AcceptMessage"),
            self.$t("message"),
    ...
    Refresh() {
      this.isBusy = true;
      AllowanceByRegionService.GetList(
        this.filter.Search,
