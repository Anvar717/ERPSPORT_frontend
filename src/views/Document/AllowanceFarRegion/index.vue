        ...
        <CCardHeader>
          <CRow class="form-group">
            ...
            <CCol lg="7" md="6" sm="12" class="text-lg-right mt-2">
              <CButton
                v-if="$can('AllowanceFarRegionEdit', 'permissions')"
                @click="Create"
                color="primary"
                variant="outline"
                class="mr-2"
          ...
          >
            ...
            <template v-slot:cell(actions)="{ item }">
              <div class="text-center">
                ...
                <b-link
                  v-if="$can('AllowanceFarRegionDelete', 'permissions')"
                ...
                <b-link
                  v-if="$can('AllowanceFarRegionAccept', 'permissions')"
                ...
                <b-link
                  v-if="$can('AllowanceFarRegionCancel', 'permissions')"
...
export default {
  ...
  methods: {
    ...
    Delete(item) {
      AllowanceFarRegionService.Delete(item.id)
        .then((res) => {
          this.makeToast(
    ...
    Approve(item) {
      var self = this;
      AllowanceFarRegionService.Approve(item.id)
        .then((res) => {
          self.makeToast(
    ...
    CancelApproval(item) {
      var self = this;
      AllowanceFarRegionService.CancelApproval(item.id)
        .then((res) => {
          self.makeToast(
            self.$t("AcceptMessage"),
            self.$t("message"),
    ...
    Refresh() {
      this.isBusy = true;
      AllowanceFarRegionService.GetList(
        this.filter.Search,
