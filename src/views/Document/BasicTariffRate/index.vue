  ...
  <CRow>
    <CCol sm="12" md="12">
      <CCard accent-color="primary">
        <h3 class="text-center mt-3">{{ $t("BasicTariffRate") }}</h3>
        <CCardHeader>
          <CRow class="form-group">
            ...
            <CCol lg="7" md="6" sm="12" class="text-lg-right mt-2">
              <CButton
                v-if="$can('BasicTariffRateEdit', 'permissions')"
                @click="Create"
                color="primary"
                variant="outline"
                class="mr-2"
        ...
        <CCardBody>
          ...
          >
            ...
            <template v-slot:head(stafftypebasictariff)>
              <div
                style="cursor : pointer"
                @click="ChangeSort('stafftypebasictariff')"
              >
                {{ $t("stafftypebasictariff") }}
                <b-icon
                  v-if="filter.SortColumn === 'stafftypebasictariff'"
                  :icon="filter.SortIcon"
                ></b-icon>
              </div>
            </template>
            <template v-slot:head(normativedoc)>
              <div style="cursor : pointer" @click="ChangeSort('normativedoc')">
                {{ $t("normativedoc") }}
                <b-icon
            ...
            <template v-slot:cell(actions)="{ item }">
              <div class="text-center">
                ...
                <b-dropdown variant="link" no-caret class="ml-n2 px-0">
                  ...
                  <div>
                    <b-dropdown-item
                      v-if="$can('BasicTariffRateApprove', 'permissions')"
                    ...
                    <b-dropdown-item
                      v-if="$can('BasicTariffRateCancelApproval', 'permissions')"
                    ...
                    <b-dropdown-item
                      v-if="$can('BasicTariffRateDelete', 'permissions')"
...
export default {
  data() {
    return {
      fields: [
        ...
        {
          key: "stafftypebasictariffname",
          label: this.$t("stafftypebasictariff"),
          sortable: false,
        },
        {
          key: "normativedoc",
  ...
  methods: {
    ...
    Delete(item) {
      BasicTariffRateService.Delete(item.id)
        .then((res) => {
          this.makeToast(
    ...
    Approve(item) {
      var self = this;
      BasicTariffRateService.Approve(item.id)
        .then((res) => {
          self.makeToast(
    ...
    CancelApproval(item) {
      var self = this;
      BasicTariffRateService.CancelApproval(item.id)
        .then((res) => {
          self.makeToast(
            self.$t("AcceptMessage"),
            self.$t("message"),
    ...
    Refresh() {
      this.isBusy = true;
      BasicTariffRateService.GetList(
        this.filter.Search,
