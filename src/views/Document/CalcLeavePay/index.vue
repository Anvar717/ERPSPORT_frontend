  ...
  <CRow>
    <CCol sm="12" md="12">
      <CCard accent-color="primary">
        <h3 class="text-center mt-3">{{ $t("CalcLeavePay") }}</h3>
        <CCardHeader>
            ...
            <CCol class="text-left mt-2">
              <b-button-group class="text-center">
                <b-button
                  size="sm"
                  :variant="
                    filter.calculationkindid == 25
                      ? 'primary'
                      : 'outline-primary'
                  "
                  @click="HREmpleaveOrder"
                >
                  {{ $t("TabHREmpleaveOrder") }}
                </b-button>
                <b-button
                  size="sm"
                  :variant="
                    filter.calculationkindid == 27
                      ? 'primary'
                      : 'outline-primary'
                  "
                  @click="HREmpSickLeave"
                >
                  {{ $t("TabHREmpSickLeave") }}
                </b-button>
                <b-button
                  size="sm"
                  :variant="
                    filter.calculationkindid == 26
                      ? 'primary'
                      : 'outline-primary'
                  "
                  @click="HREmpAcceptanceWork"
                >
                  {{ $t("TabHREmpAcceptanceWork") }}
                </b-button>
              </b-button-group>
            </CCol>
          </CRow>
        ...
        <CCardBody>
          <b-table
            :fields="fields"
            :items="items"
            style="white-space: nowrap; min-height: 300px"
            small="small"
            responsive="sm"
          ...
          >
            ...
            <template v-slot:head(calculationkind)>
              <div
                style="cursor : pointer"
                @click="ChangeSort('calculationkind')"
              >
                {{ $t("calculationkind") }}
                <b-icon v-if="filter.SortColumn === 'calculationkind'" :icon="filter.SortIcon"></b-icon>
              </div>
            </template>
            ...
            <template v-slot:head(bonusmonthcount)>
              <div
                style="cursor : pointer"
                @click="ChangeSort('bonusmonthcount')"
              >
                {{ $t("bonusmonthcount") }}
                <b-icon v-if="filter.SortColumn === 'bonusmonthcount'" :icon="filter.SortIcon"></b-icon>
              </div>
            </template>
            ...
            <template v-slot:cell(actions)="{ item }">
              <div class="text-center">
                <b-link
                  v-if="$can('CalcLeavePayEdit', 'permissions')"
                  @click="Edit(item)"
                  class="mr-2"
                ...
                <b-link
                  v-if="$can('CalcLeavePayDelete', 'permissions')"
                ...
                <b-link
                  v-if="$can('CalcLeavePayAccept', 'permissions')"
                ...
                <b-link
                  v-if="$can('CalcLeavePayCancel', 'permissions')"
...
export default {
  ...
  methods: {
    HREmpleaveOrder() {
      this.filter.calculationkindid = 25;
      this.Refresh();
    },
    HREmpSickLeave() {
      this.filter.calculationkindid = 27;
      this.Refresh();
    },
    HREmpAcceptanceWork(){
      this.filter.calculationkindid = 26
      this.Refresh()
    },
    getItemColor(id) {
      return id === 1
    ...
    Delete(item) {
      CalcLeavePayService.Delete(item.id)
        .then((res) => {
          this.makeToast(
    ...
    Approve(item) {
      var self = this;
      CalcLeavePayService.Approve(item.id)
        .then((res) => {
          self.makeToast(
    ...
    CancelApproval(item) {
      var self = this;
      CalcLeavePayService.CancelApproval(item.id)
        .then((res) => {
          self.makeToast(
            self.$t("AcceptMessage"),
            self.$t("message"),
    ...
    Refresh() {
      this.isBusy = true;
      CalcLeavePayService.GetList(
        this.filter.Search,
        this.filter.SortColumn,
        this.filter.OrderType,
        this.filter.PageNumber,
        this.filter.PageLimit,
        this.filter.calculationkindid
      ).then((res) => {
        this.isBusy = false;
