  ...
  <CRow>
    <CCol sm="12" md="12">
      <CCard accent-color="primary">
        <h3 class="text-center mt-3">{{ $t("CurriculumOrgschGrade") }}</h3>
        <CCardHeader>
          <CRow class="form-group">
            ...
            <CCol lg="2" md="2" sm="12" class="text-left mt-2 pl-0">
              <div>
            ...
            <CCol lg="3" md="3" sm="12" class="text-lg-right mt-2">
              <CButton
                v-if="$can('CurriculumOrgschGradeEdit', 'permissions')"
                @click="Create"
                color="primary"
                variant="outline"
                class="mr-2"
        ...
        <CCardBody>
          ...
          >
            ...
            <template v-slot:head(orgschoolgrade)>
              <div
                style="cursor : pointer"
                @click="ChangeSort('orgschoolgrade')"
              >
                {{ $t("orgschoolgrade") }}
                <b-icon
                  v-if="filter.SortColumn === 'orgschoolgrade'"
                  :icon="filter.SortIcon"
                ></b-icon>
              </div>
            </template>
            <template v-slot:head(totalhours)>
              <div style="cursor : pointer" @click="ChangeSort('totalhours')">
                {{ $t("totalhours") }}
                <b-icon
                  v-if="filter.SortColumn === 'totalhours'"
                  :icon="filter.SortIcon"
                ></b-icon>
              </div>
            </template>
            ...
            <template v-slot:cell(actions)="{ item }">
              <div class="text-center">
                ...
                <b-link
                  v-if="$can('CurriculumOrgschGradeDelete', 'permissions')"
...
export default {
  ...
  methods: {
    ...
    Delete(item) {
      CurriculumOrgschGradeService.Delete(item.id)
        .then((res) => {
          this.makeToast(
    ...
    Approve(item) {
      var self = this;
      CurriculumOrgschGradeService.Approve(item.id)
        .then((res) => {
          self.makeToast(
    ...
    CancelApproval(item) {
      var self = this;
      CurriculumOrgschGradeService.CancelApproval(item.id)
        .then((res) => {
          self.makeToast(
            self.$t("AcceptMessage"),
            self.$t("message"),
    ...
    Refresh() {
      this.isBusy = true;
      CurriculumOrgschGradeService.GetList(
        this.filter.Search,
