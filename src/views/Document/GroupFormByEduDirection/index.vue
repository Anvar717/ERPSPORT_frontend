  ...
  <CRow>
    <CCol sm="12" md="12">
      <CCard accent-color="primary">
        <h3 class="text-center mt-3">{{ $t("GroupFormByEduDirection") }}</h3>
        <CCardHeader>
          <CRow>
        ...
        <CCardBody>
          ...
          >
            ...
            <template v-slot:head(educationformtypename)>
              ...
              >
                {{ $t("educationformtypename") }}
                <b-icon v-if="filter.SortColumn === 'educationformtypename'" :icon="filter.SortIcon"></b-icon>
              </div>
            </template>
            ...
            <template v-slot:head(gendername)>
              ...
              >
                {{ $t("gendername") }}
                <b-icon v-if="filter.SortColumn === 'gendername'" :icon="filter.SortIcon"></b-icon>
              </div>
            </template>
            <template v-slot:head(highereduclassifiername)>
              ...
              >
                ...
                <b-icon v-if="filter.SortColumn === 'highereduclassifiername'" :icon="filter.SortIcon"></b-icon>
              </div>
            </template>
            ...
            <span style="font-weight: bold">{{item.highereduclassifiername}}</span>
            </template>
            <template v-slot:cell(groupname)="{ item }">
            ...
            <template v-slot:cell(actions)="{ item }">
              <div class="text-center">
                <b-link
                  @click="View(item)"
                  v-if="$can('GroupFormByEduDirectionView', 'permissions')"
                  class="mr-2"
                ...
                <b-dropdown variant="link" no-caret class="ml-n2 px-0">
                  ...
                  <div>
                    <b-dropdown-item
                      v-if="$can('GroupFormByEduDirectionApprove', 'permissions')"
                    ...
                    <b-dropdown-item
                      v-if="$can('GroupFormByEduDirectionCancelApproval', 'permissions')"
                    ...
                    <b-dropdown-item
                      v-if="$can('GroupFormByEduDirectionDelete', 'permissions')"
...
export default {
  ...
  methods: {
    ...
    Delete(item) {
      GroupFormByEduDirectionService.Delete(item.id)
        .then((res) => {
          this.makeToast(
    ...
    Approve(item) {
      var self = this;
      GroupFormByEduDirectionService.Approve(item.id)
        .then((res) => {
          self.makeToast(
    ...
    CancelApproval(item) {
      var self = this;
      GroupFormByEduDirectionService.CancelApproval(item.id, item)
        .then((res) => {
          self.makeToast(
    ...
    Refresh() {
      this.isBusy = true;
      GroupFormByEduDirectionService.GetList(
        this.filter.schoolYearId,
        this.filter.higHerEduClassifierId,
        this.filter.educationFormTypemId,
        this.filter.educationLanguageId,
