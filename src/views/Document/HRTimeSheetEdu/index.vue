  ...
  <CRow>
    <CCol sm="12" md="12">
      <CCard accent-color="primary">
        <h3 class="text-center mt-3">{{ $t("HRTimeSheetEdu") }}</h3>
        <CCardHeader>
          <CRow class="form-group">
            ...
            >
              <CButton
                v-if="$can('HRTimeSheetEduEdit', 'permissions')"
                @click="Create"
                color="primary"
                variant="outline"
                class="mr-2"
        ...
        <CCardBody>
          ...
          >
            ...
            <template #cell(formonth)="{item}">
              <p class="p-0 m-0">{{ getMonthName(item) }}</p>
            </template>
            ...
            <template v-slot:cell(actions)="{ item }">
              <div class="text-center">
                ...
                <b-dropdown variant="link" no-caret class="ml-n2 px-0">
                  ...
                  <div>
                    <b-dropdown-item
                      v-if="$can('HRTimeSheetEduApprove', 'permissions')"
                    ...
                    <b-dropdown-item
                      v-if="$can('HRTimeSheetEduCancelApproval', 'permissions')"
                    ...
                    <b-dropdown-item
                      v-if="$can('HRTimeSheetEduDelete', 'permissions')"
                    ...
                    <b-dropdown-item
                      ...
                      v-if="$can('HRTimeSheetEduSend', 'permissions')"
                    >
                       <b-icon icon="cursor" class="mr-2"></b-icon>
                      {{ $t("Send") }}
                    </b-dropdown-item>
                    <b-dropdown-item
                      @click="$bvModal.show('CancelDocumentModal' + item.id)"
                      v-if="$can('HRTimeSheetSend', 'permissions')"
                    >
                      <b-icon icon="check2-all" class="mr-2"></b-icon>
                      {{ $t("CancelDocument") }}
                    </b-dropdown-item>
                    <!-- <b-dropdown-item
                      @click="$bvModal.show('CancelModal2' + item.id)"
                      v-if="$can('HRTimeSheetEduReject','permissions')"
                    >
                ...
                <b-modal :id="'CancelDocumentModal' + item.id" hide-footer>
                  <template v-slot:modal-title>{{
                    $t("WantCancelDocument")
                  }}</template>
                  <div class="d-block text-right">
                    <CButton
                      @click="$bvModal.hide('CancelDocumentModal' + item.id)"
                      color="danger"
                      class="mr-2"
                    >
                      <b-icon icon="x"></b-icon>
                ...
                <!-- <b-modal :id="'CancelModal2' + item.id" hide-footer>
                  <template v-slot:modal-title>{{ $t("WantCancel") }}</template>
                  <div class="d-block text-right">
                    <CButton
                      @click="$bvModal.hide('CancelModal2' + item.id)"
                      color="danger"
                      class="mr-2"
                    >
                      <b-icon icon="x"></b-icon>
...
export default {
  ...
  methods: {
    ...
    getMonthName(item) {
      var month = item.formonth.split(".")[1];
      var monthname = "";
      switch (month) {
        case "01":
          monthname = "январь";
    ...
    Delete(item) {
      HRTimeSheetEduService.Delete(item.id)
        .then((res) => {
          this.makeToast(
    ...
    Approve(item) {
      var self = this;
      HRTimeSheetEduService.Approve(item.id)
        .then((res) => {
          self.makeToast(
    ...
    CancelApproval(item) {
      var self = this;
      HRTimeSheetEduService.CancelApproval(item.id)
        .then((res) => {
          self.makeToast(
    ...
    Send(item) {
      var self = this;
      HRTimeSheetEduService.Send(item.id)
        .then((res) => {
    ...
    Reject(item) {
      var self = this;
      HRTimeSheetEduService.Reject(item.id, this.reason)
        .then((res) => {
          self.makeToast(
    ...
    Accept(item) {
      var self = this;
      HRTimeSheetEduService.Accept(item.id)
        .then((res) => {
          self.makeToast(
            self.$t("AcceptMessage"),
            self.$t("message"),
    ...
    Cancel(item) {
      var self = this;
      HRTimeSheetEduService.Cancel(item.id)
        .then((res) => {
          self.makeToast(
            self.$t("CancelMessage"),
            self.$t("message"),
    ...
    Print(item) {
      HRTimeSheetEduService.Print(item.id)
        .then((response) => {
          ...
          fileLink.setAttribute("download", "HRTimeSheetEdu.xlsx");
