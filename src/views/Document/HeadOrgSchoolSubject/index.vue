  ...
  <CRow>
    <CCol sm="12" md="12">
      <CCard accent-color="primary">
        <h3 class="text-center mt-3">{{ $t("HeadOrgSchoolSubject") }}</h3>
        <CCardHeader>
          <CRow class="form-group">
            ...
            <CCol lg="7" md="6" sm="12" class="text-lg-right mt-2">
              <CButton
                v-if="$can('HeadOrgSchoolSubjectEdit', 'permissions')"
                @click="Create"
                color="primary"
                variant="outline"
                class="mr-2"
            ...
            <template v-slot:cell(actions)="{ item }">
              <div class="text-center">
                ...
                <b-dropdown variant="link" no-caret class="ml-n2 px-0">
                  ...
                  <div>
                    <b-dropdown-item
                      v-if="$can('HeadOrgSchoolSubjectApprove', 'permissions')"
                    ...
                    <b-dropdown-item
                      v-if="
                        $can(
                          'HeadOrgSchoolSubjectCancelApproval',
                          'permissions'
                        )
                      "
                    ...
                    <b-dropdown-item
                      v-if="$can('HeadOrgSchoolSubjectDelete', 'permissions')"
...
export default {
  ...
  methods: {
    ...
    Delete(item) {
      HeadOrgSchoolSubjectService.Delete(item.id)
        .then((res) => {
          this.makeToast(
    ...
    Approve(item) {
      var self = this;
      HeadOrgSchoolSubjectService.Approve(item.id)
        .then((res) => {
          self.makeToast(
    ...
    Cancel(item) {
      var self = this;
      HeadOrgSchoolSubjectService.CancelApproval(item.id)
        .then((res) => {
          self.makeToast(
    ...
    Refresh() {
      this.isBusy = true;
      HeadOrgSchoolSubjectService.GetList(
        this.filter.Search,
