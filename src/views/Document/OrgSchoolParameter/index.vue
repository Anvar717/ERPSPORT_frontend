  ...
  <CRow>
    <CCol sm="12" md="12">
      <CCard accent-color="primary">
        <h3 class="text-center mt-3">{{ $t("OrgSchoolParameter") }}</h3>
        <CCardHeader>
          <CRow class="form-group">
            ...
            <CCol lg="7" md="6" sm="12" class="text-lg-right mt-2">
              <CButton
                v-if="$can('OrgSchoolParameterEdit', 'permissions')"
                @click="Create"
                color="primary"
                variant="outline"
                class="mr-2"
            ...
            <template v-slot:cell(actions)="{ item }">
              <div class="text-center">
                ...
                <b-dropdown variant="link" no-caret class="ml-n2 px-0">
                  ...
                  <div>
                    <b-dropdown-item
                      v-if="$can('OrgSchoolParameterAccept', 'permissions')"
                    ...
                    <b-dropdown-item
                      v-if="$can('OrgSchoolParameterCancel', 'permissions')"
                    ...
                    <b-dropdown-item
                      v-if="$can('OrgSchoolParameterDelete', 'permissions')"
...
export default {
  ...
  methods: {
    ...
    Delete(item) {
      OrgSchoolParameterService.Delete(item.id)
        .then((res) => {
          this.makeToast(
    ...
    Approve(item) {
      var self = this;
      OrgSchoolParameterService.Approve(item.id)
        .then((res) => {
          self.makeToast(
    ...
    CancelApproval(item) {
      var self = this;
      OrgSchoolParameterService.CancelApproval(item.id)
        .then((res) => {
          self.makeToast(
    ...
    Refresh() {
      this.isBusy = true;
      OrgSchoolParameterService.GetList(
        this.filter.Search,
