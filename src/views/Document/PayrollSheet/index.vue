  ...
  <CRow>
    <CCol sm="12" md="12">
      <CCard accent-color="primary">
        <h3 class="text-center mt-3">{{ $t("PayrollSheet") }}</h3>
        <CCardHeader>
          <CRow class="form-group">
            ...
            >
              <CButton
                v-if="$can('PayrollSheetEdit', 'permissions')"
                @click="Create"
                color="primary"
                variant="outline"
                class="mr-2"
        ...
        <CCardBody>
          ...
          >
            ...
            <template v-slot:head(isadvance)>
              <div style="cursor : pointer" @click="ChangeSort('isadvance')">
                {{ $t("isadvance") }}
                <b-icon v-if="filter.SortColumn === 'isadvance'" :icon="filter.SortIcon"></b-icon>
              </div>
            </template>
            <template v-slot:head(isplastic)>
              <div style="cursor : pointer" @click="ChangeSort('isplastic')">
                {{ $t("isplastic") }}
                <b-icon v-if="filter.SortColumn === 'isplastic'" :icon="filter.SortIcon"></b-icon>
              </div>
            </template>
            <template v-slot:head(totalpayrollsum)>
              <div
                style="cursor : pointer"
                @click="ChangeSort('totalpayrollsum')"
              >
                {{ $t("payrollsum") }}
                <b-icon v-if="filter.SortColumn === 'totalpayrollsum'" :icon="filter.SortIcon"></b-icon>
              </div>
            </template>
            <template v-slot:head(totaldocsum)>
              <div style="cursor : pointer" @click="ChangeSort('totaldocsum')">
                {{ $t("docsum") }}
                <b-icon v-if="filter.SortColumn === 'totaldocsum'" :icon="filter.SortIcon"></b-icon>
              </div>
            </template>
            ...
            <template v-slot:cell(isadvance)="{ item }">
              <CBadge>{{ item.isadvance ? $t("yes") : $t("no") }}</CBadge>
            </template>
            <template v-slot:cell(isplastic)="{ item }">
              <CBadge>{{ item.isplastic ? $t("yes") : $t("no") }}</CBadge>
            </template>
            <template v-slot:cell(totalpayrollsum)="{ item }">
              <CBadge>{{
                $options.filters.currency(item.totalpayrollsum, {
                  symbol: "",
                  fractionCount: 2,
                })
              }}</CBadge>
            </template>
            <template v-slot:cell(totaldocsum)="{ item }">
              <CBadge>{{
                $options.filters.currency(item.totaldocsum, {
                  symbol: "",
                  fractionCount: 2,
                })
              }}</CBadge>
            ...
            <template v-slot:cell(actions)="{ item }">
              <div class="text-center">
                ...
                <b-dropdown variant="link" no-caret class="ml-n2 px-0">
                  ...
                  <div>
                    <b-dropdown-item
                      v-if="$can('PayrollSheetApprove', 'permissions')"
                    ...
                    <b-dropdown-item
                      v-if="$can('PayrollSheetCancelApproval', 'permissions')"
                    ...
                    <b-dropdown-item
                      v-if="$can('PayrollSheetDelete', 'permissions')"
...
export default {
  data() {
    return {
      fields: [
        ...
        {
          key: "isadvance",
          label: this.$t("isadvance"),
          sortable: false,
        },
        {
          key: "isplastic",
          label: this.$t("isplastic"),
          sortable: false,
        },
        {
          key: "totalpayrollsum",
          label: this.$t("totalpayrollsum"),
          sortable: false,
        },
        {
          key: "totaldocsum",
          label: this.$t("totaldocsum"),
          sortable: false,
        },
        {
          key: "detailinfo",
  ...
  methods: {
    ...
    Delete(item) {
      PayrollSheetService.Delete(item.id)
        .then((res) => {
          this.makeToast(
    ...
    Approve(item) {
      var self = this;
      PayrollSheetService.Approve(item.id)
        .then((res) => {
          self.makeToast(
    ...
    CancelApproval(item) {
      var self = this;
      PayrollSheetService.CancelApproval(item.id)
        .then((res) => {
          self.makeToast(
    ...
    Print(item) {
      PayrollSheetService.Print(item.id, item.isplastic)
        .then((response) => {
          ...
          fileLink.setAttribute("download", "PayrollSheet.xlsx");
