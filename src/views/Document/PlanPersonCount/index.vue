  ...
  <CRow>
    <CCol sm="12" md="12">
      <CCard accent-color="primary">
        <h3 class="text-center mt-3">{{ $t("PlanPersonCount") }}</h3>
        <CCardHeader>
          <CRow class="form-group">
            ...
            <CCol lg="7" md="6" sm="12" class="text-lg-right mt-2">
              <CButton
                v-if="$can('PlanPersonCountEdit', 'permissions')"
                @click="Create"
                color="primary"
                variant="outline"
                class="mr-2"
            ...
            <template v-slot:cell(actions)="{ item }">
              <div class="text-center">
                ...
                <b-dropdown variant="link" no-caret class="ml-n2 px-0">
                  ...
                  <div>
                    <b-dropdown-item
                      v-if="$can('PlanPersonCountApprove', 'permissions')"
                    ...
                    <b-dropdown-item
                      v-if="
                        $can('PlanPersonCountCancelApproval', 'permissions')
                      "
                    ...
                    <b-dropdown-item
                      v-if="$can('PlanPersonCountDelete', 'permissions')"
...
export default {
  ...
  methods: {
    ...
    Delete(item) {
      PlanPersonCountService.Delete(item.id)
        .then((res) => {
          this.makeToast(
    ...
    Approve(item) {
      var self = this;
      PlanPersonCountService.Approve(item.id)
        .then((res) => {
          self.makeToast(
    ...
    CancelApproval(item) {
      var self = this;
      PlanPersonCountService.CancelApproval(item.id)
        .then((res) => {
          self.makeToast(
    ...
    ChangeOblast() {
      (this.filter.regionid = ""), (this.filter.organizationid = "");
    ...
    Refresh() {
      this.isBusy = true;
      PlanPersonCountService.GetList(
        this.filter.oblastid,
        this.filter.regionid,
        this.filter.organizationid,
        this.filter.Search,
