  ...
  <CRow>
    <CCol sm="12" md="12">
      <CCard accent-color="primary">
        <h3 class="text-center mt-3">{{ $t("SchHourlyPayRate") }}</h3>
        <CCardHeader>
          <CRow class="form-group">
            ...
            <CCol lg="7" md="6" sm="12" class="text-lg-right mt-2">
              <CButton
                v-if="$can('SchHourlyPayRateEdit', 'permissions')"
                @click="Create"
                color="primary"
                variant="outline"
                class="mr-2"
            ...
            <template v-slot:cell(actions)="{ item }">
              <div class="text-center">
                ...
                <b-dropdown variant="link" no-caret class="ml-n2 px-0">
                  ...
                  <div>
                    <b-dropdown-item
                      v-if="$can('SchHourlyPayRateAccept', 'permissions')"
                    ...
                    <b-dropdown-item
                      v-if="$can('SchHourlyPayRateCancel', 'permissions')"
                    ...
                    <b-dropdown-item
                      v-if="$can('SchHourlyPayRateDelete', 'permissions')"
...
export default {
  ...
  methods: {
    ...
    Delete(item) {
      SchHourlyPayRateService.Delete(item.id)
        .then((res) => {
          this.makeToast(
    ...
    Approve(item) {
      var self = this;
      SchHourlyPayRateService.Approve(item.id)
        .then((res) => {
          self.makeToast(
    ...
    CancelApproval(item) {
      var self = this;
      SchHourlyPayRateService.CancelApproval(item.id)
        .then((res) => {
          self.makeToast(
    ...
    Refresh() {
      this.isBusy = true;
      SchHourlyPayRateService.GetList(
        this.filter.Search,
