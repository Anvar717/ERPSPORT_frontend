  ...
  <CRow>
    <CCol sm="12" md="12">
      <CCard accent-color="primary">
        <h3 class="text-center mt-3">{{ $t("Staffing") }}</h3>
        <CCardHeader>
          <CRow class="form-group">
            ...
            <CCol lg="7" md="6" sm="12" class="text-lg-right mt-2">
              <CButton
                v-if="$can('StaffingEdit', 'permissions')"
                @click="Create"
                color="primary"
                variant="outline"
                class="mr-2"
        ...
        <CCardBody>
          ...
          >
            ...
            <template v-slot:head(docsum)>
              <div style="cursor : pointer" @click="ChangeSort('docsum')">
                {{ $t("docsum") }}
                <b-icon
                  v-if="filter.SortColumn === 'docsum'"
                  :icon="filter.SortIcon"
                ></b-icon>
              </div>
            </template>
            <template v-slot:head(financeyear)>
              <div style="cursor : pointer" @click="ChangeSort('financeyear')">
                {{ $t("financeyear") }}
                <b-icon
                  v-if="filter.SortColumn === 'financeyear'"
                  :icon="filter.SortIcon"
                ></b-icon>
              </div>
            </template>
            <template v-slot:head(organizationaccount)>
              ...
              >
                ...
                <b-icon
                  v-if="filter.SortColumn === 'organizationaccount'"
                  :icon="filter.SortIcon"
                ></b-icon>
              </div>
            </template>
            ...
            <template v-slot:cell(actions)="{ item }">
              <div class="text-center">
                ...
                <b-dropdown variant="link" no-caret class="ml-n2 px-0">
                  ...
                  <div>
                    <b-dropdown-item
                      v-if="$can('StaffingAccept', 'permissions')"
                    ...
                    <b-dropdown-item
                      v-if="$can('StaffingCancel', 'permissions')"
                    ...
                    <b-dropdown-item
                      v-if="$can('StaffingDelete', 'permissions')"
...
export default {
  ...
  methods: {
    ...
    Delete(item) {
      StaffingService.Delete(item.id)
        .then((res) => {
          this.makeToast(
    ...
    Approve(item) {
      var self = this;
      StaffingService.Approve(item.id)
        .then((res) => {
          self.makeToast(
    ...
    CancelApproval(item) {
      var self = this;
      StaffingService.CancelApproval(item.id)
        .then((res) => {
          self.makeToast(
