  ...
  <CRow>
    <CCol sm="12" md="12">
      <CCard accent-color="primary">
        <h3 class="text-center mt-3">{{ $t("StaffingTemplate") }}</h3>
        <CCardHeader>
          <CRow class="form-group justify-content-between">
            ...
            <CCol lg="7" md="6" sm="12" class="text-lg-right mt-2" style="margin-right: -7px;">
              <CButton
                v-if="$can('StaffingTemplateEdit', 'permissions')"
                @click="Create"
                color="primary"
                variant="outline"
                class="mr-2"
            ...
            <template v-slot:cell(actions)="{ item }">
              <div class="text-center">
                ...
                <b-dropdown variant="link" no-caret class="ml-n2 px-0">
                  ...
                  <div>
                    <b-dropdown-item
                      v-if="$can('StaffingTemplateAccept', 'permissions')"
                    ...
                    <b-dropdown-item
                      v-if="$can('StaffingTemplateCancel', 'permissions')"
                    ...
                    <b-dropdown-item
                      v-if="$can('StaffingTemplateDelete', 'permissions')"
...
export default {
  ...
  methods: {
    ...
    Delete(item) {
      StaffingTemplateService.Delete(item.id)
        .then((res) => {
          this.makeToast(
    ...
    Approve(item) {
      var self = this;
      StaffingTemplateService.Approve(item.id)
        .then((res) => {
          self.makeToast(
    ...
    CancelApproval(item) {
      var self = this;
      StaffingTemplateService.CancelApproval(item.id)
        .then((res) => {
          self.makeToast(
    ...
    Refresh() {
      this.isBusy = true;
      StaffingTemplateService.GetList(
        this.filter.Search,
