  ...
  <CRow>
    <CCol sm="12" md="12">
      <CCard accent-color="primary">
        <div class="card-head-last p-3">
          <div class="text-center w-100">
            <h3 class="text-center mb-sm-2">{{ $t("Tariffication") }}</h3>
          </div>
          <div class="d-flex justify-content-lg-end text-lg-right w-100 mt-3">
            <CButton
              v-if="$can('TarifficationEdit', 'permissions')"
              @click="Create"
              color="primary"
              variant="outline"
              class="mr-2"
        ...
        <CCardHeader>
          <CRow class="form-group">
            <CCol lg="4" md="5" sm="12" class="text-left mt-2">
              <div class="d-flex">
                <CInput
        ...
        <CCardBody>
          ...
          >
            ...
            <template v-slot:head(financeyear)>
              <div style="cursor : pointer" @click="ChangeSort('financeyear')">
                {{ $t("financeyear") }}
                <b-icon v-if="filter.SortColumn === 'financeyear'" :icon="filter.SortIcon"></b-icon>
              </div>
            </template>
            <template v-slot:head(totalsum)>
              <div style="cursor : pointer" @click="ChangeSort('totalsum')">
                {{ $t("totalsum") }}
                <b-icon v-if="filter.SortColumn === 'totalsum'" :icon="filter.SortIcon"></b-icon>
              </div>
            </template>
            ...
            <template v-slot:cell(actions)="{ item }">
              <div class="text-center">
                ...
                <b-dropdown variant="link" no-caret class="ml-n2 px-0">
                  ...
                  <div>
                    <b-dropdown-item
                      v-if="$can('TarifficationAccept', 'permissions')"
                    ...
                    <b-dropdown-item
                      v-if="$can('TarifficationCancel', 'permissions')"
                    ...
                    <b-dropdown-item
                      v-if="$can('TarifficationDelete', 'permissions')"
...
export default {
  ...
  methods: {
    ...
    Delete(item) {
      TarifficationService.Delete(item.id)
        .then((res) => {
          this.makeToast(
    ...
    Approve(item) {
      var self = this;
      TarifficationService.Approve(item.id)
        .then((res) => {
          self.makeToast(
    ...
    CancelApproval(item) {
      var self = this;
      TarifficationService.CancelApproval(item.id)
        .then((res) => {
          self.makeToast(
            self.$t("AcceptMessage"),
            self.$t("message"),
    ...
    Print(item) {
      TarifficationService.Print(item.id)
        .then((response) => {
          ...
          fileLink.setAttribute("download", "Tariffication.xlsx");
