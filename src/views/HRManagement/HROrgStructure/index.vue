  ...
  <CRow>
    <CCol sm="12" lg="12">
      <CCard accent-color="primary">
        <div class="pl-4 pr-2 mt-3">
          <h3 class="text-center">{{ $t("HROrgStructure") }}</h3>
          <div class="d-flex justify-content-end mt-3">
            <CButton
                v-if="$can('HROrgStructureEdit', 'permissions') && items.length == 0"
                @click="Create"
                color="primary"
                variant="outline"
                class="mr-2"
          ...
          >
            ...
            <template v-slot:cell(actions)="{ item }">
              <div class="text-center">
                <b-link
                  v-if="$can('HROrgStructureEdit', 'permissions')"
                  @click="Edit(item)"
                  class="mr-2"
                ...
                <b-dropdown variant="link" no-caret class="ml-n2 px-0">
                  ...
                  <div>
                    <b-dropdown-item
                      v-if="$can('HROrgStructureAccept', 'permissions')"
                    ...
                    <b-dropdown-item
                      v-if="$can('HROrgStructureCancel', 'permissions')"
                    ...
                    <b-dropdown-item
                      v-if="$can('HROrgStructureDelete', 'permissions')"
...
export default {
  ...
  methods: {
    ...
    Approve(item) {
      var self = this;
      HROrgStructureService.Approve(item.id)
        .then((res) => {
          self.makeToast(
    ...
    CancelApproval(item) {
      var self = this;
      HROrgStructureService.CancelApproval(item.id)
        .then((res) => {
          self.makeToast(
