  ...
  <CRow>
    <CCol sm="12" md="12">
      <CCard accent-color="primary">
        <h3 class="text-center mt-3">{{ $t("HrAcceptanceWork") }}</h3>
        <CCardHeader>
          <CRow class="form-group">
            <CCol lg="9" md="7" sm="12" class="text-left mt-0">
              <b-row class="">
                <b-col lg="3" sm="4" class="personList-select">
            ...
            >
              <CButton
                v-if="$can('HrAcceptanceWorkEdit', 'permissions')"
                @click="Create"
                color="primary"
                variant="outline"
                class="mr-2"
              ...
              <!-- <a
                href="http://eduerpinfo.apptest.uz/HRManagement/HrAcceptanceWork"
        ...
        <CCardBody>
          ...
          >
            ...
            <template v-slot:head(employeenames)>
              <div
                class="text-center"
                style="cursor : pointer"
                @click="ChangeSort('employeenames')"
              >
                {{ $t("employeenames") }}
                <b-icon v-if="filter.SortColumn === 'employeenames'" :icon="filter.SortIcon"></b-icon>
              </div>
            </template>
            ...
            <template v-slot:cell(actions)="{ item }">
              <div class="text-center">
                ...
                <b-dropdown variant="link" no-caret class="ml-n2 px-0">
                  ...
                  <div>
                    <b-dropdown-item
                      v-if="$can('HrAcceptanceWorkAccept', 'permissions')"
                    ...
                    <b-dropdown-item
                      v-if="$can('HrAcceptanceWorkCancel', 'permissions')"
                    ...
                    <b-dropdown-item
                      v-if="$can('HrAcceptanceWorkDelete', 'permissions')"
...
export default {
  ...
  created() {
    if(!!this.$route.query.StartDate || !!this.$route.query.EndDate || !!this.$route.query.Search || !!this.$route.query.pagelimit){
  ...
  methods: {
    ...
    Approve(item) {
      var self = this;
      HrAcceptanceWorkService.Approve(item.id)
        .then((res) => {
          self.makeToast(
    ...
    CancelApproval(item) {
      var self = this;
      HrAcceptanceWorkService.CancelApproval(item.id)
        .then((res) => {
          self.makeToast(
