  ...
  <CRow>
    <CCol sm="12" md="12">
      <CCard accent-color="primary">
        <h3 class="text-center mt-3">{{ $t("MassPlannedCalculation") }}</h3>
        <CCardHeader>
          <CRow class="form-group">
            ...
            >
              <CButton
                v-if="$can('MassPlannedCalculationEdit', 'permissions')"
                @click="Create"
                color="primary"
                variant="outline"
                class="mr-2"
            ...
            <template v-slot:cell(actions)="{ item }">
              <div class="text-center">
                ...
                <b-dropdown variant="link" no-caret class="ml-n2 px-0">
                  ...
                  <div>
                    <b-dropdown-item
                      v-if="$can('MassPlannedCalculationAccept', 'permissions')"
                    ...
                    <b-dropdown-item
                      v-if="$can('MassPlannedCalculationCancel', 'permissions')"
                    ...
                    <b-dropdown-item
                      v-if="$can('MassPlannedCalculationDelete', 'permissions')"
...
export default {
  ...
  methods: {
    ...
    Delete(item) {
      MassPlannedCalculationService.Delete(item.id)
        .then((res) => {
          this.makeToast(
    ...
    Approve(item) {
      var self = this;
      MassPlannedCalculationService.Approve(item.id)
        .then((res) => {
          self.makeToast(
    ...
    CancelApproval(item) {
      var self = this;
      MassPlannedCalculationService.CancelApproval(item.id)
        .then((res) => {
          self.makeToast(
    ...
    Refresh() {
      this.isBusy = true;
      MassPlannedCalculationService.GetList(
