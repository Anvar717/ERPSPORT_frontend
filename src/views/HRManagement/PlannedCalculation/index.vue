  ...
  <CRow>
    <CCol sm="12" md="12">
      <CCard accent-color="primary">
        <h3 class="text-center mt-3">{{ $t("PlannedCalculation") }}</h3>
        <CCardHeader>
            ...
            <template v-slot:cell(actions)="{ item }">
              <div class="text-center">
                ...
                <b-dropdown variant="link" no-caret class="ml-n2 px-0">
                  ...
                  <div>
                    <b-dropdown-item
                      v-if="$can('PlannedCalculationAccept', 'permissions')"
                    ...
                    <b-dropdown-item
                      v-if="$can('PlannedCalculationCancel', 'permissions')"
                    ...
                    <b-dropdown-item
                      v-if="$can('PlannedCalculationDelete', 'permissions')"
...
export default {
  ...
  methods: {
    ...
    Approve(item) {
      var self = this;
      PlannedCalculationService.Approve(item.id)
        .then((res) => {
          self.makeToast(
    ...
    CancelApproval(item) {
      var self = this;
      PlannedCalculationService.CancelApproval(item.id)
        .then((res) => {
          self.makeToast(
    ...
    Refresh() {
      this.isBusy = true;
      PlannedCalculationService.GetList(
