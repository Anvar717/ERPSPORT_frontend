  ...
  <CRow>
    <CCol sm="12" md="12">
      <CCard accent-color="primary">
        <h3 class="text-center mt-3">{{ $t("AthletePassportSeries") }}</h3>
        <CCardHeader>
...
export default {
  data() {
    return {
      fields: [
        ...
        {
          key: "passportseriesfullname",
          label: this.$t("fullname"),
          sortable: false,
          thClass: "text-center"
        },
        {
          key: "passportseriesshortname",
  ...
  methods: {
    ...
    rowclicked() {},
    Create() {
      this.$router.push("/Info/AthletePassportSeries/edit/id=0");
    },
    Edit(item) {
      this.$router.push(`/Info/AthletePassportSeries/edit/id=${item.id}`);
    },
    Delete(item) {
      AthletePassportSeriesService.Delete(item.id)
        .then((res) => {
          this.makeToast(
    ...
    Refresh() {
      this.isBusy = true;
      AthletePassportSeriesService.GetList(
        this.filter.Search,
