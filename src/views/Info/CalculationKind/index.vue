  ...
  <CRow>
    <CCol sm="12" md="12">
      <CCard accent-color="primary">
        <h3 class="text-center mt-3">{{ $t("CalculationKind") }}</h3>
        <CCardHeader>
          <CRow class="form-group justify-content-between">
            ...
            <CCol lg="7" md="6" sm="12" class="text-lg-right mt-2" style="margin-right: -7px;">
              <CButton
                v-if="$can('CalculationKindEdit', 'permissions')"
                @click="Create"
                color="primary"
                variant="outline"
                class="mr-2"
        ...
        <CCardBody>
          ...
          >
            ...
            <template v-slot:head(normativedoc)>
              ...
              >
                {{ $t("normativedoc") }}
                <b-icon v-if="filter.SortColumn === 'normativedoc'" :icon="filter.SortIcon"></b-icon>
              </div>
            </template>
            <template v-slot:head(itemofexpense)>
              <div
                class="text-center"
                style="cursor : pointer"
                @click="ChangeSort('itemofexpense')"
              >
                {{ $t("itemofexpense") }}
                <b-icon v-if="filter.SortColumn === 'itemofexpense'" :icon="filter.SortIcon"></b-icon>
              </div>
            </template>
            <template v-slot:head(CalculationType)>
              <div
                class="text-center"
                style="cursor : pointer"
                @click="ChangeSort('calculationtype')"
              >
                {{ $t("CalculationType") }}
                <b-icon v-if="filter.SortColumn === 'calculationtype'" :icon="filter.SortIcon"></b-icon>
              </div>
            </template>
            <template v-slot:head(CalculationMethod)>
              <div
                class="text-center"
                style="cursor : pointer"
                @click="ChangeSort('CalculationMethod')"
              >
                {{ $t("CalculationMethod") }}
                <b-icon v-if="filter.SortColumn === 'CalculationMethod'" :icon="filter.SortIcon"></b-icon>
              </div>
            </template>
            ...
            <template v-slot:head(statename)>
              ...
              >
                {{ $t("status") }}
                <b-icon v-if="filter.SortColumn === 'statename'" :icon="filter.SortIcon"></b-icon>
              </div>
            </template>
            ...
            <template v-slot:cell(actions)="{ item }">
              <div class="text-center">
                ...
                <b-link
                  v-if="$can('CalculationKindDelete', 'permissions')"
  ...
  data() {
    return {
      fields: [
        ...
        {
          key: "calculationtype",
          label: this.$t("CalculationType"),
          sortable: false,
          class: "text-left",
        },
        {
          key: "calculationmethod",
          label: this.$t("CalculationMethod"),
          sortable: false,
          class: "text-left",
        },
        {
