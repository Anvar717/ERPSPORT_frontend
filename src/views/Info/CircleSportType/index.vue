  ...
  <CRow>
    <CCol sm="12" md="12">
      <CCard accent-color="primary">
        <h3 class="text-center mt-3">{{ $t("CircleSportType") }}</h3>
        <CCardHeader>
        ...
        <CCardBody>
          ...
          >
            ...
            <template v-slot:cell(actions)="{ item }">
              <div class="text-center">
                <b-link
                  v-if="$can('CircleSportTypeEdit', 'permissions')"
                  @click="Edit(item)"
                  class="mr-2"
                ...
                <b-link
                  v-if="$can('CircleSportTypeDelete', 'permissions')"
...
export default {
  ...
  methods: {
    ...
    Delete(item) {
      CircleSportTypeService.Delete(item.id)
        .then((res) => {
          this.makeToast(
    ...
    Refresh() {
      this.isBusy = true;
      CircleSportTypeService.GetList(
        this.filter.Search,
