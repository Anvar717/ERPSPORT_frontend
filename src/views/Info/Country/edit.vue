    ...
    <CCol>
      <CCard accent-color="primary" class="pb-4">
        <CCardHeader>
          <CRow>
            <CCol>
              <h4 class="text-center">{{$t('Country')}}</h4>
            </CCol>
          </CRow>
        </CCardHeader>
        <CCardBody>
              ...
              <CCol sm="6">
                <ValidationProvider v-slot="v" name="code" rules="required">
                  ...
                  >
                    <template #append>
                      <CButton
                        v-if="Country.TranslatableColumns.includes('shortname')"
              ...
              <CCol sm="6">
                <ValidationProvider v-slot="v" name="code" rules="required">
                  ...
                  >
                    <template #append>
                      <CButton
                        v-if="Country.TranslatableColumns.includes('fullname')"
...
export default {
  ...
  created() {
    ...
    CountryService.Get(self.selectedrowid).then(res => {
  ...
  methods: {
    check() {
      ...
      if (
        self.Country.numbercode === 0 ||
        self.Country.numbercode === null ||
        self.Country.numbercode === undefined ||
        self.Country.numbercode === ""
      ) {
        this.makeToast(
          this.$t("numbercodeNotCorrect"),
          this.$t("Error"),
          "danger"
        );
        return false;
      ...
      if (
        self.Country.stringcode === 0 ||
        self.Country.stringcode === null ||
        self.Country.stringcode === undefined ||
        self.Country.stringcode === ""
      ) {
        this.makeToast(
          this.$t("stringcodeNotCorrect"),
          this.$t("Error"),
          "danger"
        );
        return false;
      ...
      if (
        self.Country.shortname === 0 ||
        self.Country.shortname === null ||
        self.Country.shortname === undefined ||
        self.Country.shortname === ""
      ) {
        this.makeToast(
      ...
      if (
        self.Country.fullname === 0 ||
        self.Country.fullname === null ||
        self.Country.fullname === undefined ||
      ...
      if (
        self.Country.stateid === 0 ||
        self.Country.stateid === null ||
        self.Country.stateid === undefined ||
        self.Country.stateid === ""
      ) {
    ...
    translate(selectcolumnname) {
      ...
      self.translatelist = self.Country.Translates.filter(
    ...
    SaveTranslate() {
      var self = this;
      self.Country.Translates = self.Country.Translates.filter(
        x => x.columnname !== self.translatebyname
      );
      self.translatelist.forEach(item => {
        self.Country.Translates.push(item);
      });
