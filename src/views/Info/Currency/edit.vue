    ...
    <CCol>
      <CCard accent-color="primary" class="pb-4">
        <CCardHeader>
          <CRow>
            <CCol>
              <h4 class="text-center">{{ $t("Currency") }}</h4>
            </CCol>
          </CRow>
        </CCardHeader>
        <CCardBody>
                ...
                <ValidationProvider v-slot="v" name="code" rules="required">
                  ...
                  >
                    <template #append>
                      <CButton
                        v-if="
                          Currency.TranslatableColumns.includes('shortname')
                        "
              ...
              <CCol sm="6">
                <ValidationProvider v-slot="v" name="code" rules="required">
                  ...
                  >
                    <template #append>
                      <CButton
                        v-if="Currency.TranslatableColumns.includes('fullname')"
          ...
          <CCol>
            <div class="text-left">
              <CButton
                ...
                size="sm"
                color="danger"
                  @click="$router.push(`/Info/Currency`)"
              >
...
export default {
  ...
  created() {
    ...
    CurrencyService.Get(self.selectedrowid).then((res) => {
      self.Currency = res.data;
      if(self.selectedrowid == 0){
         self.Currency.Translates = []
      }
    });
  ...
  methods: {
    check() {
      ...
      if (
        self.Currency.code === 0 ||
        self.Currency.code === null ||
        self.Currency.code === undefined ||
        self.Currency.code === ""
      ) {
        this.makeToast(
      ...
      if (
        self.Currency.shortname === 0 ||
        self.Currency.shortname === null ||
        self.Currency.shortname === undefined ||
        self.Currency.shortname === ""
      ) {
        this.makeToast(
      ...
      if (
        self.Currency.fullname === 0 ||
        self.Currency.fullname === null ||
        self.Currency.fullname === undefined ||
        self.Currency.fullname === ""
      ) {
        this.makeToast(
    ...
    SaveData() {
      ...
      var self = this;
      CurrencyService.Update(self.Currency)
        .then((res) => {
          self.makeToast(
          ...
          // self.$router.push(`Info/Currency`);
          self.$router.push({ name: "Currency" });
        })
        .catch((error) => {
          self.makeToast(
    ...
    translate(selectcolumnname) {
      ...
      self.translatelist = self.Currency.Translates.filter(
    ...
    SaveTranslate() {
      var self = this;
      self.Currency.Translates = self.Currency.Translates.filter(
        (x) => x.columnname !== self.translatebyname
      );
      self.translatelist.forEach((item) => {
        self.Currency.Translates.push(item);
      });
