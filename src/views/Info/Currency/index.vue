    ...
    <CCol sm="12" md="12">
      <CCard accent-color="primary">
        <b-row class="mt-3 mx-2">
          ...
          <b-col>
            <h3 class="text-center ">{{ $t("Currency") }}</h3>
          </b-col>
          <b-col class="text-right">
            <CButton
                v-if="$can('CurrencyEdit', 'permissions')"
                @click="Create"
                color="primary"
                variant="outline"
                class="mr-2"
...
export default {
  ...
  methods: {
    ...
    rowclicked() {},
    Create() {
      this.$router.push("/Info/Currency/edit/id=0");
    },
    Edit(item) {
      this.$router.push(`/Info/Currency/edit/id=${item.id}`);
    },
    Delete(item) {
