    ...
    <CCol>
      <CCard accent-color="primary" class="pb-4">
        <CCardHeader>
          <CRow>
            <CCol>
              <h4 class="text-center">{{$t('EducationLanguage')}}</h4>
            </CCol>
          </CRow>
        </CCardHeader>
        <CCardBody>
              ...
              <CCol sm="6">
                <ValidationProvider v-slot="v" name="code" rules="required|max:9">
                  <CInput
                    ...
                    horizontal
                    :label="$t('code')"
                    v-model="EducationLanguage.code"
              ...
              <CCol sm="6">
                <ValidationProvider v-slot="v" name="code" rules="required">
                  <CInput
                    ...
                    horizontal
                    :label="$t('shortname')"
                    v-model="EducationLanguage.shortname"
                    :addInputClasses="{'is-invalid': v.classes.invalid,'is-valid':  v.classes.valid }"
                  >
                    <template #append>
                      <CButton
                        v-if="EducationLanguage.TranslatableColumns.includes('shortname')"
              ...
              <CCol sm="6">
                <ValidationProvider v-slot="v" name="code" rules="required">
                  <CInput
                    ...
                    horizontal
                    :label="$t('fullname')"
                    v-model="EducationLanguage.fullname"
                    :addInputClasses="{'is-invalid': v.classes.invalid,'is-valid':  v.classes.valid }"
                  >
                    <template #append>
                      <CButton
                        v-if="EducationLanguage.TranslatableColumns.includes('fullname')"
          ...
          <CCol>
            <div class="text-left">
              <CButton
                ...
                size="sm"
                color="danger"
                @click="$router.push({name : 'EducationLanguage'})"
              >
...
export default {
  data() {
    return {
      ...
      EducationLanguage: { TranslatableColumns: [] },
      selectedrowid: 0,
      translitefields: [
  ...
  created() {
    ...
    EducationLanguageService.Get(self.selectedrowid).then(res => {
      self.EducationLanguage = res.data;
    });
  },
  methods: {
    check() {
      ...
      if (
        self.EducationLanguage.code === 0 ||
        self.EducationLanguage.code === null ||
        self.EducationLanguage.code === undefined ||
        self.EducationLanguage.code === ""
      ) {
        this.makeToast(this.$t("codeNotCorrect"), this.$t("Error"), "danger");
        return false;
      }
      if (
        self.EducationLanguage.shortname === 0 ||
        self.EducationLanguage.shortname === null ||
        self.EducationLanguage.shortname === undefined ||
        self.EducationLanguage.shortname === ""
      ) {
        this.makeToast(
      ...
      if (
        self.EducationLanguage.fullname === 0 ||
        self.EducationLanguage.fullname === null ||
        self.EducationLanguage.fullname === undefined ||
        self.EducationLanguage.fullname === ""
      ) {
        this.makeToast(
    ...
    SaveData() {
      ...
      var self = this;
      EducationLanguageService.Update(self.EducationLanguage)
        .then(res => {
          self.makeToast(
          ...
          self.$router.push({ name: "EducationLanguage" });
        })
        .catch(error => {
          self.makeToast(
    ...
    translate(selectcolumnname) {
      ...
      self.translatelist = self.EducationLanguage.Translates.filter(
    ...
    SaveTranslate() {
      var self = this;
      self.EducationLanguage.Translates = self.EducationLanguage.Translates.filter(
        x => x.columnname !== self.translatebyname
      );
      self.translatelist.forEach(item => {
        self.EducationLanguage.Translates.push(item);
      });
