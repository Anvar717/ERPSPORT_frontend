    ...
    <CCol>
      <CCard accent-color="primary" class="pb-4">
        <CCardHeader>
          <CRow>
            <CCol>
              <h4 class="text-center">{{$t('Holiday')}}</h4>
            </CCol>
          </CRow>
        </CCardHeader>
        <CCardBody>
              ...
              <CCol sm="6">
                <ValidationProvider v-slot="v" name="code" rules="required">
                  <CInput
                    ...
                    horizontal
                    :label="$t('shortname')"
                    v-model="Holiday.shortname"
                    :addInputClasses="{'is-invalid': v.classes.invalid,'is-valid':  v.classes.valid }"
                  >
                    <template #append>
                      <CButton
                        v-if="Holiday.TranslatableColumns.includes('shortname')"
              ...
              <CCol sm="6">
                <ValidationProvider v-slot="v" name="code" rules="required">
                  <CInput
                    ...
                    horizontal
                    :label="$t('fullname')"
                    v-model="Holiday.fullname"
                    :addInputClasses="{'is-invalid': v.classes.invalid,'is-valid':  v.classes.valid }"
                  >
                    <template #append>
                      <CButton
                        v-if="Holiday.TranslatableColumns.includes('fullname')"
            ...
            <CRow>
              ...
              <CCol sm="6">
                <div class="form-group form-row">
                  ...
                  <CCol sm="1"></CCol>
                  <label class="col-form-label col-sm-2" for>{{$t('IsHoliday')}}</label>
                  <b-form-checkbox v-model="Holiday.isholiday" switch class="mt-2 ml-1"></b-form-checkbox>
                </div>
              </CCol>
            </CRow>
            <CRow>
          ...
          <CCol>
            <div class="text-left">
              <CButton
                ...
                size="sm"
                color="danger"
                @click="$router.push({name : 'Holiday'})"
              >
...
export default {
  ...
  created() {
    ...
    HolidayService.Get(self.selectedrowid).then(res => {
  ...
  methods: {
    check() {
      ...
      if (
        self.Holiday.code === 0 ||
        self.Holiday.code === null ||
        self.Holiday.code === undefined ||
        self.Holiday.code === ""
      ) {
      ...
      if (
        self.Holiday.shortname === 0 ||
        self.Holiday.shortname === null ||
        self.Holiday.shortname === undefined ||
        self.Holiday.shortname === ""
      ) {
        this.makeToast(
      ...
      if (
        self.Holiday.fullname === 0 ||
        self.Holiday.fullname === null ||
        self.Holiday.fullname === undefined ||
        self.Holiday.fullname === ""
      ) {
        this.makeToast(
      ...
      if (
        self.Holiday.stateid === 0 ||
        self.Holiday.stateid === null ||
        self.Holiday.stateid === undefined ||
        self.Holiday.stateid === ""
      ) {
        this.makeToast(this.$t("StatusNotCorrect"), this.$t("Error"), "danger");
        return false;
      }
      if (
        self.Holiday.ondate === 0 ||
        self.Holiday.ondate === null ||
        self.Holiday.ondate === undefined ||
        self.Holiday.ondate === ""
      ) {
    ...
    SaveData() {
      ...
      var self = this;
      HolidayService.Update(self.Holiday)
        .then(res => {
          self.makeToast(
    ...
    translate(selectcolumnname) {
      ...
      self.translatelist = self.Holiday.Translates.filter(
    ...
    SaveTranslate() {
      var self = this;
      self.Holiday.Translates = self.Holiday.Translates.filter(
        x => x.columnname !== self.translatebyname
      );
      self.translatelist.forEach(item => {
        self.Holiday.Translates.push(item);
      });
