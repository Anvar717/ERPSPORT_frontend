    ...
    <CCol>
      <CCard accent-color="primary" class="pb-4">
        <CCardHeader>
          <CRow>
            <CCol>
              <h4 class="text-center">{{$t('HomeSchoolingDisease')}}</h4>
            </CCol>
          </CRow>
        </CCardHeader>
        <CCardBody>
          <ValidationObserver ref="form" v-slot="errors">
              ...
              <CCol sm="6">
                <ValidationProvider v-slot="v" name="code" rules="required|max:9">
                  <CInput
                    ...
                    horizontal
                    :label="$t('code')"
                    v-model="HomeSchoolingDisease.code"
              ...
              <CCol sm="6">
                <ValidationProvider v-slot="v" name="code" rules="required">
                  <CInput
                    ...
                    horizontal
                    :label="$t('shortname')"
                    v-model="HomeSchoolingDisease.shortname"
                    :addInputClasses="{'is-invalid': v.classes.invalid,'is-valid':  v.classes.valid }"
                  >
                    <template #append>
                      <CButton
                        v-if="HomeSchoolingDisease.TranslatableColumns.includes('shortname')"
              ...
              <CCol sm="6">
                <ValidationProvider v-slot="v" name="code" rules="required">
                  <CInput
                    ...
                    horizontal
                    :label="$t('fullname')"
                    v-model="HomeSchoolingDisease.fullname"
                    :addInputClasses="{'is-invalid': v.classes.invalid,'is-valid':  v.classes.valid }"
                  >
                    <template #append>
                      <CButton
                        v-if="HomeSchoolingDisease.TranslatableColumns.includes('fullname')"
            ...
            <CRow>
              ...
              <CCol sm="6">
                <CInput
                  :placeholder="$t('numberofmonth')"
                  autocomplete="text"
                  horizontal
                  :label="$t('numberofmonth')"
                  v-model="HomeSchoolingDisease.numberofmonth"
                ></CInput>
              </CCol>
            </CRow>
            <CRow>
              ...
              <CCol sm="6">
                <div class="form-group form-row">
                  ...
                  <b-form-checkbox v-model="HomeSchoolingDisease.isgroup" switch class="mt-2 col-sm-9"></b-form-checkbox>
                </div>
              </CCol>
              <CCol sm="3"></CCol>
            </CRow>
              ...
              <CCol sm="6">
                <div class="form-group form-row">
                  ...
                  <v-select
                    class="col-sm-9"
                    :options="parentlist"
                    v-model="HomeSchoolingDisease.parentid"
                    :reduce="(item) => item.id"
              ...
              <CCol sm="6">
                <div class="form-group form-row">
                  ...
                  <v-select
                    class="col-sm-9"
                    :options="statelist"
                    v-model="HomeSchoolingDisease.stateid"
                    :reduce="item => item.id"
                    :placeholder="$t('ChooseBelow')"
                    label="name"
                  ></v-select>
          ...
          <CCol>
            <div class="text-left">
              <CButton
                ...
                size="sm"
                color="danger"
                @click="$router.push({name : 'HomeSchoolingDisease'})"
              >
...
export default {
  data() {
    return {
      ...
      HomeSchoolingDisease: { TranslatableColumns: [] },
      selectedrowid: 0,
      translitefields: [
  ...
  created() {
    ...
    HomeSchoolingDiseaseService.Get(self.selectedrowid).then(res => {
      self.HomeSchoolingDisease = res.data;
    });
  },
  methods: {
    check() {
      ...
      if (
        self.HomeSchoolingDisease.code === 0 ||
        self.HomeSchoolingDisease.code === null ||
        self.HomeSchoolingDisease.code === undefined ||
        self.HomeSchoolingDisease.code === ""
      ) {
        this.makeToast(this.$t("codeNotCorrect"), this.$t("Error"), "danger");
        return false;
      }
      if (
        self.HomeSchoolingDisease.shortname === 0 ||
        self.HomeSchoolingDisease.shortname === null ||
        self.HomeSchoolingDisease.shortname === undefined ||
        self.HomeSchoolingDisease.shortname === ""
      ) {
        this.makeToast(
      ...
      if (
        self.HomeSchoolingDisease.fullname === 0 ||
        self.HomeSchoolingDisease.fullname === null ||
        self.HomeSchoolingDisease.fullname === undefined ||
        self.HomeSchoolingDisease.fullname === ""
      ) {
        this.makeToast(
      ...
      if (
        self.HomeSchoolingDisease.stateid === 0 ||
        self.HomeSchoolingDisease.stateid === null ||
        self.HomeSchoolingDisease.stateid === undefined ||
        self.HomeSchoolingDisease.stateid === ""
      ) {
        this.makeToast(this.$t("StatusNotCorrect"), this.$t("Error"), "danger");
        return false;
      }
      if (
        self.HomeSchoolingDisease.numberofmonth === null ||
        self.HomeSchoolingDisease.numberofmonth === undefined ||
        self.HomeSchoolingDisease.numberofmonth === ""
      ) {
        this.makeToast(
          this.$t("numberofmonthNotCorrect"),
          this.$t("Error"),
          "danger"
        );
        return false;
    ...
    SaveData() {
      ...
      var self = this;
      HomeSchoolingDiseaseService.Update(self.HomeSchoolingDisease)
        .then(res => {
          self.makeToast(
          ...
          self.$router.push({ name: "HomeSchoolingDisease" });
        })
        .catch(error => {
          self.makeToast(
    ...
    translate(selectcolumnname) {
      ...
      self.translatelist = self.HomeSchoolingDisease.Translates.filter(
    ...
    SaveTranslate() {
      var self = this;
      self.HomeSchoolingDisease.Translates = self.HomeSchoolingDisease.Translates.filter(
        x => x.columnname !== self.translatebyname
      );
      self.translatelist.forEach(item => {
        self.HomeSchoolingDisease.Translates.push(item);
      });
