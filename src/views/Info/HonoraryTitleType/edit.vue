    ...
    <CCol>
      <CCard accent-color="primary" class="pb-4">
        <CCardHeader>
          <CRow>
            <CCol>
              <h4 class="text-center">{{$t('HonoraryTitleType')}}</h4>
            </CCol>
          </CRow>
        </CCardHeader>
        <CCardBody>
          <ValidationObserver ref="form" v-slot="errors">
              ...
              <CCol sm="6">
                <ValidationProvider v-slot="v" name="code" rules="required|max:9">
                  <CInput
                    ...
                    horizontal
                    :label="$t('code')"
                    v-model="HonoraryTitleType.code"
              ...
              <CCol sm="6">
                <ValidationProvider v-slot="v" name="code" rules="required">
                  <CInput
                    ...
                    horizontal
                    :label="$t('shortname')"
                    v-model="HonoraryTitleType.shortname"
                    :addInputClasses="{'is-invalid': v.classes.invalid,'is-valid':  v.classes.valid }"
                  >
                    <template #append>
                      <CButton
                        v-if="HonoraryTitleType.TranslatableColumns.includes('shortname')"
              ...
              <CCol sm="6">
                <ValidationProvider v-slot="v" name="code" rules="required">
                  <CInput
                    ...
                    horizontal
                    :label="$t('fullname')"
                    v-model="HonoraryTitleType.fullname"
                    :addInputClasses="{'is-invalid': v.classes.invalid,'is-valid':  v.classes.valid }"
                  >
                    <template #append>
                      <CButton
                        v-if="HonoraryTitleType.TranslatableColumns.includes('fullname')"
            ...
            <CRow>
              ...
              <CCol sm="6">
                <ValidationProvider v-slot="v" name="code" rules="required|max:9">
                  <CInput
                    :placeholder="$t('amountcoef')"
                    autocomplete="amountcoef"
                    horizontal
                    :label="$t('amountcoef')"
                    v-model="HonoraryTitleType.amountcoef"
                    :addInputClasses="{'is-invalid': v.classes.invalid,'is-valid':  v.classes.valid }"
                  >
                    <template slot="invalid-feedback">
                      <div class="invalid-feedback">{{ v.errors[0] }}</div>
              ...
              <CCol sm="6">
                <div class="form-group form-row">
                  ...
                  <v-select
                    ...
                    v-model="HonoraryTitleType.minimumvaluetypeid"
                    :reduce="item => item.id"
              ...
              <CCol sm="6">
                <div class="form-group form-row">
                  ...
                  <v-select
                    class="col-sm-9"
                    :options="statelist"
                    v-model="HonoraryTitleType.stateid"
                    :reduce="item => item.id"
                    :placeholder="$t('ChooseBelow')"
                    label="name"
                  ></v-select>
          ...
          <CCol>
            <div class="text-left">
              <CButton
                ...
                size="sm"
                color="danger"
                @click="$router.push({name : 'HonoraryTitleType'})"
              >
...
export default {
  data() {
    return {
      ...
      HonoraryTitleType: { TranslatableColumns: [] },
      selectedrowid: 0,
      translitefields: [
  ...
  created() {
    ...
    HelperService.GetMinimumValueTypeList(this.lang).then(res => {
      this.minimumvaluetypelist = res.data;
    });
    HonoraryTitleTypeService.Get(self.selectedrowid).then(res => {
      self.HonoraryTitleType = res.data;
    });
  },
  methods: {
    check() {
      ...
      if (
        self.HonoraryTitleType.code === 0 ||
        self.HonoraryTitleType.code === null ||
        self.HonoraryTitleType.code === undefined ||
        self.HonoraryTitleType.code === ""
      ) {
        this.makeToast(this.$t("codeNotCorrect"), this.$t("Error"), "danger");
        return false;
      }
      if (
        self.HonoraryTitleType.shortname === 0 ||
        self.HonoraryTitleType.shortname === null ||
        self.HonoraryTitleType.shortname === undefined ||
        self.HonoraryTitleType.shortname === ""
      ) {
        this.makeToast(
      ...
      if (
        self.HonoraryTitleType.fullname === 0 ||
        self.HonoraryTitleType.fullname === null ||
        self.HonoraryTitleType.fullname === undefined ||
        self.HonoraryTitleType.fullname === ""
      ) {
        this.makeToast(
      ...
      if (
        self.HonoraryTitleType.stateid === 0 ||
        self.HonoraryTitleType.stateid === null ||
        self.HonoraryTitleType.stateid === undefined ||
        self.HonoraryTitleType.stateid === ""
      ) {
    ...
    SaveData() {
      ...
      var self = this;
      HonoraryTitleTypeService.Update(self.HonoraryTitleType)
        .then(res => {
          self.makeToast(
          ...
          self.$router.push({ name: "HonoraryTitleType" });
        })
        .catch(error => {
          self.makeToast(
    ...
    translate(selectcolumnname) {
      ...
      self.translatelist = self.HonoraryTitleType.Translates.filter(
    ...
    SaveTranslate() {
      var self = this;
      self.HonoraryTitleType.Translates = self.HonoraryTitleType.Translates.filter(
        x => x.columnname !== self.translatebyname
      );
      self.translatelist.forEach(item => {
        self.HonoraryTitleType.Translates.push(item);
      });
