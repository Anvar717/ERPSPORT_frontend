    ...
    <CCol>
      <CCard accent-color="primary" class="pb-4">
        <CCardHeader>
          <CRow>
            <CCol>
              <h4 class="text-center">{{$t('IdentityDocSeries')}}</h4>
            </CCol>
          </CRow>
        </CCardHeader>
        <CCardBody>
              ...
              <CCol sm="6">
                <ValidationProvider v-slot="v" name="code" rules="required|max:9">
                  <CInput
                    ...
                    horizontal
                    :label="$t('code')"
                    v-model="IdentityDocSeries.code"
              ...
              <CCol sm="6">
                <ValidationProvider v-slot="v" name="code" rules="required">
                  <CInput
                    ...
                    horizontal
                    :label="$t('fullname')"
                    v-model="IdentityDocSeries.fullname"
                    :addInputClasses="{'is-invalid': v.classes.invalid,'is-valid':  v.classes.valid }"
                  >
              ...
              <CCol sm="6">
                <ValidationProvider v-slot="v" name="code" rules="required">
                  <CInput
                    ...
                    horizontal
                    :label="$t('shortname')"
                    v-model="IdentityDocSeries.shortname"
                    :addInputClasses="{'is-invalid': v.classes.invalid,'is-valid':  v.classes.valid }"
                  >
              ...
              <CCol sm="6">
                <div class="form-group form-row">
                  ...
                  <v-select
                    class="col-sm-9"
                    :options="statelist"
                    v-model="IdentityDocSeries.stateid"
                    :reduce="item => item.id"
          ...
          <CCol>
            <div class="text-left">
              <CButton
                ...
                size="sm"
                color="danger"
                @click="$router.push({name : 'IdentityDocSeries'})"
              >
...
import IdentityDocSeriesService from "@/services/IdentityDocSeries.service";
export default {
  data() {
    return {
      SaveLoading: false,
      translatemodal: false,
      IdentityDocSeries: { TranslatableColumns: [] },
      selectedrowid: 0,
      translitefields: [
  ...
  created() {
    ...
    IdentityDocSeriesService.Get(self.selectedrowid).then(res => {
      self.IdentityDocSeries = res.data;
    });
  },
  methods: {
    check() {
      ...
      if (
        self.IdentityDocSeries.code === 0 ||
        self.IdentityDocSeries.code === null ||
        self.IdentityDocSeries.code === undefined ||
        self.IdentityDocSeries.code === ""
      ) {
        this.makeToast(this.$t("codeNotCorrect"), this.$t("Error"), "danger");
        return false;
      }
      if (
        self.IdentityDocSeries.shortname === 0 ||
        self.IdentityDocSeries.shortname === null ||
        self.IdentityDocSeries.shortname === undefined ||
        self.IdentityDocSeries.shortname === ""
      ) {
        this.makeToast(
      ...
      if (
        self.IdentityDocSeries.fullname === 0 ||
        self.IdentityDocSeries.fullname === null ||
        self.IdentityDocSeries.fullname === undefined ||
        self.IdentityDocSeries.fullname === ""
      ) {
        this.makeToast(
      ...
      if (
        self.IdentityDocSeries.stateid === 0 ||
        self.IdentityDocSeries.stateid === null ||
        self.IdentityDocSeries.stateid === undefined ||
        self.IdentityDocSeries.stateid === ""
      ) {
    ...
    SaveData() {
      ...
      var self = this;
      IdentityDocSeriesService.Update(self.IdentityDocSeries)
        .then(res => {
          self.makeToast(
          ...
          self.$router.push({ name: "IdentityDocSeries" });
        })
        .catch(error => {
          self.makeToast(
    ...
    translate(selectcolumnname) {
      ...
      self.translatelist = self.IdentityDocSeries.Translates.filter(
    ...
    SaveTranslate() {
      var self = this;
      self.IdentityDocSeries.Translates = self.IdentityDocSeries.Translates.filter(
        x => x.columnname !== self.translatebyname
      );
      self.translatelist.forEach(item => {
        self.IdentityDocSeries.Translates.push(item);
      });
