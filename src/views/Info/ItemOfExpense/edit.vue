    ...
    <CCol>
      <CCard accent-color="primary" class="pb-4">
        <CCardHeader>
          <CRow>
            <CCol>
              <h4 class="text-center">{{$t('ItemOfExpense')}}</h4>
            </CCol>
          </CRow>
        </CCardHeader>
        <CCardBody>
          ...
          <CRow>
            ...
            <CCol sm="6">
              <CInput
                ...
                horizontal
                :label="$t('shortname')"
                v-model="ItemOfExpense.shortname"
              >
                <template #append>
                  <CButton
                    @click="ShortNameLang"
          ...
          <CRow>
            ...
            <CCol sm="6">
              <CInput
                ...
                horizontal
                :label="$t('fullname')"
                v-model="ItemOfExpense.fullname"
              >
                <template #append>
                  <CButton
                    @click="FullNameLang"
          ...
          <CRow>
            ...
            <CCol sm="6">
              <CInput
                ...
                v-model="ItemOfExpense.internalcode"
              ></CInput>
            </CCol>
            <CCol sm="3"></CCol>
          </CRow>
            ...
            <CCol sm="6">
              <div class="form-group form-row">
                ...
                <v-select
                  class="col-sm-5"
                  :options="parentlist"
                  v-model="ItemOfExpense.parentid"
                  :reduce="item => item.id"
                  :placeholder="$t('ChooseBelow')"
                  label="name"
                ></v-select>
                <CCol sm="1"></CCol>
                <label class="col-form-label" for>{{$t('IsGroup')}}</label>
                <b-form-checkbox v-model="ItemOfExpense.isgroup" switch class="mt-2 ml-1"></b-form-checkbox>
              </div>
            </CCol>
          </CRow>
          <CRow>
            ...
            <CCol>
              <div class="text-left">
                <CButton
                  ...
                  size="sm"
                  color="danger"
                  @click="$router.push({name : 'ItemOfExpense'})"
                >
...
export default {
  ...
  created() {
    ...
    ItemOfExpenseService.Get(this.$route.params.id).then(res => {
      this.ItemOfExpense = res.data;
      this.items1 =
  ...
  methods: {
    SaveData() {
      ...
      ItemOfExpenseService.Update(this.ItemOfExpense)
        .then(res => {
          this.makeToast(
    ...
    check() {
      ...
      if (
        self.ItemOfExpense.code === 0 ||
        self.ItemOfExpense.code === null ||
        self.ItemOfExpense.code === undefined ||
        self.ItemOfExpense.code === ""
      ) {
        this.makeToast(this.$t("codeNotCorrect"), this.$t("Error"), "danger");
        return false;
      }
      if (
        self.ItemOfExpense.shortname === 0 ||
        self.ItemOfExpense.shortname === null ||
        self.ItemOfExpense.shortname === undefined ||
        self.ItemOfExpense.shortname === ""
      ) {
        this.makeToast(
      ...
      if (
        self.ItemOfExpense.fullname === 0 ||
        self.ItemOfExpense.fullname === null ||
        self.ItemOfExpense.fullname === undefined ||
        self.ItemOfExpense.fullname === ""
      ) {
        this.makeToast(
      ...
      if (
        self.ItemOfExpense.stateid === 0 ||
        self.ItemOfExpense.stateid === null ||
        self.ItemOfExpense.stateid === undefined ||
        self.ItemOfExpense.stateid === ""
      ) {
    ...
    AddItem() {
      if (this.TranslatebyName == "shortname") {
        this.ItemsPush();
        this.ItemOfExpenseTranslatePush();
    ...
    ItemOfExpenseTranslatePush() {
      this.ItemOfExpense.Translates.push({
        languageid: this.Translate.languageid,
        languagename: this.LanguageList.filter(
          item => item.id == this.Translate.languageid
        )[0].name,
