    ...
    <CCol>
      <CCard accent-color="primary" class="pb-4">
        <CCardHeader>
          <CRow>
            <CCol>
              <h4 class="text-center">{{$t('Oked')}}</h4>
            </CCol>
          </CRow>
        </CCardHeader>
        <CCardBody>
              ...
              <CCol sm="6">
                <ValidationProvider v-slot="v" name="code" rules="required">
                  <CInput
                    ...
                    horizontal
                    :label="$t('shortname')"
                    v-model="Oked.shortname"
                    :addInputClasses="{'is-invalid': v.classes.invalid,'is-valid':  v.classes.valid }"
                  >
                    <template #append>
                      <CButton
                        v-if="Oked.TranslatableColumns.includes('shortname')"
              ...
              <CCol sm="6">
                <ValidationProvider v-slot="v" name="code" rules="required">
                  <CInput
                    ...
                    horizontal
                    :label="$t('fullname')"
                    v-model="Oked.fullname"
                    :addInputClasses="{'is-invalid': v.classes.invalid,'is-valid':  v.classes.valid }"
                  >
                    <template #append>
                      <CButton
                        v-if="Oked.TranslatableColumns.includes('fullname')"
            ...
            <CRow>
              ...
              <CCol sm="6">
                <div class="form-group form-row">
                  ...
                  <b-form-checkbox v-model="Oked.isgroup" switch class="mt-2 col-sm-9"></b-form-checkbox>
                </div>
              </CCol>
            </CRow>
            <CRow>
        ...
        <CRow>
          <CCol>
            <div class="text-left">
              <CButton class="ml-4" size="sm" color="danger" @click="$router.push({name : 'Oked'})">
...
export default {
  ...
  created() {
    ...
    OkedService.Get(self.selectedrowid).then(res => {
      self.Oked = res.data;
    });
  },
  methods: {
    check() {
      ...
      if (
        self.Oked.shortname === 0 ||
        self.Oked.shortname === null ||
        self.Oked.shortname === undefined ||
        self.Oked.shortname === ""
      ) {
        this.makeToast(
      ...
      if (
        self.Oked.fullname === 0 ||
        self.Oked.fullname === null ||
        self.Oked.fullname === undefined ||
        self.Oked.fullname === ""
      ) {
        this.makeToast(
      ...
      if (
        self.Oked.parentid === 0 ||
        self.Oked.parentid === null ||
        self.Oked.parentid === undefined ||
        self.Oked.parentid === ""
      ) {
      ...
      if (
        self.Oked.stateid === 0 ||
        self.Oked.stateid === null ||
        self.Oked.stateid === undefined ||
        self.Oked.stateid === ""
      ) {
    ...
    translate(selectcolumnname) {
      ...
      self.translatelist = self.Oked.Translates.filter(
    ...
    SaveTranslate() {
      var self = this;
      self.Oked.Translates = self.Oked.Translates.filter(
        x => x.columnname !== self.translatebyname
      );
      self.translatelist.forEach(item => {
        self.Oked.Translates.push(item);
      });
