    ...
    <CCol>
      <CCard accent-color="primary" class="pb-4">
        <CCardHeader>
          <CRow>
            <CCol>
              <h4 class="text-center">{{$t('OlympiadWinnerType')}}</h4>
            </CCol>
          </CRow>
        </CCardHeader>
        <CCardBody>
              ...
              <CCol sm="6">
                <ValidationProvider v-slot="v" name="code" rules="required|max:9">
                  <CInput
                    ...
                    horizontal
                    :label="$t('code')"
                    v-model="OlympiadWinnerType.code"
              ...
              <CCol sm="6">
                <ValidationProvider v-slot="v" name="code" rules="required">
                  <CInput
                    ...
                    horizontal
                    :label="$t('shortname')"
                    v-model="OlympiadWinnerType.shortname"
                    :addInputClasses="{'is-invalid': v.classes.invalid,'is-valid':  v.classes.valid }"
                  >
                    <template #append>
                      <CButton
                        v-if="OlympiadWinnerType.TranslatableColumns.includes('shortname')"
              ...
              <CCol sm="6">
                <ValidationProvider v-slot="v" name="code" rules="required">
                  <CInput
                    ...
                    horizontal
                    :label="$t('fullname')"
                    v-model="OlympiadWinnerType.fullname"
                    :addInputClasses="{'is-invalid': v.classes.invalid,'is-valid':  v.classes.valid }"
                  >
                    <template #append>
                      <CButton
                        v-if="OlympiadWinnerType.TranslatableColumns.includes('fullname')"
              ...
              <CCol sm="6">
                <div class="form-group form-row">
                  ...
                  <v-select
                    class="col-sm-9"
                    :options="statelist"
                    v-model="OlympiadWinnerType.stateid"
                    :reduce="item => item.id"
          ...
          <CCol>
            <div class="text-left">
              <CButton
                ...
                size="sm"
                color="danger"
                @click="$router.push({name : 'OlympiadWinnerType'})"
              >
...
export default {
  data() {
    return {
      ...
      OlympiadWinnerType: { TranslatableColumns: [] },
      selectedrowid: 0,
      translitefields: [
  ...
  created() {
    ...
    OlympiadWinnerTypeService.Get(self.selectedrowid).then((res) => {
      self.OlympiadWinnerType = res.data;
    });
  },
  methods: {
    check() {
      ...
      if (
        self.OlympiadWinnerType.code === 0 ||
        self.OlympiadWinnerType.code === null ||
        self.OlympiadWinnerType.code === undefined ||
        self.OlympiadWinnerType.code === ""
      ) {
        this.makeToast(this.$t("codeNotCorrect"), this.$t("Error"), "danger");
        return false;
      }
      if (
        self.OlympiadWinnerType.shortname === 0 ||
        self.OlympiadWinnerType.shortname === null ||
        self.OlympiadWinnerType.shortname === undefined ||
        self.OlympiadWinnerType.shortname === ""
      ) {
        this.makeToast(
      ...
      if (
        self.OlympiadWinnerType.fullname === 0 ||
        self.OlympiadWinnerType.fullname === null ||
        self.OlympiadWinnerType.fullname === undefined ||
        self.OlympiadWinnerType.fullname === ""
      ) {
        this.makeToast(
      ...
      if (
        self.OlympiadWinnerType.stateid === 0 ||
        self.OlympiadWinnerType.stateid === null ||
        self.OlympiadWinnerType.stateid === undefined ||
        self.OlympiadWinnerType.stateid === ""
      ) {
    ...
    SaveData() {
      ...
      var self = this;
      OlympiadWinnerTypeService.Update(self.OlympiadWinnerType)
        .then((res) => {
          self.makeToast(
          ...
          self.$router.push({ name: "OlympiadWinnerType" });
        })
        .catch((error) => {
          self.makeToast(
    ...
    translate(selectcolumnname) {
      ...
      self.translatelist = self.OlympiadWinnerType.Translates.filter(
    ...
    SaveTranslate() {
      var self = this;
      self.OlympiadWinnerType.Translates = self.OlympiadWinnerType.Translates.filter(
        (x) => x.columnname !== self.translatebyname
      );
      self.translatelist.forEach((item) => {
        self.OlympiadWinnerType.Translates.push(item);
      });
