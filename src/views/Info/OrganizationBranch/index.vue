  ...
  <CRow>
    <CCol sm="12" md="12">
      <CCard accent-color="primary">
        <h3 class="text-center mt-3">{{ $t("OrganizationBranch") }}</h3>
        <CCardHeader>
          <CRow class="form-group justify-content-between">
            ...
            <CCol lg="7" md="6" sm="12" class="text-lg-right mt-2" style="margin-right: -7px;">
              <CButton
                ...
                class="mr-2"
                size="sm"
                v-if="$can('OrganizationBranchEdit','permissions')"
              >
            ...
            <template v-slot:cell(actions)="{ item }">
              <div class="text-center">
                ...
                <b-link
                  ...
                  v-if="$can('OrganizationBranchDelete','permissions')"
                  class="mr-2"
...
<script>
import OrganziationBranchService from '@/services/organizationbranch.service'
export default {
  data() {
    return {
      fields: [
  ...
  methods: {
    ...
    Edit(item) {
      if(this.$can('OrganizationBranchEdit','permissions')){
        this.$router.push({
          name: "EditOrganizationBranch",
          params: {
            id: item.id,
    ...
    Delete(item) {
      OrganziationBranchService.Delete(item.id)
        .then((res) => {
          this.makeToast(
    ...
    Refresh() {
      this.isBusy = true;
      OrganziationBranchService.GetList(
        this.filter.Search,
