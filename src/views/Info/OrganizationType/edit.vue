    ...
    <CCol>
      <CCard accent-color="primary" class="pb-4">
        <CCardHeader>
          <CRow>
            <CCol>
              <h4 class="text-center">{{$t('OrganizationType')}}</h4>
            </CCol>
          </CRow>
        </CCardHeader>
        <CCardBody>
              ...
              <CCol sm="6">
                <ValidationProvider v-slot="v" name="code" rules="required">
                  ...
                  >
                    <template #append>
                      <CButton
                        v-if="OrganizationType.TranslatableColumns.includes('shortname')"
              ...
              <CCol sm="6">
                <ValidationProvider v-slot="v" name="code" rules="required">
                  ...
                  >
                    <template #append>
                      <CButton
                        v-if="OrganizationType.TranslatableColumns.includes('fullname')"
          ...
          <CCol>
            <div class="text-left">
              <CButton
                ...
                size="sm"
                color="danger"
                @click="$router.push({name : 'OrganizationType'})"
              >
...
export default {
  data() {
    return {
      ...
      OrganizationType: { TranslatableColumns: [] },
      selectedrowid: 0,
      translitefields: [
  ...
  created() {
    ...
    OrganizationTypeService.Get(self.selectedrowid).then(res => {
      self.OrganizationType = res.data;
    });
  },
  methods: {
    check() {
      ...
      if (
        self.OrganizationType.code === 0 ||
        self.OrganizationType.code === null ||
        self.OrganizationType.code === undefined ||
        self.OrganizationType.code === ""
      ) {
        this.makeToast(this.$t("codeNotCorrect"), this.$t("Error"), "danger");
        return false;
      }
      if (
        self.OrganizationType.shortname === 0 ||
        self.OrganizationType.shortname === null ||
        self.OrganizationType.shortname === undefined ||
        self.OrganizationType.shortname === ""
      ) {
        this.makeToast(
      ...
      if (
        self.OrganizationType.fullname === 0 ||
        self.OrganizationType.fullname === null ||
        self.OrganizationType.fullname === undefined ||
        self.OrganizationType.fullname === ""
      ) {
        this.makeToast(
    ...
    SaveData() {
      ...
      var self = this;
      OrganizationTypeService.Update(self.OrganizationType)
        .then(res => {
          self.makeToast(
          ...
          self.$router.push({ name: "OrganizationType" });
        })
        .catch(error => {
          self.makeToast(
    ...
    translate(selectcolumnname) {
      ...
      self.translatelist = self.OrganizationType.Translates.filter(
    ...
    SaveTranslate() {
      var self = this;
      self.OrganizationType.Translates = self.OrganizationType.Translates.filter(
        x => x.columnname !== self.translatebyname
      );
      self.translatelist.forEach(item => {
        self.OrganizationType.Translates.push(item);
      });
