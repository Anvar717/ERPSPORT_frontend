    ...
    <CCol>
      <CCard accent-color="primary" class="pb-4">
        <CCardHeader>
          <CRow>
            <CCol>
              <h4 class="text-center">{{$t('RegionGroup')}}</h4>
            </CCol>
          </CRow>
        </CCardHeader>
        <CCardBody>
              ...
              <CCol sm="6">
                <ValidationProvider v-slot="v" name="code" rules="required">
                  <CInput
                    ...
                    horizontal
                    :label="$t('shortname')"
                    v-model="RegionGroup.shortname"
                    :addInputClasses="{'is-invalid': v.classes.invalid,'is-valid':  v.classes.valid }"
                  >
                    <template #append>
                      <CButton
                        v-if="RegionGroup.TranslatableColumns.includes('shortname')"
              ...
              <CCol sm="6">
                <ValidationProvider v-slot="v" name="code" rules="required">
                  <CInput
                    ...
                    horizontal
                    :label="$t('fullname')"
                    v-model="RegionGroup.fullname"
                    :addInputClasses="{'is-invalid': v.classes.invalid,'is-valid':  v.classes.valid }"
                  >
                    <template #append>
                      <CButton
                        v-if="RegionGroup.TranslatableColumns.includes('fullname')"
          ...
          <CCol>
            <div class="text-left">
              <CButton
                ...
                size="sm"
                color="danger"
                @click="$router.push({name : 'RegionGroup'})"
              >
...
export default {
  data() {
    return {
      ...
      RegionGroup: { TranslatableColumns: [] },
      selectedrowid: 0,
      translitefields: [
  ...
  created() {
    ...
    RegionGroupService.Get(self.selectedrowid).then(res => {
      self.RegionGroup = res.data;
    });
  },
  methods: {
    check() {
      ...
      if (
        self.RegionGroup.code === 0 ||
        self.RegionGroup.code === null ||
        self.RegionGroup.code === undefined ||
        self.RegionGroup.code === ""
      ) {
        this.makeToast(this.$t("codeNotCorrect"), this.$t("Error"), "danger");
        return false;
      }
      if (
        self.RegionGroup.shortname === 0 ||
        self.RegionGroup.shortname === null ||
        self.RegionGroup.shortname === undefined ||
        self.RegionGroup.shortname === ""
      ) {
        this.makeToast(
      ...
      if (
        self.RegionGroup.fullname === 0 ||
        self.RegionGroup.fullname === null ||
        self.RegionGroup.fullname === undefined ||
        self.RegionGroup.fullname === ""
      ) {
        this.makeToast(
      ...
      if (
        self.RegionGroup.stateid === 0 ||
        self.RegionGroup.stateid === null ||
        self.RegionGroup.stateid === undefined ||
        self.RegionGroup.stateid === ""
      ) {
    ...
    SaveData() {
      ...
      var self = this;
      RegionGroupService.Update(self.RegionGroup)
        .then(res => {
          self.makeToast(
          ...
          self.$router.push({ name: "RegionGroup" });
        })
        .catch(error => {
          self.makeToast(
    ...
    translate(selectcolumnname) {
      ...
      self.translatelist = self.RegionGroup.Translates.filter(
    ...
    SaveTranslate() {
      var self = this;
      self.RegionGroup.Translates = self.RegionGroup.Translates.filter(
        x => x.columnname !== self.translatebyname
      );
      self.translatelist.forEach(item => {
        self.RegionGroup.Translates.push(item);
      });
