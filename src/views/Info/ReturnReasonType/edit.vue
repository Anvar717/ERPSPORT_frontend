<template>
      ...
      <CCol>
        <CCard accent-color="primary" class="pb-4">
          <CCardHeader>
            <CRow>
              <CCol>
                <h4 class="text-center">{{$t('ReturnReasonType')}}</h4>
              </CCol>
            </CRow>
          </CCardHeader>
          <CCardBody>
            ...
            <CRow>
              ...
              <CCol sm="6">
                <CInput
                  ...
                  horizontal
                  :label="$t('shortname')"
                  v-model="ReturnReasonType.shortname"
                >
                  <template #append>
                    <CButton
                      @click="ShortNameLang"
            ...
            <CRow>
              ...
              <CCol sm="6">
                <CInput
                  ...
                  horizontal
                  :label="$t('fullname')"
                  v-model="ReturnReasonType.fullname"
                >
                  <template #append>
                    <CButton
                      @click="FullNameLang"
            ...
            <CRow>
              ...
              <CCol sm="6">
                <div class="form-group form-row">
                  <label class="col-form-label col-sm-3" for>{{$t('tablename')}}</label>
                  <v-select
                    class="col-sm-9"
                    :options="tablelist"
                    v-model="ReturnReasonType.tableid"
                    :reduce="item => item.id"
              ...
              <CCol sm="6">
                <div class="form-group form-row">
                  ...
                  <v-select
                    class="col-sm-9"
                    :options="statelist"
                    v-model="ReturnReasonType.stateid"
                    :reduce="item => item.id"
                    :placeholder="$t('ChooseBelow')"
                    label="name"
                  ></v-select>
              ...
              <CCol>
                <div class="text-left">
                  <CButton
                    ...
                    size="sm"
                    color="danger"
                    @click="$router.push({name : 'ReturnReasonType'})"
                  >
  ...
  export default {
    ...
    created() {
      ...
      ReturnReasonTypeService.Get(this.$route.params.id).then(res => {
        this.ReturnReasonType = res.data;
        this.items1 =
    ...
    methods: {
      SaveData() {
        ...
        ReturnReasonTypeService.Update(this.ReturnReasonType)
          .then(res => {
            this.makeToast(
      ...
      check() {
        ...
        if (
          self.ReturnReasonType.code === 0 ||
          self.ReturnReasonType.code === null ||
          self.ReturnReasonType.code === undefined ||
          self.ReturnReasonType.code === ""
        ) {
          this.makeToast(this.$t("codeNotCorrect"), this.$t("Error"), "danger");
          return false;
        }
        if (
          self.ReturnReasonType.shortname === 0 ||
          self.ReturnReasonType.shortname === null ||
          self.ReturnReasonType.shortname === undefined ||
          self.ReturnReasonType.shortname === ""
        ) {
          this.makeToast(
        ...
        if (
          self.ReturnReasonType.fullname === 0 ||
          self.ReturnReasonType.fullname === null ||
          self.ReturnReasonType.fullname === undefined ||
          self.ReturnReasonType.fullname === ""
        ) {
          this.makeToast(
        ...
        if (
          self.ReturnReasonType.stateid === 0 ||
          self.ReturnReasonType.stateid === null ||
          self.ReturnReasonType.stateid === undefined ||
          self.ReturnReasonType.stateid === ""
        ) {
      ...
      AddItem() {
        if (this.TranslatebyName == "shortname") {
          this.ItemsPush();
          this.ReturnReasonTypeTranslatePush();
      ...
      ReturnReasonTypeTranslatePush() {
        this.ReturnReasonType.Translates.push({
          languageid: this.Translate.languageid,
          languagename: this.LanguageList.filter(
            item => item.id == this.Translate.languageid
          )[0].name,
