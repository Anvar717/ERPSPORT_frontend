<template>
    <CRow>
      <CCol sm="12" md="12">
        <CCard accent-color="primary">
          <h3 class="text-center mt-3">{{ $t("ReturnReasonType") }}</h3>
          <CCardHeader>
            ...
            >
              ...
              <template v-slot:cell(actions)="{ item }">
                <div class="text-center">
                  <b-link
                  v-if="$can('ReturnReasonTypeEdit', 'permissions')"
                    @click="Edit(item)"
                    class="mr-2"
                ...
                <b-modal :id="'DeleteModal' + item.id" hide-footer>
                  ...
                  <div class="d-block text-right">
                    ...
                    <CButton 
                     v-if="$can('ReturnReasonTypeDelete', 'permissions')"
                     @click="Delete(item)" color="success">
  ...
  export default {
    ...
    methods: {
      ...
      Delete(item) {
        ReturnReasonTypeService.Delete(item.id)
          .then((res) => {
            this.makeToast(
      ...
      Refresh() {
        this.isBusy = true;
        ReturnReasonTypeService.GetList(
          this.filter.Search,
