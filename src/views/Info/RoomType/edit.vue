    ...
    <CCol>
      <CCard accent-color="primary" class="pb-4">
        <CCardHeader>
          <CRow>
            <CCol>
              <h4 class="text-center">{{$t('RoomType')}}</h4>
            </CCol>
          </CRow>
        </CCardHeader>
        <CCardBody>
              ...
              <CCol sm="6">
                <ValidationProvider v-slot="v" name="code" rules="required">
                  <CInput
                    ...
                    horizontal
                    :label="$t('shortname')"
                    v-model="RoomType.shortname"
                    :addInputClasses="{'is-invalid': v.classes.invalid,'is-valid':  v.classes.valid }"
                  >
                    <template #append>
                      <CButton
                        v-if="RoomType.TranslatableColumns.includes('shortname')"
              ...
              <CCol sm="6">
                <ValidationProvider v-slot="v" name="code" rules="required">
                  <CInput
                    ...
                    horizontal
                    :label="$t('fullname')"
                    v-model="RoomType.fullname"
                    :addInputClasses="{'is-invalid': v.classes.invalid,'is-valid':  v.classes.valid }"
                  >
                    <template #append>
                      <CButton
                        v-if="RoomType.TranslatableColumns.includes('fullname')"
            ...
            <CRow>
              ...
              <CCol sm="6">
                <div class="form-group form-row">
                  ...
                  <b-form-checkbox v-model="RoomType.isgroup" switch class="mt-2 col-sm-9"></b-form-checkbox>
                </div>
              </CCol>
              <CCol sm="3"></CCol>
            </CRow>
          ...
          <CCol>
            <div class="text-left">
              <CButton
                ...
                size="sm"
                color="danger"
                @click="$router.push({name : 'RoomType'})"
              >
...
export default {
  ...
  created() {
    ...
    RoomTypeService.Get(self.selectedrowid).then(res => {
  ...
  methods: {
    check() {
      ...
      if (
        self.RoomType.code === 0 ||
        self.RoomType.code === null ||
        self.RoomType.code === undefined ||
        self.RoomType.code === ""
      ) {
      ...
      if (
        self.RoomType.shortname === 0 ||
        self.RoomType.shortname === null ||
        self.RoomType.shortname === undefined ||
        self.RoomType.shortname === ""
      ) {
        this.makeToast(
      ...
      if (
        self.RoomType.fullname === 0 ||
        self.RoomType.fullname === null ||
        self.RoomType.fullname === undefined ||
        self.RoomType.fullname === ""
      ) {
        this.makeToast(
      ...
      if (
        self.RoomType.stateid === 0 ||
        self.RoomType.stateid === null ||
        self.RoomType.stateid === undefined ||
        self.RoomType.stateid === ""
      ) {
    ...
    SaveData() {
      ...
      var self = this;
      RoomTypeService.Update(self.RoomType)
        .then(res => {
          self.makeToast(
          ...
          self.$router.push({ name: "RoomType" });
        })
        .catch(error => {
          self.makeToast(
    ...
    translate(selectcolumnname) {
      ...
      self.translatelist = self.RoomType.Translates.filter(
    ...
    SaveTranslate() {
      var self = this;
      self.RoomType.Translates = self.RoomType.Translates.filter(
        x => x.columnname !== self.translatebyname
      );
      self.translatelist.forEach(item => {
        self.RoomType.Translates.push(item);
      });
