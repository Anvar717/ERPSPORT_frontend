  ...
  <CRow>
    <CCol sm="12" md="12">
      <CCard accent-color="primary">
        <h3 class="text-center mt-3">{{ $t("SchoolGradeGroup") }}</h3>
        <CCardHeader>
          <CRow class="form-group justify-content-between">
            ...
            <CCol lg="7" md="6" sm="12" class="text-lg-right mt-2" style="margin-right: -7px;">
              <CButton
                v-if="$can('SchoolGradeGroupEdit', 'permissions')"
                @click="Create"
                color="primary"
                variant="outline"
                class="mr-2"
        ...
        <CCardBody>
          ...
          >
            ...
            <template v-slot:head(workhours)>
              <div
                class="text-center"
                style="cursor : pointer"
                @click="ChangeSort('workhours')"
              >
                {{ $t("workhours") }}
                <b-icon v-if="filter.SortColumn === 'workhours'" :icon="filter.SortIcon"></b-icon>
              </div>
            </template>
            <template v-slot:head(teachinghours)>
              <div
                class="text-center"
                style="cursor : pointer"
                @click="ChangeSort('teachinghours')"
              >
                {{ $t("teachinghours") }}
                <b-icon v-if="filter.SortColumn === 'teachinghours'" :icon="filter.SortIcon"></b-icon>
              </div>
            </template>
            <template v-slot:head(totalhours)>
              <div
                class="text-center"
                style="cursor : pointer"
                @click="ChangeSort('totalhours')"
              >
                {{ $t("totalhours") }}
                <b-icon v-if="filter.SortColumn === 'totalhours'" :icon="filter.SortIcon"></b-icon>
              </div>
            </template>
            <template v-slot:head(monthlytotalhours)>
              <div
                class="text-center"
                style="cursor : pointer"
                @click="ChangeSort('monthlytotalhours')"
              >
                {{ $t("monthlytotalhours") }}
                <b-icon v-if="filter.SortColumn === 'monthlytotalhours'" :icon="filter.SortIcon"></b-icon>
              </div>
            </template>
            ...
            <template v-slot:cell(actions)="{ item }">
              <div class="text-center">
                ...
                <b-link
                  v-if="$can('SchoolGradeGroupDelete', 'permissions')"
...
export default {
  data() {
    return {
      fields: [
        ...
        {
          key: "workhours",
          label: this.$t("workhours"),
          sortable: false,
          class: "text-left",
        },
        {
          key: "teachinghours",
          label: this.$t("teachinghours"),
          sortable: false,
          class: "text-left",
        },
        {
          key: "totalhours",
          label: this.$t("totalhours"),
          sortable: false,
          class: "text-left",
        },
        {
          key: "monthlytotalhours",
          label: this.$t("monthlytotalhours"),
          sortable: false,
          class: "text-left",
        },
        {
  ...
  methods: {
    ...
    Delete(item) {
      SchoolGradeGroupService.Delete(item.id)
        .then((res) => {
          this.makeToast(
    ...
    Refresh() {
      this.isBusy = true;
      SchoolGradeGroupService.GetList(
        this.filter.Search,
