    ...
    <CCol>
      <CCard accent-color="primary" class="pb-4">
        <CCardHeader>
          <CRow>
            <CCol>
              <h4 class="text-center">{{$t('SchoolGradeLetter')}}</h4>
            </CCol>
          </CRow>
        </CCardHeader>
        <CCardBody>
              ...
              <CCol sm="6">
                <ValidationProvider v-slot="v" name="code" rules="required|max:9">
                  <CInput
                    ...
                    horizontal
                    :label="$t('code')"
                    v-model="SchoolGradeLetter.code"
              ...
              <CCol sm="6">
                <ValidationProvider v-slot="v" name="code" rules="required">
                  <CInput
                    ...
                    horizontal
                    :label="$t('shortname')"
                    v-model="SchoolGradeLetter.shortname"
                    :addInputClasses="{'is-invalid': v.classes.invalid,'is-valid':  v.classes.valid }"
                  >
                    <template #append>
                      <CButton
                        v-if="SchoolGradeLetter.TranslatableColumns.includes('shortname')"
              ...
              <CCol sm="6">
                <ValidationProvider v-slot="v" name="code" rules="required">
                  <CInput
                    ...
                    horizontal
                    :label="$t('fullname')"
                    v-model="SchoolGradeLetter.fullname"
                    :addInputClasses="{'is-invalid': v.classes.invalid,'is-valid':  v.classes.valid }"
                  >
                    <template #append>
                      <CButton
                        v-if="SchoolGradeLetter.TranslatableColumns.includes('fullname')"
              ...
              <CCol sm="6">
                <div class="form-group form-row">
                  ...
                  <v-select
                    class="col-sm-9"
                    :options="statelist"
                    v-model="SchoolGradeLetter.stateid"
                    :reduce="item => item.id"
          ...
          <CCol>
            <div class="text-left">
              <CButton
                ...
                size="sm"
                color="danger"
                @click="$router.push({name : 'SchoolGradeLetter'})"
              >
...
export default {
  data() {
    return {
      ...
      SchoolGradeLetter: { TranslatableColumns: [] },
      selectedrowid: 0,
      translitefields: [
  ...
  created() {
    ...
    SchoolGradeLetterService.Get(self.selectedrowid).then(res => {
      self.SchoolGradeLetter = res.data;
    });
  },
  methods: {
    check() {
      ...
      if (
        self.SchoolGradeLetter.code === 0 ||
        self.SchoolGradeLetter.code === null ||
        self.SchoolGradeLetter.code === undefined ||
        self.SchoolGradeLetter.code === ""
      ) {
        this.makeToast(this.$t("codeNotCorrect"), this.$t("Error"), "danger");
        return false;
      }
      if (
        self.SchoolGradeLetter.shortname === 0 ||
        self.SchoolGradeLetter.shortname === null ||
        self.SchoolGradeLetter.shortname === undefined ||
        self.SchoolGradeLetter.shortname === ""
      ) {
        this.makeToast(
      ...
      if (
        self.SchoolGradeLetter.fullname === 0 ||
        self.SchoolGradeLetter.fullname === null ||
        self.SchoolGradeLetter.fullname === undefined ||
        self.SchoolGradeLetter.fullname === ""
      ) {
        this.makeToast(
      ...
      if (
        self.SchoolGradeLetter.stateid === 0 ||
        self.SchoolGradeLetter.stateid === null ||
        self.SchoolGradeLetter.stateid === undefined ||
        self.SchoolGradeLetter.stateid === ""
      ) {
    ...
    SaveData() {
      ...
      var self = this;
      SchoolGradeLetterService.Update(self.SchoolGradeLetter)
        .then(res => {
          self.makeToast(
          ...
          self.$router.push({ name: "SchoolGradeLetter" });
        })
        .catch(error => {
          self.makeToast(
    ...
    translate(selectcolumnname) {
      ...
      self.translatelist = self.SchoolGradeLetter.Translates.filter(
    ...
    SaveTranslate() {
      var self = this;
      self.SchoolGradeLetter.Translates = self.SchoolGradeLetter.Translates.filter(
        x => x.columnname !== self.translatebyname
      );
      self.translatelist.forEach(item => {
        self.SchoolGradeLetter.Translates.push(item);
      });
