    ...
    <CCol>
      <CCard accent-color="primary" class="pb-4">
        <CCardHeader>
          <CRow>
            <CCol>
              <h4 class="text-center">{{$t('SchoolGroupContingent')}}</h4>
            </CCol>
          </CRow>
        </CCardHeader>
        <CCardBody>
              ...
              <CCol sm="6">
                <ValidationProvider v-slot="v" name="code" rules="required">
                  ...
                  >
                    <template #append>
                      <CButton
                        v-if="SchoolGroupContingent.TranslatableColumns.includes('shortname')"
              ...
              <CCol sm="6">
                <ValidationProvider v-slot="v" name="code" rules="required">
                  ...
                  >
                    <template #append>
                      <CButton
                        v-if="SchoolGroupContingent.TranslatableColumns.includes('fullname')"
          ...
          <CCol>
            <div class="text-left">
              <CButton
                ...
                size="sm"
                color="danger"
                @click="$router.push({name : 'SchoolGroupContingent'})"
              >
...
export default {
  data() {
    return {
      ...
      SchoolGroupContingent: { TranslatableColumns: [] },
      selectedrowid: 0,
      translitefields: [
  ...
  created() {
    ...
    SchoolGroupContingentService.Get(self.selectedrowid).then(res => {
      self.SchoolGroupContingent = res.data;
    });
  },
  methods: {
    check() {
      ...
      if (
        self.SchoolGroupContingent.code === 0 ||
        self.SchoolGroupContingent.code === null ||
        self.SchoolGroupContingent.code === undefined ||
        self.SchoolGroupContingent.code === ""
      ) {
        this.makeToast(this.$t("codeNotCorrect"), this.$t("Error"), "danger");
        return false;
      }
      if (
        self.SchoolGroupContingent.shortname === 0 ||
        self.SchoolGroupContingent.shortname === null ||
        self.SchoolGroupContingent.shortname === undefined ||
        self.SchoolGroupContingent.shortname === ""
      ) {
        this.makeToast(
      ...
      if (
        self.SchoolGroupContingent.fullname === 0 ||
        self.SchoolGroupContingent.fullname === null ||
        self.SchoolGroupContingent.fullname === undefined ||
        self.SchoolGroupContingent.fullname === ""
      ) {
        this.makeToast(
      ...
      if (
        self.SchoolGroupContingent.stateid === 0 ||
        self.SchoolGroupContingent.stateid === null ||
        self.SchoolGroupContingent.stateid === undefined ||
        self.SchoolGroupContingent.stateid === ""
      ) {
    ...
    SaveData() {
      ...
      var self = this;
      SchoolGroupContingentService.Update(self.SchoolGroupContingent)
        .then(res => {
          self.makeToast(
          ...
          self.$router.push({ name: "SchoolGroupContingent" });
        })
        .catch(error => {
          self.makeToast(
    ...
    translate(selectcolumnname) {
      ...
      self.translatelist = self.SchoolGroupContingent.Translates.filter(
    ...
    SaveTranslate() {
      var self = this;
      self.SchoolGroupContingent.Translates = self.SchoolGroupContingent.Translates.filter(
        x => x.columnname !== self.translatebyname
      );
      self.translatelist.forEach(item => {
        self.SchoolGroupContingent.Translates.push(item);
      });
