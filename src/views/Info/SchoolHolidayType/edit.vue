    ...
    <CCol>
      <CCard accent-color="primary" class="pb-4">
        <CCardHeader>
          <CRow>
            <CCol>
              <h4 class="text-center">{{$t('SchoolHolidayType')}}</h4>
            </CCol>
          </CRow>
        </CCardHeader>
        <CCardBody>
              ...
              <CCol sm="6">
                <ValidationProvider v-slot="v" name="code" rules="required|max:9">
                  <CInput
                    ...
                    horizontal
                    :label="$t('code')"
                    v-model="SchoolHolidayType.code"
              ...
              <CCol sm="6">
                <ValidationProvider v-slot="v" name="code" rules="required">
                  <CInput
                    ...
                    horizontal
                    :label="$t('shortname')"
                    v-model="SchoolHolidayType.shortname"
                    :addInputClasses="{'is-invalid': v.classes.invalid,'is-valid':  v.classes.valid }"
                  >
                    <template #append>
                      <CButton
                        v-if="SchoolHolidayType.TranslatableColumns.includes('shortname')"
              ...
              <CCol sm="6">
                <ValidationProvider v-slot="v" name="code" rules="required">
                  <CInput
                    ...
                    horizontal
                    :label="$t('fullname')"
                    v-model="SchoolHolidayType.fullname"
                    :addInputClasses="{'is-invalid': v.classes.invalid,'is-valid':  v.classes.valid }"
                  >
                    <template #append>
                      <CButton
                        v-if="SchoolHolidayType.TranslatableColumns.includes('fullname')"
              ...
              <CCol sm="6">
                <div class="form-group form-row">
                  ...
                  <v-select
                    class="col-sm-9"
                    :options="statelist"
                    v-model="SchoolHolidayType.stateid"
                    :reduce="item => item.id"
          ...
          <CCol>
            <div class="text-left">
              <CButton
                ...
                size="sm"
                color="danger"
                @click="$router.push({name : 'SchoolHolidayType'})"
              >
...
export default {
  data() {
    return {
      ...
      SchoolHolidayType: { TranslatableColumns: [] },
      selectedrowid: 0,
      translitefields: [
  ...
  created() {
    ...
    SchoolHolidayTypeService.Get(self.selectedrowid).then(res => {
      self.SchoolHolidayType = res.data;
    });
  },
  methods: {
    check() {
      ...
      if (
        self.SchoolHolidayType.code === 0 ||
        self.SchoolHolidayType.code === null ||
        self.SchoolHolidayType.code === undefined ||
        self.SchoolHolidayType.code === ""
      ) {
        this.makeToast(this.$t("codeNotCorrect"), this.$t("Error"), "danger");
        return false;
      }
      if (
        self.SchoolHolidayType.shortname === 0 ||
        self.SchoolHolidayType.shortname === null ||
        self.SchoolHolidayType.shortname === undefined ||
        self.SchoolHolidayType.shortname === ""
      ) {
        this.makeToast(
      ...
      if (
        self.SchoolHolidayType.fullname === 0 ||
        self.SchoolHolidayType.fullname === null ||
        self.SchoolHolidayType.fullname === undefined ||
        self.SchoolHolidayType.fullname === ""
      ) {
        this.makeToast(
      ...
      if (
        self.SchoolHolidayType.stateid === 0 ||
        self.SchoolHolidayType.stateid === null ||
        self.SchoolHolidayType.stateid === undefined ||
        self.SchoolHolidayType.stateid === ""
      ) {
    ...
    SaveData() {
      ...
      var self = this;
      SchoolHolidayTypeService.Update(self.SchoolHolidayType)
        .then(res => {
          self.makeToast(
          ...
          self.$router.push({ name: "SchoolHolidayType" });
        })
        .catch(error => {
          self.makeToast(
    ...
    translate(selectcolumnname) {
      ...
      self.translatelist = self.SchoolHolidayType.Translates.filter(
    ...
    SaveTranslate() {
      var self = this;
      self.SchoolHolidayType.Translates = self.SchoolHolidayType.Translates.filter(
        x => x.columnname !== self.translatebyname
      );
      self.translatelist.forEach(item => {
        self.SchoolHolidayType.Translates.push(item);
      });
