    ...
    <CCol>
      <CCard accent-color="primary" class="pb-4">
        <CCardHeader>
          <CRow>
            <CCol>
              <h4 class="text-center">{{$t('SchoolLegalForm')}}</h4>
            </CCol>
          </CRow>
        </CCardHeader>
        <CCardBody>
              ...
              <CCol sm="6">
                <ValidationProvider v-slot="v" name="code" rules="required">
                  ...
                  >
                    <template #append>
                      <CButton
                        v-if="SchoolLegalForm.TranslatableColumns.includes('shortname')"
              ...
              <CCol sm="6">
                <ValidationProvider v-slot="v" name="code" rules="required">
                  ...
                  >
                    <template #append>
                      <CButton
                        v-if="SchoolLegalForm.TranslatableColumns.includes('fullname')"
          ...
          <CCol>
            <div class="text-left">
              <CButton
                ...
                size="sm"
                color="danger"
                @click="$router.push({name : 'SchoolLegalForm'})"
              >
...
export default {
  data() {
    return {
      ...
      SchoolLegalForm: { TranslatableColumns: [] },
      selectedrowid: 0,
      translitefields: [
  ...
  created() {
    ...
    SchoolLegalFormService.Get(self.selectedrowid).then(res => {
      self.SchoolLegalForm = res.data;
    });
  },
  methods: {
    check() {
      ...
      if (
        self.SchoolLegalForm.code === 0 ||
        self.SchoolLegalForm.code === null ||
        self.SchoolLegalForm.code === undefined ||
        self.SchoolLegalForm.code === ""
      ) {
        this.makeToast(this.$t("codeNotCorrect"), this.$t("Error"), "danger");
        return false;
      }
      if (
        self.SchoolLegalForm.shortname === 0 ||
        self.SchoolLegalForm.shortname === null ||
        self.SchoolLegalForm.shortname === undefined ||
        self.SchoolLegalForm.shortname === ""
      ) {
        this.makeToast(
      ...
      if (
        self.SchoolLegalForm.fullname === 0 ||
        self.SchoolLegalForm.fullname === null ||
        self.SchoolLegalForm.fullname === undefined ||
        self.SchoolLegalForm.fullname === ""
      ) {
        this.makeToast(
      ...
      if (
        self.SchoolLegalForm.stateid === 0 ||
        self.SchoolLegalForm.stateid === null ||
        self.SchoolLegalForm.stateid === undefined ||
        self.SchoolLegalForm.stateid === ""
      ) {
    ...
    SaveData() {
      ...
      var self = this;
      SchoolLegalFormService.Update(self.SchoolLegalForm)
        .then(res => {
          self.makeToast(
          ...
          self.$router.push({ name: "SchoolLegalForm" });
        })
        .catch(error => {
          self.makeToast(
    ...
    translate(selectcolumnname) {
      ...
      self.translatelist = self.SchoolLegalForm.Translates.filter(
    ...
    SaveTranslate() {
      var self = this;
      self.SchoolLegalForm.Translates = self.SchoolLegalForm.Translates.filter(
        x => x.columnname !== self.translatebyname
      );
      self.translatelist.forEach(item => {
        self.SchoolLegalForm.Translates.push(item);
      });
