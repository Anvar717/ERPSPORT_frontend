    ...
    <CCol>
      <CCard accent-color="primary" class="pb-4">
        <CCardHeader>
          <CRow>
            <CCol>
              <h4 class="text-center">{{$t('SchoolSubjectGroup')}}</h4>
            </CCol>
          </CRow>
        </CCardHeader>
        <CCardBody>
              ...
              <CCol sm="6">
                <ValidationProvider v-slot="v" name="code" rules="required">
                  ...
                  >
                    <template #append>
                      <CButton
                        v-if="SchoolSubjectGroup.TranslatableColumns.includes('shortname')"
              ...
              <CCol sm="6">
                <ValidationProvider v-slot="v" name="code" rules="required">
                  ...
                  >
                    <template #append>
                      <CButton
                        v-if="SchoolSubjectGroup.TranslatableColumns.includes('fullname')"
          ...
          <CCol>
            <div class="text-left">
              <CButton
                ...
                size="sm"
                color="danger"
                @click="$router.push({name : 'SchoolSubjectGroup'})"
              >
...
export default {
  data() {
    return {
      ...
      SchoolSubjectGroup: { TranslatableColumns: [] },
      selectedrowid: 0,
      translitefields: [
  ...
  created() {
    ...
    SchoolSubjectGroupService.Get(self.selectedrowid).then(res => {
      self.SchoolSubjectGroup = res.data;
    });
  },
  methods: {
    check() {
      ...
      if (
        self.SchoolSubjectGroup.code === 0 ||
        self.SchoolSubjectGroup.code === null ||
        self.SchoolSubjectGroup.code === undefined ||
        self.SchoolSubjectGroup.code === ""
      ) {
        this.makeToast(this.$t("codeNotCorrect"), this.$t("Error"), "danger");
        return false;
      }
      if (
        self.SchoolSubjectGroup.shortname === 0 ||
        self.SchoolSubjectGroup.shortname === null ||
        self.SchoolSubjectGroup.shortname === undefined ||
        self.SchoolSubjectGroup.shortname === ""
      ) {
        this.makeToast(
      ...
      if (
        self.SchoolSubjectGroup.fullname === 0 ||
        self.SchoolSubjectGroup.fullname === null ||
        self.SchoolSubjectGroup.fullname === undefined ||
        self.SchoolSubjectGroup.fullname === ""
      ) {
        this.makeToast(
      ...
      if (
        self.SchoolSubjectGroup.stateid === 0 ||
        self.SchoolSubjectGroup.stateid === null ||
        self.SchoolSubjectGroup.stateid === undefined ||
        self.SchoolSubjectGroup.stateid === ""
      ) {
    ...
    SaveData() {
      ...
      var self = this;
      SchoolSubjectGroupService.Update(self.SchoolSubjectGroup)
        .then(res => {
          self.makeToast(
          ...
          self.$router.push({ name: "SchoolSubjectGroup" });
        })
        .catch(error => {
          self.makeToast(
    ...
    translate(selectcolumnname) {
      ...
      self.translatelist = self.SchoolSubjectGroup.Translates.filter(
    ...
    SaveTranslate() {
      var self = this;
      self.SchoolSubjectGroup.Translates = self.SchoolSubjectGroup.Translates.filter(
        x => x.columnname !== self.translatebyname
      );
      self.translatelist.forEach(item => {
        self.SchoolSubjectGroup.Translates.push(item);
      });
