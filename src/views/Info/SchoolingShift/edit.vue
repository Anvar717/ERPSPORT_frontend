    ...
    <CCol>
      <CCard accent-color="primary" class="pb-4">
        <CCardHeader>
          <CRow>
            <CCol>
              <h4 class="text-center">{{$t('SchoolingShift')}}</h4>
            </CCol>
          </CRow>
        </CCardHeader>
        <CCardBody>
              ...
              <CCol sm="6">
                <ValidationProvider v-slot="v" name="code" rules="required">
                  <CInput
                    ...
                    horizontal
                    :label="$t('shortname')"
                    v-model="SchoolingShift.shortname"
                    :addInputClasses="{'is-invalid': v.classes.invalid,'is-valid':  v.classes.valid }"
                  >
                    <template #append>
                      <CButton
                        v-if="SchoolingShift.TranslatableColumns.includes('shortname')"
              ...
              <CCol sm="6">
                <ValidationProvider v-slot="v" name="code" rules="required">
                  <CInput
                    ...
                    horizontal
                    :label="$t('fullname')"
                    v-model="SchoolingShift.fullname"
                    :addInputClasses="{'is-invalid': v.classes.invalid,'is-valid':  v.classes.valid }"
                  >
                    <template #append>
                      <CButton
                        v-if="SchoolingShift.TranslatableColumns.includes('fullname')"
              ...
              <CCol sm="6">
                <div class="form-group form-row">
                  ...
                  <v-select
                    class="col-sm-9"
                    :options="statelist"
                    v-model="SchoolingShift.stateid"
                    :reduce="item => item.id"
          ...
          <CCol>
            <div class="text-left">
              <CButton
                ...
                size="sm"
                color="danger"
                @click="$router.push({name : 'SchoolingShift'})"
              >
...
export default {
  data() {
    return {
      ...
      SchoolingShift: { TranslatableColumns: [] },
      selectedrowid: 0,
      translitefields: [
  ...
  created() {
    ...
    SchoolingShiftService.Get(self.selectedrowid).then(res => {
      self.SchoolingShift = res.data;
    });
  },
  methods: {
    check() {
      ...
      if (
        self.SchoolingShift.code === 0 ||
        self.SchoolingShift.code === null ||
        self.SchoolingShift.code === undefined ||
        self.SchoolingShift.code === ""
      ) {
        this.makeToast(this.$t("codeNotCorrect"), this.$t("Error"), "danger");
        return false;
      }
      if (
        self.SchoolingShift.shortname === 0 ||
        self.SchoolingShift.shortname === null ||
        self.SchoolingShift.shortname === undefined ||
        self.SchoolingShift.shortname === ""
      ) {
        this.makeToast(
      ...
      if (
        self.SchoolingShift.fullname === 0 ||
        self.SchoolingShift.fullname === null ||
        self.SchoolingShift.fullname === undefined ||
        self.SchoolingShift.fullname === ""
      ) {
        this.makeToast(
      ...
      if (
        self.SchoolingShift.stateid === 0 ||
        self.SchoolingShift.stateid === null ||
        self.SchoolingShift.stateid === undefined ||
        self.SchoolingShift.stateid === ""
      ) {
    ...
    SaveData() {
      ...
      var self = this;
      SchoolingShiftService.Update(self.SchoolingShift)
        .then(res => {
          self.makeToast(
          ...
          self.$router.push({ name: "SchoolingShift" });
        })
        .catch(error => {
          self.makeToast(
    ...
    translate(selectcolumnname) {
      ...
      self.translatelist = self.SchoolingShift.Translates.filter(
    ...
    SaveTranslate() {
      var self = this;
      self.SchoolingShift.Translates = self.SchoolingShift.Translates.filter(
        x => x.columnname !== self.translatebyname
      );
      self.translatelist.forEach(item => {
        self.SchoolingShift.Translates.push(item);
      });
