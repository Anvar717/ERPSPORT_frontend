    ...
    <CCol>
      <CCard accent-color="primary" class="pb-4">
        <CCardHeader>
          <CRow>
            <CCol>
              <h4 class="text-center">{{$t('StaffCategory')}}</h4>
            </CCol>
          </CRow>
        </CCardHeader>
        <CCardBody>
              ...
              <CCol sm="6">
                <ValidationProvider v-slot="v" name="code" rules="required">
                  ...
                  >
                    <template #append>
                      <CButton
                        v-if="StaffCategory.TranslatableColumns.includes('shortname')"
              ...
              <CCol sm="6">
                <ValidationProvider v-slot="v" name="code" rules="required">
                  ...
                  >
                    <template #append>
                      <CButton
                        v-if="StaffCategory.TranslatableColumns.includes('fullname')"
          ...
          <CCol>
            <div class="text-left">
              <CButton
                ...
                size="sm"
                color="danger"
                @click="$router.push({name : 'StaffCategory'})"
              >
...
export default {
  data() {
    return {
      ...
      StaffCategory: { TranslatableColumns: [] },
      selectedrowid: 0,
      translitefields: [
  ...
  created() {
    ...
    StaffCategoryService.Get(self.selectedrowid).then(res => {
      self.StaffCategory = res.data;
    });
  },
  methods: {
    check() {
      ...
      if (
        self.StaffCategory.code === 0 ||
        self.StaffCategory.code === null ||
        self.StaffCategory.code === undefined ||
        self.StaffCategory.code === ""
      ) {
        this.makeToast(this.$t("codeNotCorrect"), this.$t("Error"), "danger");
        return false;
      }
      if (
        self.StaffCategory.shortname === 0 ||
        self.StaffCategory.shortname === null ||
        self.StaffCategory.shortname === undefined ||
        self.StaffCategory.shortname === ""
      ) {
        this.makeToast(
      ...
      if (
        self.StaffCategory.fullname === 0 ||
        self.StaffCategory.fullname === null ||
        self.StaffCategory.fullname === undefined ||
        self.StaffCategory.fullname === ""
      ) {
        this.makeToast(
      ...
      if (
        self.StaffCategory.stateid === 0 ||
        self.StaffCategory.stateid === null ||
        self.StaffCategory.stateid === undefined ||
        self.StaffCategory.stateid === ""
      ) {
    ...
    SaveData() {
      ...
      var self = this;
      StaffCategoryService.Update(self.StaffCategory)
        .then(res => {
          self.makeToast(
          ...
          self.$router.push({ name: "StaffCategory" });
        })
        .catch(error => {
          self.makeToast(
    ...
    translate(selectcolumnname) {
      ...
      self.translatelist = self.StaffCategory.Translates.filter(
    ...
    SaveTranslate() {
      var self = this;
      self.StaffCategory.Translates = self.StaffCategory.Translates.filter(
        x => x.columnname !== self.translatebyname
      );
      self.translatelist.forEach(item => {
        self.StaffCategory.Translates.push(item);
      });
