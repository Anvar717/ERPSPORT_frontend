    ...
    <CCol>
      <CCard accent-color="primary" class="pb-4">
        <CCardHeader>
          <CRow>
            <CCol>
              <h4 class="text-center">{{ $t("StreetState") }}</h4>
            </CCol>
          </CRow>
        </CCardHeader>
        <CCardBody>
          ...
          <CCol>
            <div class="text-left">
              <CButton
                ...
                size="sm"
                color="danger"
                @click="$router.push({ name: 'StreetState' })"
              >
...
import StreetStateService from "@/services/StreetState.service";
...
export default {
  ...
  created() {
    ...
    StreetStateService.Get(self.selectedrowid).then((res) => {
      self.StreetState = res.data;
      this.getOrgList();
    });
  ...
  methods: {
    // ChangeOblast() {
    //   this.StreetState.regionid = 0;
    //   this.StreetState.streetid = 0;
    //   this.getRegionList();
    // },
    // getRegionList() {
    //   if (!!this.StreetState.oblastid) {
    //     this.StreetState.oblastname = this.Oblastlist.filter(
    //       (item) => item.id === this.StreetState.oblastid
    //     )[0].name;
    //     RegionService.GetAll(this.StreetState.oblastid).then((res) => {
    //       this.regionlist = res.data;
    //     });
    //   }
    // },
    getOrgList() {
      if (!!this.StreetState.regionid) {
        ...
        this.Street.street = this.streetlist.filter(
          (item) => item.id === this.Street,
          streetid
        )[0].name;
      }
    },
    ...
    check() {
      ...
      if (
        self.StreetState.ondate === 0 ||
        self.StreetState.ondate === null ||
        self.StreetState.ondate === undefined ||
        self.StreetState.ondate === ""
      ) {
        this.makeToast(this.$t("codeNotCorrect"), this.$t("Error"), "danger");
        return false;
      }
      if (
        self.StreetState.regionid === 0 ||
        self.StreetState.regionid === null ||
        self.StreetState.regionid === undefined ||
        self.StreetState.regionid === ""
      ) {
        this.makeToast(this.$t("regionNotCorrect"), this.$t("Error"), "danger");
        return false;
      }
      if (
        self.StreetState.streetid === 0 ||
        self.StreetState.streetid === null ||
        self.StreetState.streetid === undefined ||
        self.StreetState.streetid === ""
      ) {
        this.makeToast(this.$t("StreetNotCorrect"), this.$t("Error"), "danger");
        return false;
      }
      if (
        self.StreetState.stateid === 0 ||
        self.StreetState.stateid === null ||
        self.StreetState.stateid === undefined ||
        self.StreetState.stateid === ""
      ) {
    ...
    SaveData() {
      ...
      console.log(this.StreetState);
      var self = this;
      StreetStateService.Update(self.StreetState)
        .then((res) => {
          self.makeToast(
          ...
          self.$router.push({ name: "StreetState" });
        })
        .catch((error) => {
          self.makeToast(
    ...
    translate(selectcolumnname) {
      ...
      self.translatelist = self.StreetState.Translates.filter(
    ...
    SaveTranslate() {
      var self = this;
      self.StreetState.Translates = self.StreetState.Translates.filter(
        (x) => x.columnname !== self.translatebyname
      );
      self.translatelist.forEach((item) => {
        self.StreetState.Translates.push(item);
      });
