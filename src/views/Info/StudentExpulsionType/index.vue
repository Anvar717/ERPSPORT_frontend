  ...
  <CRow>
    <CCol sm="12" md="12">
      <CCard accent-color="primary">
        <h3 class="text-center mt-3">{{ $t("StudentExpulsionType") }}</h3>
        <CCardHeader>
          <CRow class="form-group justify-content-between">
            ...
            <CCol lg="7" md="6" sm="12" class="text-lg-right mt-2" style="margin-right: -7px;">
              <CButton
                v-if="$can('StudentExpulsionTypeEdit', 'permissions')"
                @click="Create"
                color="primary"
                variant="outline"
                class="mr-2"
        ...
        <CCardBody>
          ...
          >
            ...
            <template v-slot:cell(isgraduation)="{ item }">
              <CBadge
                :color="item.isgraduation === true ? 'primary' : 'danger'"
              >
                {{ item.isgraduation === true ? $t("yes") : $t("no") }}
              </CBadge>
              <!-- <p class="m-0" :color="getItemColor(item.isgraduation)">
                {{ item.isgraduation === true ? $t("yes") : $t("no") }}
              </p> -->
            </template>
            <template v-slot:cell(istoschool)="{ item }">
              <CBadge :color="item.istoschool === true ? 'primary' : 'danger'">
                {{ item.istoschool === true ? $t("yes") : $t("no") }}
              </CBadge>
            </template>
            ...
            <template v-slot:cell(actions)="{ item }">
              <div class="text-center">
                ...
                <b-link
                  v-if="$can('StudentExpulsionTypeDelete', 'permissions')"
...
export default {
  data() {
    return {
      fields: [
        ...
        {
          key: "isgraduation",
          label: this.$t("isgraduation"),
          sortable: false,
          class: "text-left",
        },
        {
          key: "istoschool",
          label: this.$t("istoschool"),
          sortable: false,
          class: "text-left",
        },
        {
  ...
  methods: {
    ...
    Delete(item) {
      StudentExpulsionTypeService.Delete(item.id)
        .then((res) => {
          this.makeToast(
    ...
    Refresh() {
      this.isBusy = true;
      StudentExpulsionTypeService.GetList(
        this.filter.Search,
