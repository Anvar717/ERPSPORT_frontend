  ...
  <CRow>
    <CCol sm="12" md="12">
      <CCard accent-color="primary">
        <h3 class="text-center mt-3">{{ $t("TableColumnTemplate") }}</h3>
        <CCardHeader>
          <CRow class="form-group justify-content-between">
            ...
            <CCol lg="7" md="6" sm="12" class="text-lg-right mt-2" style="margin-right: -7px;">
              <CButton
                v-if="$can('TableColumnTemplateEdit', 'permissions')"
                @click="Create"
                color="primary"
                variant="outline"
                class="mr-2"
        ...
        <CCardBody>
          ...
          >
            ...
            <template v-slot:head(ordernumber)>
              ...
              >
                {{ $t("ordernumber") }}
                <b-icon v-if="filter.SortColumn === 'ordernumber'" :icon="filter.SortIcon"></b-icon>
              </div>
            </template>
            <template v-slot:head(tablename)>
              <div
                class="text-center"
                style="cursor : pointer"
                @click="ChangeSort('tablename')"
              >
                {{ $t("tablename") }}
                <b-icon v-if="filter.SortColumn === 'tablename'" :icon="filter.SortIcon"></b-icon>
              </div>
            </template>
            <template v-slot:head(columnname)>
              <div
                class="text-center"
                style="cursor : pointer"
              ...
              >
                {{ $t("columnname") }}
                <b-icon v-if="filter.SortColumn === 'columnname'" :icon="filter.SortIcon"></b-icon>
              </div>
            </template>
            <template v-slot:head(templatetext)>
              <div
                class="text-center"
                style="cursor : pointer"
                @click="ChangeSort('templatetext')"
              >
                {{ $t("templatetext") }}
                <b-icon v-if="filter.SortColumn === 'templatetext'" :icon="filter.SortIcon"></b-icon>
              </div>
            </template>
            <template v-slot:head(isother)>
              <div
                class="text-center"
                style="cursor : pointer"
                @click="ChangeSort('isother')"
              >
                {{ $t("isother") }}
                <b-icon v-if="filter.SortColumn === 'isother'" :icon="filter.SortIcon"></b-icon>
              </div>
            </template>
            ...
            <template v-slot:cell(actions)="{ item }">
              <div class="text-center">
                ...
                <b-link
                  v-if="$can('TableColumnTemplateDelete', 'permissions')"
...
export default {
  data() {
    return {
      fields: [
        ...
        {
          key: "templatetext",
          label: this.$t("templatetext"),
          sortable: false,
          class: "text-left",
        },
        {
          key: "isother",
          label: this.$t("isother"),
          sortable: false,
          class: "text-left",
        },
        {
  ...
  methods: {
    ...
    Delete(item) {
      TableColumnTemplateService.Delete(item.id)
        .then((res) => {
          this.makeToast(
    ...
    Refresh() {
      this.isBusy = true;
      TableColumnTemplateService.GetList(
        this.filter.Search,
