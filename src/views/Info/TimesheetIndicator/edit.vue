    ...
    <CCol>
      <CCard accent-color="primary" class="pb-4">
        <CCardHeader>
          <CRow>
            <CCol>
              <h4 class="text-center">{{$t('TimesheetIndicator')}}</h4>
            </CCol>
          </CRow>
        </CCardHeader>
        <CCardBody>
              ...
              <CCol sm="6">
                <ValidationProvider v-slot="v" name="code" rules="required|max:9">
                  <CInput
                    ...
                    horizontal
                    :label="$t('code')"
                    v-model="TimesheetIndicator.code"
              ...
              <CCol sm="6">
                <ValidationProvider v-slot="v" name="code" rules="required">
                  <CInput
                    ...
                    horizontal
                    :label="$t('shortname')"
                    v-model="TimesheetIndicator.shortname"
                    :addInputClasses="{'is-invalid': v.classes.invalid,'is-valid':  v.classes.valid }"
                  >
                    <template #append>
                      <CButton
                        v-if="TimesheetIndicator.TranslatableColumns.includes('shortname')"
              ...
              <CCol sm="6">
                <ValidationProvider v-slot="v" name="code" rules="required">
                  <CInput
                    ...
                    horizontal
                    :label="$t('fullname')"
                    v-model="TimesheetIndicator.fullname"
                    :addInputClasses="{'is-invalid': v.classes.invalid,'is-valid':  v.classes.valid }"
                  >
                    <template #append>
                      <CButton
                        v-if="TimesheetIndicator.TranslatableColumns.includes('fullname')"
              ...
              <CCol sm="6">
                <div class="form-group form-row">
                  ...
                  <v-select
                    class="col-sm-9"
                    :options="statelist"
                    v-model="TimesheetIndicator.stateid"
                    :reduce="item => item.id"
          ...
          <CCol>
            <div class="text-left">
              <CButton
                ...
                size="sm"
                color="danger"
                @click="$router.push({name : 'TimesheetIndicator'})"
              >
...
export default {
  data() {
    return {
      ...
      TimesheetIndicator: { TranslatableColumns: [] },
      selectedrowid: 0,
      translitefields: [
  ...
  created() {
    ...
    TimesheetIndicatorService.Get(self.selectedrowid).then(res => {
      self.TimesheetIndicator = res.data;
    });
  },
  methods: {
    check() {
      ...
      if (
        self.TimesheetIndicator.code === 0 ||
        self.TimesheetIndicator.code === null ||
        self.TimesheetIndicator.code === undefined ||
        self.TimesheetIndicator.code === ""
      ) {
        this.makeToast(this.$t("codeNotCorrect"), this.$t("Error"), "danger");
        return false;
      }
      if (
        self.TimesheetIndicator.shortname === 0 ||
        self.TimesheetIndicator.shortname === null ||
        self.TimesheetIndicator.shortname === undefined ||
        self.TimesheetIndicator.shortname === ""
      ) {
        this.makeToast(
      ...
      if (
        self.TimesheetIndicator.fullname === 0 ||
        self.TimesheetIndicator.fullname === null ||
        self.TimesheetIndicator.fullname === undefined ||
        self.TimesheetIndicator.fullname === ""
      ) {
        this.makeToast(
      ...
      if (
        self.TimesheetIndicator.stateid === 0 ||
        self.TimesheetIndicator.stateid === null ||
        self.TimesheetIndicator.stateid === undefined ||
        self.TimesheetIndicator.stateid === ""
      ) {
    ...
    SaveData() {
      ...
      var self = this;
      TimesheetIndicatorService.Update(self.TimesheetIndicator)
        .then(res => {
          self.makeToast(
          ...
          self.$router.push({ name: "TimesheetIndicator" });
        })
        .catch(error => {
          self.makeToast(
    ...
    translate(selectcolumnname) {
      ...
      self.translatelist = self.TimesheetIndicator.Translates.filter(
    ...
    SaveTranslate() {
      var self = this;
      self.TimesheetIndicator.Translates = self.TimesheetIndicator.Translates.filter(
        x => x.columnname !== self.translatebyname
      );
      self.translatelist.forEach(item => {
        self.TimesheetIndicator.Translates.push(item);
      });
