  ...
  <CRow>
    <CCol sm="12" md="12">
      <CCard accent-color="primary">
        <h3 class="text-center mt-3">{{ $t("TimesheetIndicator") }}</h3>
        <CCardHeader>
          <CRow class="form-group justify-content-between">
            ...
            <CCol lg="7" md="6" sm="12" class="text-lg-right mt-2" style="margin-right: -7px;">
              <CButton
                v-if="$can('TimesheetIndicatorEdit', 'permissions')"
                @click="Create"
                color="primary"
                variant="outline"
                class="mr-2"
          ...
          >
            ...
            <template v-slot:cell(actions)="{ item }">
              <div class="text-center">
                ...
                <b-link
                  v-if="$can('TimesheetIndicatorDelete', 'permissions')"
...
export default {
  ...
  methods: {
    ...
    Delete(item) {
      TimesheetIndicatorService.Delete(item.id)
        .then((res) => {
          this.makeToast(
    ...
    Refresh() {
      this.isBusy = true;
      TimesheetIndicatorService.GetList(
        this.filter.Search,
