    ...
    <CCol>
      <CCard accent-color="primary" class="pb-4">
        <CCardHeader>
          <CRow>
            <CCol>
              <h4 class="text-center">{{ $t("EquipmentConditionType") }}</h4>
            </CCol>
          </CRow>
        </CCardHeader>
        <CCardBody>
              ...
              <CCol sm="6">
                ...
                >
                  <CInput
                    ...
                    :label="$t('code1')"
                    type="number"
                    v-model="EquipmentConditionType.code"
                    :addInputClasses="{
              ...
              <CCol sm="6">
                <ValidationProvider v-slot="v" name="code" rules="required">
                  <CInput
                    ...
                    horizontal
                    :label="$t('shortname')"
                    v-model="EquipmentConditionType.shortname"
                    :addInputClasses="{
                      'is-invalid': v.classes.invalid,
                      'is-valid': v.classes.valid,
                    }"
                  >
                    <template #append>
                      <CButton
                        v-if="
                          EquipmentConditionType.TranslatableColumns.includes(
                            'shortname'
                          )
                        "
              ...
              <CCol sm="6">
                <ValidationProvider v-slot="v" name="code" rules="required">
                  <CInput
                    ...
                    horizontal
                    :label="$t('fullname')"
                    v-model="EquipmentConditionType.fullname"
                    :addInputClasses="{
                      'is-invalid': v.classes.invalid,
                      'is-valid': v.classes.valid,
                    }"
              ...
              <CCol sm="6">
                <div class="form-group form-row">
                  ...
                  <v-select
                    class="col-sm-9"
                    :options="statelist"
                    v-model="EquipmentConditionType.stateid"
                    :reduce="(item) => item.id"
          ...
          <CCol>
            <div class="text-left">
              <CButton
                ...
                size="sm"
                color="danger"
                @click="$router.push({ name: 'EquipmentConditionType' })"
              >
...
export default {
  data() {
    return {
      ...
      EquipmentConditionType: { TranslatableColumns: [] },
      selectedrowid: 0,
      translitefields: [
  ...
  created() {
    ...
    EquipmentConditionTypeService.Get(this.$route.params.id).then((res) => {
      self.EquipmentConditionType = res.data;
    });
  },
  methods: {
    translate(selectcolumnname) {
      ...
      self.translatelist = self.EquipmentConditionType.Translates.filter(
    ...
    SaveTranslate() {
      var self = this;
      self.EquipmentConditionType.Translates = self.EquipmentConditionType.Translates.filter(
        (x) => x.columnname !== self.translatebyname
      );
      self.translatelist.forEach((item) => {
        self.EquipmentConditionType.Translates.push(item);
      });
    ...
    check() {
      ...
      if (checkCode(self.EquipmentConditionType.code)) {
        this.makeToast(this.$t("codeNotCorrect"), this.$t("Error"), "danger");
        return false;
      }
      if (checkStringSpace(self.EquipmentConditionType.shortname)) {
        this.makeToast(
      ...
      if (checkStringSpace(self.EquipmentConditionType.fullname)) {
        this.makeToast(
      ...
      //   }
      if (
        self.EquipmentConditionType.stateid === 0 ||
        self.EquipmentConditionType.stateid === null ||
        self.EquipmentConditionType.stateid === undefined ||
        self.EquipmentConditionType.stateid === ""
      ) {
    ...
    SaveData() {
      ...
      var self = this;
      EquipmentConditionTypeService.Update(self.EquipmentConditionType)
        .then((res) => {
          self.makeToast(
          ...
          self.$router.push({ name: "EquipmentConditionType" });
        })
        .catch((error) => {
          self.makeToast(
