    ...
    <CCol>
      <CCard accent-color="primary" class="pb-4">
        <CCardHeader>
          <CRow>
            <CCol>
              <h4 class="text-center">{{ $t("Manufacturer") }}</h4>
            </CCol>
          </CRow>
        </CCardHeader>
        <CCardBody>
              ...
              <CCol sm="6">
                <ValidationProvider v-slot="v" name="code" rules="required">
                  <CInput
                    ...
                    horizontal
                    :label="$t('shortname')"
                    v-model="Manufacturer.shortname"
                    :addInputClasses="{
                      'is-invalid': v.classes.invalid,
                      'is-valid': v.classes.valid,
                    }"
                  >
                    <!-- <template #append>
                      <CButton
                        v-if="
                          Manufacturer.TranslatableColumns.includes('shortname')
                        "
                ...
                <ValidationProvider v-slot="v" name="code" rules="required">
                  ...
                  >
                    <!-- <template #append>
                      <CButton
                        v-if="
                          Manufacturer.TranslatableColumns.includes('fullname')
                        "
        ...
        <b-row class="mx-3">
          <b-col>
            ...
            >
              ...
              <template v-slot:cell(countryname)="{ item }">
          ...
          <CCol>
            <div class="text-left">
              <CButton
                ...
                size="sm"
                color="danger"
                @click="$router.push({ name: 'Manufacturer' })"
              >
...
export default {
  data() {
    return {
      ...
      Manufacturer: { TranslatableColumns: [] },
      selectedrowid: 0,
      translitefields: [
  ...
  created() {
    ...
    ManufacturerService.Get(this.$route.params.id).then((res) => {
  ...
  methods: {
    ...
    translate(selectcolumnname) {
      ...
      self.translatelist = self.Manufacturer.Translates.filter(
    ...
    SaveTranslate() {
      var self = this;
      self.Manufacturer.Translates = self.Manufacturer.Translates.filter(
        (x) => x.columnname !== self.translatebyname
      );
      self.translatelist.forEach((item) => {
        self.Manufacturer.Translates.push(item);
      });
    ...
    AddTable() {
      var self = this;
      if (
        self.tabrow.countryid === 0 ||
        self.tabrow.countryid === null ||
        self.tabrow.countryid === undefined ||
        self.tabrow.countryid === ""
      ) {
        this.makeToast(
      ...
      self.Manufacturer.Tables.push(self.tabrow);
      self.tabrow = {
        id: 0,
        ownerid: 0,
        countryid: 0,
    ...
    check() {
      ...
      if (checkCode(self.Manufacturer.code)) {
        this.makeToast(this.$t("codeNotCorrect"), this.$t("Error"), "danger");
        return false;
      }
      if (checkStringSpace(self.Manufacturer.shortname)) {
        this.makeToast(
      ...
      if (checkStringSpace(self.Manufacturer.fullname)) {
        this.makeToast(
      ...
      if (
        self.Manufacturer.stateid === 0 ||
        self.Manufacturer.stateid === null ||
        self.Manufacturer.stateid === undefined ||
        self.Manufacturer.stateid === ""
      ) {
        this.makeToast(this.$t("StatusNotCorrect"), this.$t("Error"), "danger");
        return false;
      }
      // if (this.Manufacturer.Tables.length > 0) {
      //   for (let i = 0; i < this.Manufacturer.Tables.length; i++) {
      //     this.Manufacturer.Tables[i].Status = 3;
      //   }
      //   this.ExamGroupTable = this.Manufacturer.Tables;
      // }
      if (
        this.Manufacturer.Tables.filter((item) => item.status != 3).length == 0
      ) {
        this.makeToast(
          this.$t("countryNotCorrect"),
          this.$t("Error"),
    ...
    SaveData() {
      ...
      var self = this;
      ManufacturerService.Update(self.Manufacturer)
        .then((res) => {
          self.makeToast(
          ...
          self.$router.push({ name: "Manufacturer" });
        })
        .catch((error) => {
          self.makeToast(
