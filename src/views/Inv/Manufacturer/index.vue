  ...
  <CRow>
    <CCol sm="12" md="12">
      <CCard accent-color="primary">
        <h3 class="text-center mt-3">{{ $t("Manufacturer") }}</h3>
        <CCardHeader>
          <CRow class="form-group">
            ...
            <CCol lg="7" md="6" sm="12" class="text-lg-right mt-2">
              <CButton
                ...
                size="sm"
              >
                <!-- v-if="$can('ManufacturerEdit', 'permissions')" -->
        ...
        <CCardBody>
          ...
          >
            ...
            <template v-slot:head(countryname)>
              <div
                class="text-center"
                style="cursor: pointer"
                @click="ChangeSort('countryname')"
              >
                {{ $t("country") }}
                <b-icon
                  v-if="filter.SortColumn === 'countryname'"
                  :icon="filter.SortIcon"
                ></b-icon>
              </div>
            </template>
...
export default {
  ...
  methods: {
    ...
    Delete(item) {
      ManufacturerService.Delete(item.id)
        .then((res) => {
          this.makeToast(
