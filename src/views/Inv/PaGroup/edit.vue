    ...
    <CCol>
      <CCard accent-color="primary" class="pb-4">
        <CCardHeader>
          <CRow>
            <CCol>
              <h4 class="text-center">{{ $t("PaGroup") }}</h4>
            </CCol>
          </CRow>
        </CCardHeader>
        <CCardBody>
          <ValidationObserver ref="form" v-slot="errors">
              ...
              <CCol sm="6">
                <ValidationProvider v-slot="v" name="code" rules="required">
                  ...
                  >
                    <!-- <template #append>
                      <CButton
                        v-if="PaGroup.TranslatableColumns.includes('shortname')"
              ...
              <CCol sm="6">
                <ValidationProvider v-slot="v" name="code" rules="required">
                  ...
                  >
                    <!-- <template #append>
                      <CButton
                        v-if="PaGroup.TranslatableColumns.includes('fullname')"
            ...
            <CRow>
              ...
              <CCol sm="6">
                ...
                >
                  <CInput
                    :placeholder="$t('ageingrate')"
                    autocomplete="ageingrate"
                    horizontal
                    type="number"
                    min="0"
                    :label="$t('ageingrate')"
                    v-model="PaGroup.ageingrate"
                    @keyup="ChangeAgeingrate"
                    :addInputClasses="{
                      'is-invalid': v.classes.invalid,
                      'is-valid': v.classes.valid,
                    }"
          ...
          <CCol>
            <div class="text-left">
              <CButton
                ...
                size="sm"
                color="danger"
                @click="$router.push({ name: 'PaGroup' })"
              >
...
export default {
  ...
  created() {
    ...
    PaGroupService.Get(this.$route.params.id).then((res) => {
      self.PaGroup = res.data;
    });
  ...
  methods: {
    ...
    translate(selectcolumnname) {
      ...
      self.translatelist = self.PaGroup.Translates.filter(
    ...
    SaveTranslate() {
      var self = this;
      self.PaGroup.Translates = self.PaGroup.Translates.filter(
        (x) => x.columnname !== self.translatebyname
      );
      self.translatelist.forEach((item) => {
        self.PaGroup.Translates.push(item);
      });
    ...
    check() {
      ...
      if (checkCode(self.PaGroup.code)) {
        this.makeToast(this.$t("codeNotCorrect"), this.$t("Error"), "danger");
        return false;
      }
      if (
        self.PaGroup.ageingrate === null ||
        self.PaGroup.ageingrate === undefined ||
        self.PaGroup.ageingrate === "" ||
        !Number.isInteger(Number(self.PaGroup.ageingrate)) ||
        Number(self.PaGroup.ageingrate) < 0
      ) {
        this.makeToast(
          this.$t("ageingrateNotCorrect"),
          this.$t("Error"),
          "danger"
        );
        return false;
      ...
      if (checkStringSpace(self.PaGroup.shortname)) {
        this.makeToast(
      ...
      if (checkStringSpace(self.PaGroup.fullname)) {
        this.makeToast(
      ...
      //   }
      if (
        self.PaGroup.stateid === 0 ||
        self.PaGroup.stateid === null ||
        self.PaGroup.stateid === undefined ||
        self.PaGroup.stateid === ""
      ) {
    ...
    SaveData() {
      ...
      var self = this;
      PaGroupService.Update(self.PaGroup)
        .then((res) => {
          self.makeToast(
