    ...
    <CCol>
      <CCard accent-color="primary" class="pb-4">
        <CCardHeader>
          <CRow>
            <CCol>
              <h4 class="text-center">{{ $t("PaSubGroup") }}</h4>
            </CCol>
          </CRow>
        </CCardHeader>
        <CCardBody>
                ...
                <ValidationProvider v-slot="v" name="code" rules="required">
                  ...
                  >
                    <!-- <template #append>
                      <CButton
                        v-if="
                          PaSubGroup.TranslatableColumns.includes('shortname')
                        "
                ...
                <ValidationProvider v-slot="v" name="code" rules="required">
                  ...
                  >
                    <!-- <template #append>
                      <CButton
                        v-if="
                          PaSubGroup.TranslatableColumns.includes('fullname')
                        "
          ...
          <CCol>
            <div class="text-left">
              <CButton
                ...
                size="sm"
                color="danger"
                @click="$router.push({ name: 'PaSubGroup' })"
              >
...
export default {
  data() {
    return {
      ...
      PaSubGroup: { TranslatableColumns: [] },
      selectedrowid: 0,
      translitefields: [
  ...
  created() {
    ...
    PaSubGroupService.Get(this.$route.params.id).then((res) => {
      self.PaSubGroup = res.data;
    });
  },
  methods: {
    translate(selectcolumnname) {
      ...
      self.translatelist = self.PaSubGroup.Translates.filter(
    ...
    SaveTranslate() {
      var self = this;
      self.PaSubGroup.Translates = self.PaSubGroup.Translates.filter(
        (x) => x.columnname !== self.translatebyname
      );
      self.translatelist.forEach((item) => {
        self.PaSubGroup.Translates.push(item);
      });
    ...
    check() {
      ...
      if (checkCode(self.PaSubGroup.code)) {
        this.makeToast(this.$t("codeNotCorrect"), this.$t("Error"), "danger");
        return false;
      }
      if (checkStringSpace(self.PaSubGroup.shortname)) {
        this.makeToast(
      ...
      if (checkStringSpace(self.PaSubGroup.fullname)) {
        this.makeToast(
      ...
      //   }
      if (
        self.PaSubGroup.stateid === 0 ||
        self.PaSubGroup.stateid === null ||
        self.PaSubGroup.stateid === undefined ||
        self.PaSubGroup.stateid === ""
      ) {
    ...
    SaveData() {
      ...
      var self = this;
      PaSubGroupService.Update(self.PaSubGroup)
        .then((res) => {
          self.makeToast(
          ...
          self.$router.push({ name: "PaSubGroup" });
        })
        .catch((error) => {
          self.makeToast(
