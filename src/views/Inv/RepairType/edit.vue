    ...
    <CCol>
      <CCard accent-color="primary" class="pb-4">
        <CCardHeader>
          <CRow>
            <CCol>
              <h4 class="text-center">{{ $t("RepairType") }}</h4>
            </CCol>
          </CRow>
        </CCardHeader>
        <CCardBody>
                ...
                <ValidationProvider v-slot="v" name="code" rules="required">
                  ...
                  >
                    <template #append>
                      <CButton
                        v-if="
                          RepairType.TranslatableColumns.includes('shortname')
                        "
                ...
                <ValidationProvider v-slot="v" name="code" rules="required">
                  ...
                  >
                    <template #append>
                      <CButton
                        v-if="
                          RepairType.TranslatableColumns.includes('fullname')
                        "
...
export default {
  data() {
    return {
      ...
      RepairType: { TranslatableColumns: [] },
      selectedrowid: 0,
      translitefields: [
  ...
  created() {
    ...
    RepairTypeService.Get(this.$route.params.id).then((res) => {
      self.RepairType = res.data;
    });
  },
  methods: {
    translate(selectcolumnname) {
      ...
      self.translatelist = self.RepairType.Translates.filter(
    ...
    SaveTranslate() {
      var self = this;
      self.RepairType.Translates = self.RepairType.Translates.filter(
        (x) => x.columnname !== self.translatebyname
      );
      self.translatelist.forEach((item) => {
        self.RepairType.Translates.push(item);
      });
    ...
    check() {
      ...
      if (checkCode(self.RepairType.code)) {
        this.makeToast(this.$t("codeNotCorrect"), this.$t("Error"), "danger");
        return false;
      }
      if (checkStringSpace(self.RepairType.shortname)) {
        this.makeToast(
      ...
      if (checkStringSpace(self.RepairType.fullname)) {
        this.makeToast(
      ...
      //   }
      if (
        self.RepairType.stateid === 0 ||
        self.RepairType.stateid === null ||
        self.RepairType.stateid === undefined ||
        self.RepairType.stateid === ""
      ) {
    ...
    SaveData() {
      ...
      var self = this;
      RepairTypeService.Update(self.RepairType)
        .then((res) => {
          self.makeToast(
          ...
          self.$router.push({ name: "RepairType" });
        })
        .catch((error) => {
          self.makeToast(
