  ...
  <CRow>
    <CCol sm="12" md="12">
      <CCard accent-color="primary">
        <h3 class="text-center mt-3">{{ $t("ResponsiblePerson") }}</h3>
        <CCardHeader>
          <CRow class="form-group">
            ...
            <CCol lg="7" md="6" sm="12" class="text-lg-right mt-2">
              <CButton
                ...
                size="sm"
              >
                <!-- v-if="$can('ResponsiblePersonEdit', 'permissions')" -->
...
export default {
  ...
  methods: {
    ...
    Delete(item) {
      ResponsiblePersonService.Delete(item.id)
        .then((res) => {
          this.makeToast(
    ...
    Refresh() {
      this.isBusy = true;
      ResponsiblePersonService.GetList(
        this.filter.Search,
