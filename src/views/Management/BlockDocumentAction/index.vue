  ...
  <CRow>
    <CCol sm="12" md="12">
      <CCard accent-color="primary">
        <h3 class="text-center mt-3">{{ $t("BlockDocumentAction") }}</h3>
        <CCardHeader>
          <CRow class="form-group justify-content-between">
            ...
            <CCol lg="7" md="6" sm="12" class="text-lg-right mt-2" style="margin-right: -7px;">
              <CButton
                v-if="$can('BlockDocumentActionEdit', 'permissions')"
                @click="Create"
                color="primary"
                variant="outline"
                class="mr-2"
        ...
        <CCardBody>
          <b-table
            :fields="fields"
            :items="items"
            style="white-space: nowrap;height:450px"
            small="small"
            responsive="sm"
          ...
          >
            ...
            <template v-slot:head(blockuntildate)>
              <div
                style="cursor : pointer"
                @click="ChangeSort('blockuntildate')"
              >
                {{ $t("blockuntildate") }}
                <b-icon v-if="filter.SortColumn === 'blockuntildate'" :icon="filter.SortIcon"></b-icon>
              </div>
            </template>
            <template v-slot:head(organization)>
              <div style="cursor : pointer" @click="ChangeSort('organization')">
                {{ $t("organization") }}
                <b-icon v-if="filter.SortColumn === 'organization'" :icon="filter.SortIcon"></b-icon>
              </div>
            </template>
              ...
              <div class="text-center">
                ...
                <b-dropdown variant="link" no-caret class="ml-n2 px-0">
                  ...
                  <div>
                    ...
                    <b-dropdown-item
                      v-if="$can('BlockDocumentActionDelete', 'permissions')"
...
export default {
  ...
  methods: {
    ...
    Delete(item) {
      BlockDocumentActionService.Delete(item.id)
        .then((res) => {
          this.makeToast(
    ...
    Approve(item) {
      var self = this;
      BlockDocumentActionService.Approve(item.id)
        .then((res) => {
          self.makeToast(
    ...
    CancelApproval(item) {
      var self = this;
      BlockDocumentActionService.CancelApproval(item.id)
        .then((res) => {
          self.makeToast(
    ...
    Refresh() {
      this.isBusy = true;
      BlockDocumentActionService.GetList(
        this.filter.Search,
