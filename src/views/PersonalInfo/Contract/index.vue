  ...
  <CRow>
    <CCol sm="12" md="12">
      <CCard accent-color="primary">
        <h3 class="text-center mt-3">{{ $t("Contract") }}</h3>
        <CCardHeader>
          <CRow class="form-group">
            ...
            <CCol lg="4" md="6" sm="12" class="text-lg-right mt-4" style="margin-right: -7px;">
              <CButton
                v-if="$can('ContractEdit', 'permissions')"
                @click="Create"
                color="primary"
                variant="outline"
                size="sm"
              ...
              <div class="text-center">
                ...
                <b-dropdown variant="link" no-caret class="ml-n2 px-0">
                  ...
                  <div>
                    ...
                    <b-dropdown-item
                      v-if="$can('ContractDelete', 'permissions')"
...
export default {
  ...
  methods: {
    createDate() {
      ...
      var startmm = String(todaydate.getMonth() - 2).padStart(2, "0");
      var yyyy = todaydate.getFullYear();
      this.filter.StartDate = dd + "." + startmm + "." + yyyy;
    ...
    Delete(item) {
      ContractService.Delete(item.id)
        .then((res) => {
          this.makeToast(
    ...
    Approve(item) {
      var self = this;
      ContractService.Approve(item.id)
        .then((res) => {
          self.makeToast(
    ...
    CancelApproval(item) {
      var self = this;
      ContractService.CancelApproval(item.id)
        .then((res) => {
          self.makeToast(
