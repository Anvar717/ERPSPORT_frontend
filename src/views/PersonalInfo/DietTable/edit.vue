    ...
    <CCol>
      <CCard accent-color="primary" class="pb-4">
        <CCardHeader>
          <CRow>
            <CCol>
              <h4 class="text-center">{{$t('DietTable')}}</h4>
            </CCol>
          </CRow>
        </CCardHeader>
        <CCardBody>
              ...
              <CCol sm="6">
                <ValidationProvider v-slot="v" name="code" rules="required">
                  <CInput
                    ...
                    horizontal
                    :label="$t('shortname')"
                    v-model="DietTable.shortname"
                    :addInputClasses="{'is-invalid': v.classes.invalid,'is-valid':  v.classes.valid }"
                  >
                    <template #append>
                      <CButton
                        v-if="DietTable.TranslatableColumns.includes('shortname')"
              ...
              <CCol sm="6">
                <ValidationProvider v-slot="v" name="code" rules="required">
                  <CInput
                    ...
                    horizontal
                    :label="$t('fullname')"
                    v-model="DietTable.fullname"
                    :addInputClasses="{'is-invalid': v.classes.invalid,'is-valid':  v.classes.valid }"
                  >
                    <template #append>
                      <CButton
                        v-if="DietTable.TranslatableColumns.includes('fullname')"
          ...
          <CCol>
            <div class="text-left">
              <CButton
                ...
                size="sm"
                color="danger"
                @click="$router.push({name : 'DietTable'})"
              >
...
export default {
  ...
  created() {
    ...
    DietTableService.Get(self.selectedrowid).then(res => {
      self.DietTable = res.data;
    });
  },
  methods: {
    check() {
      ...
      if (
        self.DietTable.code === 0 ||
        self.DietTable.code === null ||
        self.DietTable.code === undefined ||
        self.DietTable.code === ""
      ) {
      ...
      if (
        self.DietTable.shortname === 0 ||
        self.DietTable.shortname === null ||
        self.DietTable.shortname === undefined ||
        self.DietTable.shortname === ""
      ) {
        this.makeToast(
      ...
      if (
        self.DietTable.fullname === 0 ||
        self.DietTable.fullname === null ||
        self.DietTable.fullname === undefined ||
        self.DietTable.fullname === ""
      ) {
        this.makeToast(
      ...
      if (
        self.DietTable.stateid === 0 ||
        self.DietTable.stateid === null ||
        self.DietTable.stateid === undefined ||
        self.DietTable.stateid === ""
      ) {
    ...
    SaveData() {
      ...
      var self = this;
      DietTableService.Update(self.DietTable)
        .then(res => {
          self.makeToast(
          ...
          self.$router.push({ name: "DietTable" });
        })
        .catch(error => {
          self.makeToast(
    ...
    translate(selectcolumnname) {
      ...
      self.translatelist = self.DietTable.Translates.filter(
    ...
    SaveTranslate() {
      var self = this;
      self.DietTable.Translates = self.DietTable.Translates.filter(
        x => x.columnname !== self.translatebyname
      );
      self.translatelist.forEach(item => {
        self.DietTable.Translates.push(item);
      });
