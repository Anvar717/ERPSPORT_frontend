    ...
    <CCol>
      <CCard accent-color="primary" class="pb-4">
        <CCardHeader>
          <CRow>
            <CCol>
              <h4 class="text-center">{{$t('DishGroup')}}</h4>
            </CCol>
          </CRow>
        </CCardHeader>
        <CCardBody>
              ...
              <CCol sm="6">
                <ValidationProvider v-slot="v" name="code" rules="required">
                  <CInput
                    ...
                    horizontal
                    :label="$t('shortname')"
                    v-model="DishGroup.shortname"
                    :addInputClasses="{'is-invalid': v.classes.invalid,'is-valid':  v.classes.valid }"
                  >
                    <template #append>
                      <CButton
                        v-if="DishGroup.TranslatableColumns.includes('shortname')"
              ...
              <CCol sm="6">
                <ValidationProvider v-slot="v" name="code" rules="required">
                  <CInput
                    ...
                    horizontal
                    :label="$t('fullname')"
                    v-model="DishGroup.fullname"
                    :addInputClasses="{'is-invalid': v.classes.invalid,'is-valid':  v.classes.valid }"
                  >
                    <template #append>
                      <CButton
                        v-if="DishGroup.TranslatableColumns.includes('fullname')"
          ...
          <CCol>
            <div class="text-left">
              <CButton
                ...
                size="sm"
                color="danger"
                @click="$router.push({name : 'DishGroup'})"
              >
...
export default {
  ...
  created() {
    ...
    DishGroupService.GetAll().then(res => {
      this.DishGrouplist = res.data;
    });
    HelperService.GetStateList().then(res => {
      self.statelist = res.data;
    });
    DishGroupService.Get(self.selectedrowid).then(res => {
      self.DishGroup = res.data;
    });
  },
  methods: {
    check() {
      ...
      if (
        self.DishGroup.code === 0 ||
        self.DishGroup.code === null ||
        self.DishGroup.code === undefined ||
        self.DishGroup.code === ""
      ) {
      ...
      if (
        self.DishGroup.shortname === 0 ||
        self.DishGroup.shortname === null ||
        self.DishGroup.shortname === undefined ||
        self.DishGroup.shortname === ""
      ) {
        this.makeToast(
      ...
      if (
        self.DishGroup.fullname === 0 ||
        self.DishGroup.fullname === null ||
        self.DishGroup.fullname === undefined ||
        self.DishGroup.fullname === ""
      ) {
        this.makeToast(
      ...
      if (
        self.DishGroup.stateid === 0 ||
        self.DishGroup.stateid === null ||
        self.DishGroup.stateid === undefined ||
        self.DishGroup.stateid === ""
      ) {
    ...
    SaveData() {
      ...
      var self = this;
      DishGroupService.Update(self.DishGroup)
        .then(res => {
          self.makeToast(
          ...
          self.$router.push({ name: "DishGroup" });
        })
        .catch(error => {
          self.makeToast(
    ...
    translate(selectcolumnname) {
      ...
      self.translatelist = self.DishGroup.Translates.filter(
    ...
    SaveTranslate() {
      var self = this;
      self.DishGroup.Translates = self.DishGroup.Translates.filter(
        x => x.columnname !== self.translatebyname
      );
      self.translatelist.forEach(item => {
        self.DishGroup.Translates.push(item);
      });
