  ...
  <CRow>
    <CCol sm="12" md="12">
      <CCard accent-color="primary">
        <h3 class="text-center mt-3">{{ $t("IHIncome") }}</h3>
        <CCardHeader>
          <CRow class="form-group">
            ...
            <CCol lg="7" md="6" sm="12" class="text-lg-right mt-2">
              <CButton
                v-if="$can('IHIncomeEdit', 'permissions')"
                @click="Create"
                color="primary"
                variant="outline"
                class="mr-2"
        ...
        <CCardBody>
          ...
          >
            ...
            <template v-slot:head(sellername)>
              <div style="cursor : pointer" @click="ChangeSort('sellername')">
                {{ $t("sellername") }}
                <b-icon v-if="filter.SortColumn === 'sellername'" :icon="filter.SortIcon"></b-icon>
              </div>
            </template>
              ...
              <div class="text-center">
                ...
                <b-dropdown variant="link" no-caret class="ml-n2 px-0">
                  ...
                  <div>
                    ...
                    <b-dropdown-item
                      v-if="$can('IHIncomeDelete', 'permissions')"
...
export default {
  ...
  methods: {
    ...
    Delete(item) {
      IHIncomeService.Delete(item.id)
        .then((res) => {
          this.makeToast(
    ...
    Approve(item) {
      var self = this;
      IHIncomeService.Approve(item.id)
        .then((res) => {
          self.makeToast(
    ...
    CancelApproval(item) {
      var self = this;
      IHIncomeService.CancelApproval(item.id)
        .then((res) => {
          self.makeToast(
