    ...
    <CCol>
      <CCard accent-color="primary" class="pb-4">
        <CCardHeader>
          <CRow>
            <CCol>
              <h4 class="text-center">{{$t('MenuType')}}</h4>
            </CCol>
          </CRow>
        </CCardHeader>
        <CCardBody>
              ...
              <CCol sm="6">
                <ValidationProvider v-slot="v" name="code" rules="required">
                  <CInput
                    ...
                    horizontal
                    :label="$t('shortname')"
                    v-model="MenuType.shortname"
                    :addInputClasses="{'is-invalid': v.classes.invalid,'is-valid':  v.classes.valid }"
                  >
                    <template #append>
                      <CButton
                        v-if="MenuType.TranslatableColumns.includes('shortname')"
              ...
              <CCol sm="6">
                <ValidationProvider v-slot="v" name="code" rules="required">
                  <CInput
                    ...
                    horizontal
                    :label="$t('fullname')"
                    v-model="MenuType.fullname"
                    :addInputClasses="{'is-invalid': v.classes.invalid,'is-valid':  v.classes.valid }"
                  >
                    <template #append>
                      <CButton
                        v-if="MenuType.TranslatableColumns.includes('fullname')"
          ...
          <CCol>
            <div class="text-left">
              <CButton
                ...
                size="sm"
                color="danger"
                @click="$router.push({name : 'MenuType'})"
              >
...
export default {
  ...
  created() {
    ...
    MenuTypeService.Get(self.selectedrowid).then(res => {
      self.MenuType = res.data;
    });
  },
  methods: {
    check() {
      ...
      if (
        self.MenuType.code === 0 ||
        self.MenuType.code === null ||
        self.MenuType.code === undefined ||
        self.MenuType.code === ""
      ) {
      ...
      if (
        self.MenuType.shortname === 0 ||
        self.MenuType.shortname === null ||
        self.MenuType.shortname === undefined ||
        self.MenuType.shortname === ""
      ) {
        this.makeToast(
      ...
      if (
        self.MenuType.fullname === 0 ||
        self.MenuType.fullname === null ||
        self.MenuType.fullname === undefined ||
        self.MenuType.fullname === ""
      ) {
        this.makeToast(
      ...
      if (
        self.MenuType.stateid === 0 ||
        self.MenuType.stateid === null ||
        self.MenuType.stateid === undefined ||
        self.MenuType.stateid === ""
      ) {
    ...
    SaveData() {
      ...
      var self = this;
      MenuTypeService.Update(self.MenuType)
        .then(res => {
          self.makeToast(
          ...
          self.$router.push({ name: "MenuType" });
        })
        .catch(error => {
          self.makeToast(
    ...
    translate(selectcolumnname) {
      ...
      self.translatelist = self.MenuType.Translates.filter(
    ...
    SaveTranslate() {
      var self = this;
      self.MenuType.Translates = self.MenuType.Translates.filter(
        x => x.columnname !== self.translatebyname
      );
      self.translatelist.forEach(item => {
        self.MenuType.Translates.push(item);
      });
