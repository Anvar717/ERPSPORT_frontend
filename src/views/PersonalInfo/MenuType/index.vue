  ...
  <CRow>
    <CCol sm="12" md="12">
      <CCard accent-color="primary">
        <h3 class="text-center mt-3">{{ $t("MenuType") }}</h3>
        <CCardHeader>
          <CRow class="form-group">
            ...
            <CCol lg="7" md="6" sm="12" class="text-lg-right mt-2">
              <CButton v-if="$can('MenuTypeEdit', 'permissions')" @click="Create" color="primary" variant="outline"
                class="mr-2" size="sm">
          ...
          <b-table :fields="fields" :items="items" style="white-space: nowrap" small="small" responsive="sm"
            ...
            <template v-slot:cell(actions)="{ item }">
              <div class="text-center">
                <b-link v-if="$can('MenuTypeEdit', 'permissions')" @click="Edit(item)" class="mr-2"
                  v-c-tooltip="{ content: $t('Edit') }">
                  <b-icon icon="pencil"></b-icon>
                </b-link>
                <b-link v-if="$can('MenuTypeDelete', 'permissions')" @click="$bvModal.show('DeleteModal' + item.id)"
                  class="mr-2" v-c-tooltip="{ content: $t('Delete') }">
...
export default {
  ...
  methods: {
    ...
    Delete(item) {
      MenuTypeService.Delete(item.id)
        .then((res) => {
          this.makeToast(
