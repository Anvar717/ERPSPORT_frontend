        ...
        <CRow>
          <CCol class="text-right mb-4 mr-3">
            <b-tabs>
              ...
              <b-tab>
                <template slot="title">{{ $t("Manual") }}</template>
                <CRow class="my-2">
                  <CCol>
                    <div class="mobileStyle mx-3">
                      <b-table
              ...
              <b-tab>
                <template slot="title">{{ $t("Automatic") }}</template>
                <CRow class="my-2">
                  <CCol>
                    <div class="mobileStyle mx-3">
                      <b-table-simple bordered>
                        ...
                        <b-tbody>
                          <b-tr
                            ...
                            :key="index"
                            v-if="
                              item.Status != 3 && item.isautomatonformed == true
                            "
                          >
                            <b-td
                              v-if="
          ...
          <CCol>
            <div class="text-left">
              <CButton
                ...
                size="sm"
                color="danger"
                @click="$router.push({ name: 'SportOrgRating' })"
              >
        ...
        <b-modal id="AddModal" size="xl" no-close-on-backdrop>
          ...
          <CCard sm="6">
            <CCardBody sm="6">
              <CRow>
                <CCol>
                  <span style="color: blue; font-size: 20px">{{ sportorgratingindicatorname }}</span>
                </CCol>
              </CRow>
              <CRow>
                <CCol sm="5">
  ...
  created() {
    ...
    SportOrgRatingService.Get(self.selectedrowid).then(res => {
     ...
     HelperService.GetAllRatingPeriod().then(res => {
      this.ratingperiodlist = res.data;
    });
  },
  methods: {
    ...
    calculateAllCounts(key) {
      if (
        Array.isArray(this.SportOrgRating.sportOrgRatingTable) &&
        this.SportOrgRating.sportOrgRatingTable.length > 0
      ) {
        let total = 0;
    ...
    UpdateTable() {
      ...
      if (this.selected.length > 0) {
        this.tabrow.sportorgratingcriteriascoretableid = this.selected[0].sportorgratingcriteriascoretableid;
        this.tabrow.sportschoolrealindicator = this.tabrowParent.sportschoolrealindicator;
