  ...
  <CRow>
    <CCol sm="12" md="12">
      <CCard accent-color="primary">
        <h3 class="text-center mt-3">{{ $t("SportOrgRatingCriteria") }}</h3>
        <CCardHeader>
          <CRow class="form-group justify-content-between">
            ...
            >
              <CButton
                v-if="$can('SportOrgRatingCriteriaEdit', 'permissions')"
                @click="Create"
                color="primary"
                variant="outline"
                class="mr-2"
          ...
          <b-col>
            <b-button-group @click="Refresh" size="sm">
              ...
              <b-button
                ...
                :variant="filter.statusid == 4 ? 'primary' : 'outline-primary'"
              >
                {{ $t("edited") }}
              </b-button>
              <b-button
              ...
              <b-button
                ...
                :variant="filter.statusid == 3 ? 'primary' : 'outline-primary'"
              >
                {{ $t("canceled") }}
              </b-button>
            </b-button-group>
          ...
          >
            ...
            <template v-slot:head(organizationtype)>
              <div
                ...
                @click="ChangeSort('organizationtype')"
              >
                {{ $t("organizationtype") }}
                <b-icon
                  v-if="filter.SortColumn === 'organizationtype'"
                  :icon="filter.SortIcon"
                ></b-icon>
              </div>
            </template>
            ...
            <template v-slot:cell(actions)="{ item }">
              <div class="text-center">
                ...
                >
                  <b-icon v-if="item.statusid!=2" icon="pencil"></b-icon>
                </b-link>
                <b-link
                  v-if="$can('SportOrgRatingCriteriaView', 'permissions')"
                  @click="View(item)"
                  class="mr-2"
                ...
                <b-dropdown variant="link" no-caret class="ml-n2 px-0">
                  ...
                  <div>
                    <b-dropdown-item
                      v-if="$can('SportOrgRatingCriteriaApprove', 'permissions')"
                    ...
                    <b-dropdown-item
                      v-if="$can('SportOrgRatingCriteriaCancelApproval', 'permissions')"
                    ...
                    <b-dropdown-item
                      v-if="$can('SportOrgRatingCriteriaDelete', 'permissions')"
...
export default {
  ...
  computed: {
    firstNumber() {
      return (this.filter.PageNumber - 1) * this.filter.PageNumber + 1;
    },
    lastNumber() {
      if (this.totalRows < this.filter.PageNumber) {
        return this.totalRows;
      } else {
        if (this.filter.PageNumber * this.filter.PageNumber > this.totalRows) {
          return this.totalRows;
        } else {
          return this.filter.PageNumber * this.filter.PageNumber;
        }
      }
    },
  },
  methods: {
    ...
    Delete(item) {
      SportOrgRatingCriteriaService.Delete(item.id)
        .then((res) => {
          this.makeToast(
    ...
    Approve(item) {
      var self = this;
      SportOrgRatingCriteriaService.Approve(item.id)
        .then((res) => {
          self.makeToast(
    ...
    CancelApproval(item) {
      var self = this;
      SportOrgRatingCriteriaService.CancelApproval(item.id)
        .then((res) => {
          self.makeToast(
    ...
    Refresh() {
      this.isBusy = true;
      SportOrgRatingCriteriaService.GetList(
        this.filter.Search,
