  ...
  <CRow>
    <CCol sm="12" md="12">
      <CCard accent-color="primary">
        <h3 class="text-center mt-3">{{ $t("SportOrgRatingIndicator") }}</h3>
        <CCardHeader>
        ...
        <CCardBody>
          ...
          >
                <template v-slot:cell(isgroup)="{ item }">
                    <div v-if="lang == 'ru'">
                      <b-badge :variant="getItemColor(item.isgroup)">{{
                        item.isgroup ? "Да" : "Нет"
                      }}</b-badge>
                    </div>
            ...
            <template v-slot:cell(actions)="{ item }">
              <div class="text-center">
                <b-link
                  v-if="$can('SportOrgRatingIndicatorEdit', 'permissions')"
                  @click="Edit(item)"
                  class="mr-2"
              ...
              <b-modal :id="'DeleteModal' + item.id" hide-footer>
                ...
                <div class="d-block text-right">
                  ...
                  <CButton
                   v-if="$can('SportOrgRatingIndicatorDelete', 'permissions')"
                    @click="Delete(item)"
                    color="success"
                  >
...
export default {
  ...
  methods: {
    ...
    Delete(item) {
      SportOrgRatingIndicatorService.Delete(item.id)
        .then(res => {
          this.makeToast(
    ...
    Refresh() {
      this.isBusy = true;
      SportOrgRatingIndicatorService.GetList(
        this.filter.Search,
        this.filter.SortColumn,
        this.filter.OrderType,
        this.filter.PageNumber,
