      ...
      <CCard accent-color="primary">
        <CRow class="card-head mt-3 mx-1 d-flex">
          ...
          <CCol class="text-center" lg="6">
            <h3>{{ $t("GetReportAppointQualCategoryCoachList") }}</h3>
          </CCol>
          <b-col lg="3">
...
export default {
  ...
  created() {
    ...
    (this.filter.oblastid =
      ...
      (this.filter.qualificationcategoryid =
        parseInt(this.$route.query.qualificationcategoryid) || this.filter.qualificationcategoryid),
  ...
  methods: {
    ...
    Search() {
      ...
      SportReportService.PrintReportAppointQualCategoryCoachList(
        this.filter.oblastid,
        this.filter.regionid,
        this.filter.organizationid,
        ...
        .then((response) => {
          ...
          if (this.lang == "ru") {
            fileLink.setAttribute(
              "download",
              "Реестр тренеров с квалификационной категорией.xlsx"
            );
    ...
    Refresh() {
      this.isBusy = true;
      SportReportService.GetReportAppointQualCategoryCoachList(this.filter).then((res) => {
        this.isBusy = false;
        this.items = res.data.data.rows;
        this.totalRows = res.data.total;
        if (res.data.sportqualspecialtyid == 1) {
