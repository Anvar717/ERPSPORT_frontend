<template>
  <div>
    ...
    <b-modal ref="my-modal" :title="$t('SportCompetetionAddJury')" v-model="createModal" hide-footer size="xl"
      no-close-on-backdrop>
      <JuryCreate v-if="createModal" :uid="uid" @save="Add" @changePosition="changePosition"
...
import JuryCreate from "./_juryCreate.vue";

export default {
  components: {
    CustomNumberInput, JuryCreate
  },
  data() {
    return {
      step1: {},
      ...
      fields: [
        ...
        { key: 'positionname', label: this.$t('position') },
        { key: 'fullname', label: this.$t('fullname') },
        { key: 'qualificationcategoryname', label: this.$t('qualificationcategory') },
  ...
  computed: {
    isApproved() {
      ...
      if (this.SportCompetition && this.SportCompetition.Steps) {
        isapproved = this.SportCompetition.Steps[9].isapproved
      }
      return isapproved
    },
    isFilled() {
      let isfilled = false
      if (this.SportCompetition && this.SportCompetition.Steps) {
        isfilled = this.SportCompetition.Steps[9].isfilled
      }
      return isfilled
    },
    isNewAdded() {
  ...
  methods: {
    GetJuryList() {
      SportCompetitionService.GetJuryList(this.uid).then(res => {
    ...
    Save() {
      ...
      SportCompetitionService.UpdateJury(this.step1list).then(res => {
    ...
    Accept() {
      SportCompetitionService.ApproveJury(this.uid).then(res => {
        this.$emit('next')
        this.$emit('GetAction', true)
    ...
    Add(data) {
      ...
      var self = this
      if (self.step1list.filter(item => item.documentnumber == saveData.documentnumber && item.documentseries == saveData.documentseries && item.positionid == saveData.positionid).length > 0) {
        self.makeToast(self.$t('AlreadyPositionSelected'), self.$t("Error"), 'danger')
        return false
      }
      else {
    ...
    changeQualification(item) {
      this.tabrow.qualificationcategoryname = item.qualificationcategoryname
      this.tabrow.qualificationcategoryid = item.qualificationcategoryid
    },
    Edit(item) {
      if (item.statusid == 0) {
        item.statusid = 2;
    ...
    Delete(item) {
      ...
      if (item.id) {
        SportCompetitionService.DeleteJury({ sportCompetitionUId: this.uid, id: item.id }).then(res => {
    ...
    GetIndicator() {
      this.loading = true
      SportCompetitionService.GetJury(0, this.uid).then((res) => {
