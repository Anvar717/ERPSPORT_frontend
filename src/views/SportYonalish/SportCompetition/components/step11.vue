<template>
  <div>
    ...
    <b-modal ref="my-modal" :title="$t('SportCompetetionStaffComposition')" v-model="createModal" hide-footer size="xl"
...
export default {
  ...
  methods: {
    GetJuryList() {
      SportCompetitionService.GetStaffCompositionList(this.uid).then(res => {
    ...
    Save() {
      ...
      SportCompetitionService.UpdateStaffComposition(this.step1list).then(res => {
    ...
    Accept() {
      SportCompetitionService.ApproveStaffComposition(this.uid).then(res => {
        this.$emit('next')
        this.$emit('GetAction', true)
    ...
    Delete(item) {
      ...
      if (item.id) {
        SportCompetitionService.DeleteStaffComposition({ sportCompetitionUId: this.uid, id: item.id }).then(res => {
    ...
    GetIndicator() {
      this.loading = true
      SportCompetitionService.GetStaffComposition(0, this.uid).then((res) => {
