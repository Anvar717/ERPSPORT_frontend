<template>
  <div>
    ...
    <b-row>
      <b-col>
        <b-table :fields="fields" :items="step1list" small responsive="sm" bordered head-row-variant="info"
          ...
          <template #cell(sporttypeclassifierdisciplinename)="{ item }">
            <span v-html="item.sporttypeclassifierdisciplinename.replaceAll(',', '<br>')"></span>
          </template>
    ...
    <b-modal ref="my-modal" :title="$t('SportCompetetionAddAthlete')" v-model="createModal" hide-footer size="xl"
      no-close-on-backdrop>
      <AthleteCreate v-if="createModal" :uid="uid" @save="Add" @changePosition="changePosition"
        @changeQualification="changeQualification" :tabrow="tabrow" :SportCompetition="SportCompetition"
        typeForm="AthleteCreate" />
    </b-modal>
    <b-row>
...
import AthleteCreate from "./_juryCreate.vue";

export default {
  components: {
    CustomNumberInput, AthleteCreate
  },
  data() {
    return {
      step1: {},
      ...
      fields: [
        ...
        { key: 'sporttitlename', label: this.$t('sporttitle') },
  ...
  computed: {
    thisStep() {
      return this.SportCompetition.Steps.find(el => el.ordernumber == 13)
    },
    isApproved() {
  ...
  methods: {
    GetAthleteList() {
      SportCompetitionService.GetAthleteList(this.uid).then(res => {
    ...
    Save() {
      ...
      SportCompetitionService.UpdateAthlete(this.step1list).then(res => {
    ...
    Accept() {
      SportCompetitionService.ApproveAthlete(this.uid).then(res => {
        this.$emit('next')
        this.$emit('GetAction', true)
    ...
    Add(data) {
      ...
      console.log(data)
      saveData.sporttypeclassifierdisciplinename = data.athleteTable.map(el => el.sporttypeclassifierdisciplinename).join(', ')
      ...
      saveData.mobilenumber = data.contactinfo
      if (data.identitydocumentid && data.identitydocumentid == 4) {
        saveData.islocalathlete = false
      } else {
        saveData.islocalathlete = true
      }
      var self = this
    ...
    Delete(item) {
      ...
      if (item.id) {
        SportCompetitionService.DeleteAthlete({ sportCompetitionUId: this.uid, id: item.id }).then(res => {
    ...
    GetIndicator() {
      this.loading = true
      SportCompetitionService.GetAthlete(0, this.uid).then((res) => {
