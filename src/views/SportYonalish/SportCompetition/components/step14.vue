<template>
    <div>
        ...
        <b-row>
            <b-col>
                <b-table :fields="fields" :items="step1list" small responsive="sm" bordered head-row-variant="info"
                    :tbody-tr-class="rowClass" show-empty class="text-center">
                    <template #cell(index)="{ item, index }">
                        {{index + 1}}
                    </template>
                    <template #cell(actions)="{ item }">
                        <div class="text-center">
                            <b-link @click="Delete(item)" v-if="item.statusid != 2" class="mr-2"
        ...
        <b-modal ref="my-modal" :title="$t('SportCompetetionAddResult')" v-model="createModal" hide-footer size="xl"
            no-close-on-backdrop>
            <ResultCreate v-if="createModal" :uid="uid" @save="Add" @changePosition="changePosition"
                @changeQualification="changeQualification" :tabrow="tabrow" :SportCompetition="SportCompetition"
                typeForm="ResultCreate" />
        </b-modal>
        <b-row>
            <b-col class="text-right">
                ...
                <b-button variant="success" v-else-if="!isApproved && isFilled" class="mr-2" @click="Accept"> {{
                    $t('Accept') }} </b-button>
...
import ResultCreate from "./_juryCreate.vue";

export default {
    components: {
        CustomNumberInput, ResultCreate
    },
    data() {
        return {
            step1: {},
            ...
            editedIndex: -1,
            fields: [
                { key: 'index', label: this.$t('№'),thClass : 'text-center' },
                { key: 'fullname', label: this.$t('personname1'),thClass : 'text-center' },
                { key: 'dateofbirth', label: this.$t('dateofbirth'),thClass : 'text-center' },
                { key: 'pinfl', label: this.$t('pinfl'),thClass : 'text-center' },
                { key: 'gendername', label: this.$t('gender'),thClass : 'text-center' },
                { key: 'sporttypeclassifierdisciplinename', label: this.$t('sporttypeclassifierdiscipline'),thClass : 'text-center' },
                { key: 'athleteindicator', label: this.$t('athleteindicator'),thClass : 'text-center' },
                { key: 'unitofmeasurename', label: this.$t('unitofmeasure'), thClass: 'text-center' },
                { key: 'sportrankname', label: this.$t('SportRank'),thClass : 'text-center' },
                { key: !this.isView ? 'actions' : '', label: this.$t('actions'),thClass : 'text-center' },
            ],
        };
    },
    props: {
    ...
    computed: {
        thisStep() {
            return this.SportCompetition.Steps.find(el => el.ordernumber == 14)
        },
        isApproved() {
    ...
    methods: {
        GetResultList() {
            SportCompetitionService.GetResultList(this.uid).then(res => {
        ...
        Save() {
            ...
            SportCompetitionService.UpdateResult(this.step1list).then(res => {
        ...
        Accept() {
            SportCompetitionService.ApproveResult(this.uid).then(res => {
                this.$emit('next')
                this.$emit('GetAction', true)
        ...
        Add(data) {
            ...
            Object.keys(saveData).forEach(key => {
                if (key == 'base64photo' || key == 'CheckResult' || key == 'DocumentTables' || key == 'PersonPhoto' || key == 'LivePlaceTables') {
                    delete saveData[key]
                }
            })
        ...
        Delete(item) {
            ...
            SportCompetitionService.DeleteResult({ sportCompetitionUId: this.uid, id: item.id }).then(res => {
        ...
        GetIndicator() {
            this.loading = true
            SportCompetitionService.GetResult(0, this.uid).then((res) => {
