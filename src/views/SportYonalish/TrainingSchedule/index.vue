  ...
  <CRow>
    <CCol sm="12" md="12">
      <CCard accent-color="primary">
        <h3 class="text-center mt-3">{{ $t("TrainingSchedule") }}</h3>
        <CCardHeader>
        ...
        <CCardBody>
          ...
          >
            ...
            <template v-slot:head(fromorgschoolgrade)>
              <div style="cursor: pointer" @click="ChangeSort('fromorgschoolgrade')">
            ...
            <template v-slot:head(toorgschoolgrade)>
              <div style="cursor: pointer" @click="ChangeSort('toorgschoolgrade')">
                {{ $t("toorgschoolgrade") }}
            ...
            <template v-slot:cell(actions)="{ item }">
              <div class="text-center">
                ...
                <b-link
                  v-if="$can('TrainingScheduleEdit', 'permissions') && item.statusid != 2"
                  @click="Edit(item)"
                  class="mr-2"
                ...
                <b-link
                  v-if="$can('TrainingScheduleDelete', 'permissions') && item.statusid != 2"
                ...
                <b-link
                  v-if="$can('TrainingScheduleApprove', 'permissions') && item.statusid != 2"
                ...
                <b-link
                  v-if="$can('TrainingScheduleCancelApproval', 'permissions') && item.statusid != 3 &&
                                            item.statusid == 2 
                                    "
                ...
                >
                  <b-icon icon="files" scale="1.2"></b-icon>
                </b-link>
                <!-- <b-dropdown variant="link" no-caret class="ml-n2 px-0">
                                    ...
                                    <div>
                                        <b-dropdown-item v-if="$can('AppealApprove', 'permissions') &&
                                            item.statusid != 2
                                            " @click="$bvModal.show('AcceptModal' + item.id)">
                                            <b-icon class="mr-2" icon="check2"></b-icon>
                                            &nbsp; {{ $t("Acceptt") }}
                                        </b-dropdown-item>
                                        <b-dropdown-item v-if="$can('AppealCancelApproval', 'permissions') &&
                                            item.statusid != 3 &&
                                            item.statusid != 1
                                            " @click="$bvModal.show('CancelModal' + item.id)">
                                        ...
                                        <b-dropdown-item v-if="item.statusid != 1 && item.statusid != 8"
  ...
  methods: {
    ...
    Search() {
      if (this.filter.searchtype === 1) {
        this.filter.sportsmename = this.filter.fakeSearch;
        // this.filter.Search = ""
      }
      if (this.filter.searchtype === 2) {
        this.filter.coachname = this.filter.fakeSearch;
        // this.filter.Search = ""
      }
    ...
    Approve(item) {
      var self = this;
      TrainingScheduleService.Approve(item.id)
        .then(res => {
          self.makeToast(
    ...
    Send(item) {
      var self = this;
      TrainingScheduleService.Sent(item.id)
        .then(res => {
    ...
    CancelApproval(item) {
      var self = this;
      TrainingScheduleService.CancelApproval(item.id)
        .then(res => {
          self.makeToast(
    ...
    Delete(item) {
      TrainingScheduleService.Delete(item.id)
        .then(res => {
          this.makeToast(
    ...
    Print() {
      TrainingScheduleService.PrintAppeal(
        this.filter.Search,
        ...
        .then(res => {
          this.forceFileDownload(res, "PrintAppeal");
        })
        .catch(error => {
          this.makeToast(
            error.response.data.error,
    ...
    Refresh() {
      this.isBusy = true;
      TrainingScheduleService.GetList(
