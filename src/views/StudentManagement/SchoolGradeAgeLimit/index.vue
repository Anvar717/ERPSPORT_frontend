  ...
  <CRow>
    <CCol sm="12" md="12">
      <CCard accent-color="primary">
        <h3 class="text-center mt-3">{{ $t("SchoolGradeAgeLimit") }}</h3>
        <CCardHeader>
          ...
          <b-table :fields="fields" :items="items" style="white-space: nowrap" small="small" responsive="sm"
            ...
            <template v-slot:head(code)>
              <div class="text-center" style="cursor : pointer" @click="ChangeSort('code')">
                {{ $t("fromage") }}
                <b-icon v-if="filter.SortColumn === 'fromage'" :icon="filter.SortIcon"></b-icon>
              </div>
            </template>
            ...
            <template v-slot:head(gradegroup)>
              <div class="text-center" style="cursor : pointer" @click="ChangeSort('gradegroup')">
                {{ $t("gradegroup") }}
                <b-icon v-if="filter.SortColumn === 'gradegroup'" :icon="filter.SortIcon"></b-icon>
              </div>
            </template>
            <template v-slot:head(state)>
              <div class="text-center" style="cursor : pointer" @click="ChangeSort('state')">
                {{ $t("state") }}
                <b-icon v-if="filter.SortColumn === 'state'" :icon="filter.SortIcon"></b-icon>
              </div>
            </template>
            ...
            <template v-slot:cell(actions)="{ item }">
              <div class="text-center">
                <b-link v-if="$can('SchoolGradeAgeLimitEdit', 'permissions')" @click="Edit(item)" class="mr-2"
                  v-c-tooltip="{ content: $t('Edit') }">
                  <b-icon icon="pencil"></b-icon>
                </b-link>
                <b-link v-if="$can('SchoolGradeAgeLimitDelete', 'permissions')"
...
export default {
  data() {
    return {
      fields: [
        ...
        {
          key: "schoolgradename",
          label: this.$t("SchoolGrade"),
          sortable: false,
          thClass: "text-center",
        },
        {
          key: "fromage",
          label: this.$t("fromage"),
          sortable: false,
          thClass: "text-center",
        },
        {
  ...
  methods: {
    ...
    Delete(item) {
      SchoolGradeAgeLimitService.Delete(item.id)
        .then((res) => {
          this.makeToast(
    ...
    Refresh() {
      this.isBusy = true;
      SchoolGradeAgeLimitService.GetList(
        this.filter.Search,
