  ...
  <CRow>
    <CCol sm="12" md="12">
      <CCard accent-color="primary">
        <h3 class="text-center mt-3">{{ $t("StudentHomeSchooling") }}</h3>
        <CCardHeader>
          <CRow class="form-group">
            ...
            <CCol lg="2" md="3" sm="11" class="text-left mt-2">
              <v-select
                v-model="filter.studentschoolyearid"
                :reduce="(item) => item.id"
                label="name"
            ...
            <CCol lg="2" md="3" sm="11" class="text-left mt-2">
              <v-select
                v-model="filter.studentorgschoolgradeid"
            ...
            <CCol lg="4" md="3" sm="11" class="text-left mt-2">
              <v-select
                v-model="filter.curriculumtypeid"
                :options="CurriculumTypeList"
                :reduce="(item) => item.id"
                label="name"
                :placeholder="$t('CurriculumType')"
              >
              </v-select>
            </CCol>
            ...
            <CCol lg="12" md="6" sm="12" class="text-lg-right mt-2">
              <CButton
                @click="Create"
                color="primary"
                variant="outline"
...
export default {
  data() {
    return {
      ...
      isBusy: true,
      filter: {
        studentschoolyearid: 0,
        studentorgschoolgradeid: 0,
        curriculumtypeid: 0,
        Search: "",
        SortColumn: "",
        OrderType: "asc",
  ...
  created() {
    ...
    CurriculumTypeService.GetAll(this.lang).then((res) => {
  ...
  methods: {
    ...
    Delete(item) {
      StudentHomeSchoolingService.Delete(item.id)
        .then((res) => {
          this.makeToast(
    ...
    Approve(item) {
      var self = this;
      StudentHomeSchoolingService.Approve(item.id)
        .then((res) => {
          self.makeToast(
    ...
    CancelApproval(item) {
      var self = this;
      StudentHomeSchoolingService.CancelApproval(item.id)
        .then((res) => {
          self.makeToast(
    ...
    Refresh() {
      this.isBusy = true;
      StudentHomeSchoolingService.GetList(
        this.filter.studentschoolyearid,
        this.filter.studentorgschoolgradeid,
        this.filter.curriculumtypeid,
        this.filter.Search,
