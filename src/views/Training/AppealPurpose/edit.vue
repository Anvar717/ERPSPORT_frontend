      ...
      <CCol>
        <CCard accent-color="primary" class="pb-4">
          <CCardHeader>
            <CRow>
              <CCol>
                <h4 class="text-center">{{$t('AppealPurpose')}}</h4>
              </CCol>
            </CRow>
          </CCardHeader>
          <CCardBody>
                ...
                <CCol sm="6">
                  <ValidationProvider v-slot="v" name="code" rules="required">
                    <CInput
                      ...
                      horizontal
                      :label="$t('shortname')"
                      v-model="AppealPurpose.shortname"
                      :addInputClasses="{'is-invalid': v.classes.invalid,'is-valid':  v.classes.valid }"
                    >
                      <template #append>
                        <CButton
                          v-if="AppealPurpose.TranslatableColumns.includes('shortname')"
                ...
                <CCol sm="6">
                  <ValidationProvider v-slot="v" name="code" rules="required">
                    <CInput
                      ...
                      horizontal
                      :label="$t('fullname')"
                      v-model="AppealPurpose.fullname"
                      :addInputClasses="{'is-invalid': v.classes.invalid,'is-valid':  v.classes.valid }"
                    >
                      <template #append>
                        <CButton
                          v-if="AppealPurpose.TranslatableColumns.includes('fullname')"
            ...
            <CCol>
              <div class="text-left">
                <CButton
                  ...
                  size="sm"
                  color="danger"
                  @click="$router.push({name : 'AppealPurpose'})"
                >
  ...
  export default {
    data() {
      return {
        ...
        AppealPurpose: { TranslatableColumns: [] },
        selectedrowid: 0,
        translitefields: [
    ...
    created() {
      ...
      AppealPurposeService.Get(self.selectedrowid).then(res => {
        self.AppealPurpose = res.data;
      });
    },
    methods: {
      check() {
        ...
        if (
          self.AppealPurpose.code === 0 ||
          self.AppealPurpose.code === null ||
          self.AppealPurpose.code === undefined ||
          self.AppealPurpose.code === ""
        ) {
          this.makeToast(this.$t("codeNotCorrect"), this.$t("Error"), "danger");
          return false;
        }
        if (
          self.AppealPurpose.shortname === 0 ||
          self.AppealPurpose.shortname === null ||
          self.AppealPurpose.shortname === undefined ||
          self.AppealPurpose.shortname === ""
        ) {
          this.makeToast(
        ...
        if (
          self.AppealPurpose.fullname === 0 ||
          self.AppealPurpose.fullname === null ||
          self.AppealPurpose.fullname === undefined ||
          self.AppealPurpose.fullname === ""
        ) {
          this.makeToast(
      ...
      SaveData() {
        ...
        var self = this;
        AppealPurposeService.Update(self.AppealPurpose)
          .then(res => {
            self.makeToast(
            ...
            self.$router.push({ name: "AppealPurpose" });
          })
          .catch(error => {
            self.makeToast(
      ...
      translate(selectcolumnname) {
        ...
        self.translatelist = self.AppealPurpose.Translates.filter(
      ...
      SaveTranslate() {
        var self = this;
        self.AppealPurpose.Translates = self.AppealPurpose.Translates.filter(
          x => x.columnname !== self.translatebyname
        );
        self.translatelist.forEach(item => {
          self.AppealPurpose.Translates.push(item);
        });
