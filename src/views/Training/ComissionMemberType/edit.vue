      ...
      <CCol>
        <CCard accent-color="primary" class="pb-4">
          <CCardHeader>
            <CRow>
              <CCol>
                <h4 class="text-center">{{ $t("ComissionMemberType") }}</h4>
              </CCol>
            </CRow>
          </CCardHeader>
          <CCardBody>
                ...
                <CCol sm="6">
                  ...
                  >
                    <CInput
                      ...
                      horizontal
                      :label="$t('code')"
                      v-model="ComissionMemberType.code"
                      :addInputClasses="{
                ...
                <CCol sm="6">
                  <ValidationProvider v-slot="v" name="code" rules="required">
                    <CInput
                      ...
                      horizontal
                      :label="$t('shortname')"
                      v-model="ComissionMemberType.shortname"
                      :addInputClasses="{
                        'is-invalid': v.classes.invalid,
                        'is-valid': v.classes.valid,
                      }"
                    >
                      <template #append>
                        <CButton
                          v-if="
                            ComissionMemberType.TranslatableColumns.includes(
                              'shortname'
                            )
                          "
                ...
                <CCol sm="6">
                  <ValidationProvider v-slot="v" name="code" rules="required">
                    <CInput
                      ...
                      horizontal
                      :label="$t('fullname')"
                      v-model="ComissionMemberType.fullname"
                      :addInputClasses="{
                        'is-invalid': v.classes.invalid,
                        'is-valid': v.classes.valid,
                      }"
              ...
              <CRow>
                ...
                <CCol sm="6">
                  <div class="form-group form-row">
                    ...
                    <b-form-checkbox
                      v-model="ComissionMemberType.isheader"
                      switch
                      class="mt-2 ml-1"
                    ></b-form-checkbox>
                  </div>
                ...
                <CCol sm="6">
                  <div class="form-group form-row">
                    ...
                    <v-select
                      class="col-sm-9"
                      :options="statelist"
                      v-model="ComissionMemberType.stateid"
                      :reduce="(item) => item.id"
            ...
            <CCol>
              <div class="text-left">
                <CButton
                  ...
                  size="sm"
                  color="danger"
                  @click="$router.push({ name: 'ComissionMemberType' })"
                >
  ...
  export default {
    data() {
      return {
        ...
        ComissionMemberType: { TranslatableColumns: [] },
        selectedrowid: 0,
        translitefields: [
    ...
    created() {
      ...
      ComissionMemberTypeService.Get(self.selectedrowid).then((res) => {
        self.ComissionMemberType = res.data;
      });
    },
    methods: {
      check() {
        ...
        if (
          self.ComissionMemberType.code === 0 ||
          self.ComissionMemberType.code === null ||
          self.ComissionMemberType.code === undefined ||
          self.ComissionMemberType.code === ""
        ) {
          this.makeToast(this.$t("codeNotCorrect"), this.$t("Error"), "danger");
          return false;
        }
        if (
          self.ComissionMemberType.shortname === 0 ||
          self.ComissionMemberType.shortname === null ||
          self.ComissionMemberType.shortname === undefined ||
          self.ComissionMemberType.shortname === ""
        ) {
          this.makeToast(
        ...
        if (
          self.ComissionMemberType.fullname === 0 ||
          self.ComissionMemberType.fullname === null ||
          self.ComissionMemberType.fullname === undefined ||
          self.ComissionMemberType.fullname === ""
        ) {
          this.makeToast(
      ...
      SaveData() {
        ...
        var self = this;
        ComissionMemberTypeService.Update(self.ComissionMemberType)
          .then((res) => {
            self.makeToast(
            ...
            self.$router.push({ name: "ComissionMemberType" });
          })
          .catch((error) => {
            self.makeToast(
      ...
      translate(selectcolumnname) {
        ...
        self.translatelist = self.ComissionMemberType.Translates.filter(
      ...
      SaveTranslate() {
        var self = this;
        self.ComissionMemberType.Translates = self.ComissionMemberType.Translates.filter(
          (x) => x.columnname !== self.translatebyname
        );
        self.translatelist.forEach((item) => {
          self.ComissionMemberType.Translates.push(item);
        });
