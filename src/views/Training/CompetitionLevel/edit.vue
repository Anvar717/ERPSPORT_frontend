<template>
      ...
      <CCol>
        <CCard accent-color="primary" class="pb-4">
          <CCardHeader>
            <CRow>
              <CCol>
                <h4 class="text-center">{{ $t("CompetitionLevel") }}</h4>
              </CCol>
            </CRow>
          </CCardHeader>
          <CCardBody>
            ...
            <CRow>
              ...
              <CCol sm="6">
                <CInput
                  ...
                  horizontal
                  :label="$t('shortname')"
                  v-model="CompetitionLevel.shortname"
                >
                  <template #append>
                    <CButton
                      @click="ShortNameLang"
            ...
            <CRow>
              ...
              <CCol sm="6">
                <CInput
                  ...
                  horizontal
                  :label="$t('fullname')"
                  v-model="CompetitionLevel.fullname"
                >
                  <template #append>
                    <CButton
                      @click="FullNameLang"
              ...
              <CCol sm="6">
                <div class="form-group form-row">
                  ...
                  <v-select
                    class="col-sm-9"
                    :options="statelist"
                    v-model="CompetitionLevel.stateid"
                    :reduce="(item) => item.id"
              ...
              <CCol>
                <div class="text-left">
                  <CButton
                    ...
                    size="sm"
                    color="danger"
                    @click="$router.push({ name: 'CompetitionLevel' })"
                  >
  ...
  export default {
    ...
    created() {
      ...
      CompetitionLevelService.Get(this.$route.params.id).then((res) => {
        this.CompetitionLevel = res.data;
        this.items1 =
    ...
    methods: {
      check() {
        ...
        if (
          self.CompetitionLevel.code === 0 ||
          self.CompetitionLevel.code === null ||
          self.CompetitionLevel.code === undefined ||
          self.CompetitionLevel.code === ""
        ) {
          this.makeToast(this.$t("codeNotCorrect"), this.$t("Error"), "danger");
          return false;
        }
        if (
          self.CompetitionLevel.shortname === 0 ||
          self.CompetitionLevel.shortname === null ||
          self.CompetitionLevel.shortname === undefined ||
          self.CompetitionLevel.shortname === ""
        ) {
          this.makeToast(
        ...
        if (
          self.CompetitionLevel.fullname === 0 ||
          self.CompetitionLevel.fullname === null ||
          self.CompetitionLevel.fullname === undefined ||
          self.CompetitionLevel.fullname === ""
        ) {
          this.makeToast(
        ...
        if (
          self.CompetitionLevel.stateid === 0 ||
          self.CompetitionLevel.stateid === null ||
          self.CompetitionLevel.stateid === undefined ||
          self.CompetitionLevel.stateid === ""
        ) {
      ...
      SaveData() {
        ...
        CompetitionLevelService.Update(this.CompetitionLevel)
          .then((res) => {
            this.makeToast(
            ...
            this.$router.push({ name: "CompetitionLevel" });
          })
          .catch((error) => {
            this.makeToast(
      ...
      AddItem() {
        if (this.TranslatebyName == "shortname") {
          this.ItemsPush();
          this.CompetitionLevelTranslatePush();
      ...
      CompetitionLevelTranslatePush() {
        this.CompetitionLevel.Translates.push({
          languageid: this.Translate.languageid,
          languagename: this.LanguageList.filter(
            (item) => item.id == this.Translate.languageid
          )[0].name,
