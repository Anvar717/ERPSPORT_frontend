    ...
    <CCol>
      <CCard accent-color="primary" class="pb-4">
        <CCardHeader>
          <CRow>
            <CCol>
              <h4 class="text-center">{{ $t("CompetitionType") }}</h4>
            </CCol>
          </CRow>
        </CCardHeader>
        <CCardBody>
          ...
          <CRow>
            ...
            <CCol sm="6">
              <CInput
                ...
                horizontal
                :label="$t('shortname')"
                v-model="CompetitionType.shortname"
              >
                <template #append>
                  <CButton
                    @click="ShortNameLang"
          ...
          <CRow>
            ...
            <CCol sm="6">
              <CInput
                ...
                horizontal
                :label="$t('fullname')"
                v-model="CompetitionType.fullname"
              >
                <template #append>
                  <CButton
                    @click="FullNameLang"
            ...
            <CCol sm="6">
              <div class="form-group form-row">
                ...
                <v-select
                  ...
                  v-model="CompetitionType.sporteventtypeid"
                  :reduce="(item) => item.id"
          ...
          <CRow>
            ...
            <CCol sm="6">
              <div class="form-group form-row">
                ...
                <v-select
                  :options="competitionlevellist"
                  class="col-sm-9"
                  v-model="CompetitionType.competitionlevelid"
                  :reduce="(item) => item.id"
                  :placeholder="$t('ChooseBelow')"
                  label="name"
                ></v-select>
            ...
            <CCol sm="6">
              <div class="form-group form-row">
                ...
                <v-select
                  class="col-sm-9"
                  :options="statelist"
                  v-model="CompetitionType.stateid"
                  :reduce="(item) => item.id"
                  :placeholder="$t('ChooseBelow')"
                  label="name"
                ></v-select>
            ...
            <CCol>
              <div class="text-left">
                <CButton
                  ...
                  size="sm"
                  color="danger"
                  @click="$router.push({ name: 'CompetitionType' })"
                >
...
export default {
  ...
  created() {
    ...
    CompetitionTypeService.Get(this.$route.params.id).then((res) => {
  ...
  methods: {
    check() {
      ...
      if (
        self.CompetitionType.code === 0 ||
        self.CompetitionType.code === null ||
        self.CompetitionType.code === undefined ||
        self.CompetitionType.code === ""
      ) {
        this.makeToast(this.$t("codeNotCorrect"), this.$t("Error"), "danger");
        return false;
      }
      if (
        self.CompetitionType.shortname === 0 ||
        self.CompetitionType.shortname === null ||
        self.CompetitionType.shortname === undefined ||
        self.CompetitionType.shortname === ""
      ) {
        this.makeToast(
      ...
      if (
        self.CompetitionType.fullname === 0 ||
        self.CompetitionType.fullname === null ||
        self.CompetitionType.fullname === undefined ||
        self.CompetitionType.fullname === ""
      ) {
        this.makeToast(
      ...
      if (
        self.CompetitionType.stateid === 0 ||
        self.CompetitionType.stateid === null ||
        self.CompetitionType.stateid === undefined ||
        self.CompetitionType.stateid === ""
      ) {
    ...
    SaveData() {
      ...
      CompetitionTypeService.Update(this.CompetitionType)
        .then((res) => {
          this.makeToast(
          ...
          this.$router.push({ name: "CompetitionType" });
        })
        .catch((error) => {
          this.makeToast(
    ...
    AddItem() {
      if (this.TranslatebyName == "shortname") {
        this.ItemsPush();
        this.CompetitionTypeTranslatePush();
    ...
    CompetitionTypeTranslatePush() {
      this.CompetitionType.Translates.push({
        languageid: this.Translate.languageid,
        languagename: this.LanguageList.filter(
          (item) => item.id == this.Translate.languageid
        )[0].name,
