      ...
      <CCol>
        <CCard accent-color="primary" class="pb-4">
          <CCardHeader>
            <CRow>
              <CCol>
                <h4 class="text-center">{{$t('PhysicalTrainingType')}}</h4>
              </CCol>
            </CRow>
          </CCardHeader>
          <CCardBody>
                ...
                <CCol sm="6">
                  <ValidationProvider v-slot="v" name="code" rules="required|max:9">
                    <CInput
                      ...
                      horizontal
                      :label="$t('code')"
                      v-model="PhysicalTrainingType.code"
                ...
                <CCol sm="6">
                  <ValidationProvider v-slot="v" name="code" rules="required">
                    <CInput
                      ...
                      horizontal
                      :label="$t('shortname')"
                      v-model="PhysicalTrainingType.shortname"
                      :addInputClasses="{'is-invalid': v.classes.invalid,'is-valid':  v.classes.valid }"
                    >
                      <template #append>
                        <CButton
                          v-if="PhysicalTrainingType.TranslatableColumns.includes('shortname')"
                ...
                <CCol sm="6">
                  <ValidationProvider v-slot="v" name="code" rules="required">
                    <CInput
                      ...
                      horizontal
                      :label="$t('fullname')"
                      v-model="PhysicalTrainingType.fullname"
                      :addInputClasses="{'is-invalid': v.classes.invalid,'is-valid':  v.classes.valid }"
                    >
                      <template #append>
                        <CButton
                          v-if="PhysicalTrainingType.TranslatableColumns.includes('fullname')"
                ...
                <CCol sm="6">
                  <div class="form-group form-row">
                    ...
                    <v-select
                      class="col-sm-9"
                      :options="statelist"
                      v-model="PhysicalTrainingType.stateid"
                      :reduce="item => item.id"
            ...
            <CCol>
              <div class="text-left">
                <CButton
                  ...
                  size="sm"
                  color="danger"
                  @click="$router.push({name : 'PhysicalTrainingType'})"
                >
  ...
  export default {
    data() {
      return {
        ...
        PhysicalTrainingType: { TranslatableColumns: [] },
        selectedrowid: 0,
        translitefields: [
    ...
    created() {
      ...
      PhysicalTrainingTypeService.Get(self.selectedrowid).then(res => {
        self.PhysicalTrainingType = res.data;
      });
    },
    methods: {
      check() {
        ...
        if (
          self.PhysicalTrainingType.code === 0 ||
          self.PhysicalTrainingType.code === null ||
          self.PhysicalTrainingType.code === undefined ||
          self.PhysicalTrainingType.code === ""
        ) {
          this.makeToast(this.$t("codeNotCorrect"), this.$t("Error"), "danger");
          return false;
        }
        if (
          self.PhysicalTrainingType.shortname === 0 ||
          self.PhysicalTrainingType.shortname === null ||
          self.PhysicalTrainingType.shortname === undefined ||
          self.PhysicalTrainingType.shortname === ""
        ) {
          this.makeToast(
        ...
        if (
          self.PhysicalTrainingType.fullname === 0 ||
          self.PhysicalTrainingType.fullname === null ||
          self.PhysicalTrainingType.fullname === undefined ||
          self.PhysicalTrainingType.fullname === ""
        ) {
          this.makeToast(
      ...
      SaveData() {
        ...
        var self = this;
        PhysicalTrainingTypeService.Update(self.PhysicalTrainingType)
          .then(res => {
            self.makeToast(
            ...
            self.$router.push({ name: "PhysicalTrainingType" });
          })
          .catch(error => {
            self.makeToast(
      ...
      translate(selectcolumnname) {
        ...
        self.translatelist = self.PhysicalTrainingType.Translates.filter(
      ...
      SaveTranslate() {
        var self = this;
        self.PhysicalTrainingType.Translates = self.PhysicalTrainingType.Translates.filter(
          x => x.columnname !== self.translatebyname
        );
        self.translatelist.forEach(item => {
          self.PhysicalTrainingType.Translates.push(item);
        });
