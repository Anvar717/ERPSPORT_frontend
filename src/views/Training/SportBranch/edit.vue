    ...
    <CCol>
      <CCard accent-color="primary" class="pb-4">
        <CCardHeader>
          <CRow>
            <CCol>
              <h4 class="text-center">{{ $t("SportBranch") }}</h4>
            </CCol>
          </CRow>
        </CCardHeader>
        <CCardBody>
            ...
            <CCol>
              <div class="text-left">
                <CButton
                  ...
                  size="sm"
                  color="danger"
                  @click="$router.push({ name: 'SportBranch' })"
                >
...
import SportBranchService from "@/services/SportBranch.service";
export default {
  data() {
    return {
      SaveLoading: false,
  ...
  created() {
    ...
    SportBranchService.Get(this.$route.params.id).then((res) => {
      this.SportBranch = res.data;
      this.items1 =
  ...
  methods: {
    check() {
      ...
      if (
        self.SportBranch.code === 0 ||
        self.SportBranch.code === null ||
        self.SportBranch.code === undefined ||
        self.SportBranch.code === ""
      ) {
        this.makeToast(this.$t("codeNotCorrect"), this.$t("Error"), "danger");
        return false;
      }
      if (
        self.SportBranch.shortname === 0 ||
        self.SportBranch.shortname === null ||
        self.SportBranch.shortname === undefined ||
        self.SportBranch.shortname === ""
      ) {
        this.makeToast(
      ...
      if (
        self.SportBranch.fullname === 0 ||
        self.SportBranch.fullname === null ||
        self.SportBranch.fullname === undefined ||
        self.SportBranch.fullname === ""
      ) {
        this.makeToast(
      ...
      if (
        self.SportBranch.stateid === 0 ||
        self.SportBranch.stateid === null ||
        self.SportBranch.stateid === undefined ||
        self.SportBranch.stateid === ""
      ) {
    ...
    SaveData() {
      ...
      SportBranchService.Update(this.SportBranch)
        .then((res) => {
          this.makeToast(
          ...
          this.$router.push({ name: "SportBranch" });
        })
        .catch((error) => {
          this.makeToast(
    ...
    SportBranchTranslatePush() {
      this.SportBranch.Translates.push({
        languageid: this.Translate.languageid,
        languagename: this.LanguageList.filter(
          (item) => item.id == this.Translate.languageid
        )[0].name,
