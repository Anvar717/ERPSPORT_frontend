  ...
  <CRow>
    <CCol sm="12" md="12">
      <CCard accent-color="primary">
        <h3 class="text-center mt-3">{{ $t("SportCompetitionStepInfo") }}</h3>
        <CCardHeader>
...
export default {
  data() {
    return {
      fields: [
        ...
        {
          key: "previousstepcode",
          label: this.$t("previousstepcode"),
          sortable: false,
          class: "text-left",
        },
        {
          key: "nextstepcode",
          label: this.$t("nextstepcode"),
          sortable: false,
          class: "text-left",
        },
        {
  ...
  methods: {
    ...
    Delete(item) {
      SportCompetitionStepInfoService.Delete(item.id)
        .then((res) => {
          this.makeToast(
    ...
    Refresh() {
      this.isBusy = true;
      SportCompetitionStepInfoService.GetList(
        this.filter.Search,
