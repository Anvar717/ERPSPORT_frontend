<template>
    <CRow>
      <CCol sm="12" md="12">
        <CCard accent-color="primary">
          <h3 class="text-center mt-3">{{ $t("SportEventCategory") }}</h3>
          <CCardHeader>
            ...
            >
              ...
              <template v-slot:cell(actions)="{ item }">
                <div class="text-center">
                  <b-link
                  v-if="$can('SportEventCategoryEdit', 'permissions')"
                    @click="Edit(item)"
                    class="mr-2"
                ...
                <b-modal :id="'DeleteModal' + item.id" hide-footer>
                  ...
                  <div class="d-block text-right">
                    ...
                    <CButton
                    v-if="$can('SportEventCategoryDelete', 'permissions')"
  ...
  export default {
    ...
    methods: {
      ...
      Delete(item) {
        SportEventCategoryService.Delete(item.id)
          .then((res) => {
            this.makeToast(
      ...
      Refresh() {
        this.isBusy = true;
        SportEventCategoryService.GetList(
          this.filter.Search,
