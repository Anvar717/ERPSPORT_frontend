<template>
      ...
      <CCol>
        <CCard accent-color="primary" class="pb-4">
          <CCardHeader>
            <CRow>
              <CCol>
                <h4 class="text-center">{{ $t("SportEventType") }}</h4>
              </CCol>
            </CRow>
          </CCardHeader>
          <CCardBody>
            ...
            <CRow>
              ...
              <CCol sm="6">
                <CInput
                  ...
                  horizontal
                  :label="$t('shortname')"
                  v-model="SportEventType.shortname"
                >
                  <template #append>
                    <CButton
                      @click="ShortNameLang"
            ...
            <CRow>
              ...
              <CCol sm="6">
                <CInput
                  ...
                  horizontal
                  :label="$t('fullname')"
                  v-model="SportEventType.fullname"
                >
                  <template #append>
                    <CButton
                      @click="FullNameLang"
              ...
              <CCol sm="6">
                <div class="form-group form-row">
                  ...
                  <v-select
                    ...
                    v-model="SportEventType.sporteventcategoryid"
                    :reduce="(item) => item.id"
              ...
              <CCol sm="6">
                <div class="form-group form-row">
                  ...
                  <v-select
                    class="col-sm-9"
                    :options="statelist"
                    v-model="SportEventType.stateid"
                    :reduce="(item) => item.id"
                    :placeholder="$t('ChooseBelow')"
                    label="name"
                  ></v-select>
              ...
              <CCol>
                <div class="text-left">
                  <CButton
                    ...
                    size="sm"
                    color="danger"
                    @click="$router.push({ name: 'SportEventType' })"
                  >
  ...
  export default {
    ...
    created() {
      ...
      SportEventTypeService.Get(this.$route.params.id).then((res) => {
        this.SportEventType = res.data;
        this.items1 =
    ...
    methods: {
      check() {
        ...
        if (
          self.SportEventType.code === 0 ||
          self.SportEventType.code === null ||
          self.SportEventType.code === undefined ||
          self.SportEventType.code === ""
        ) {
          this.makeToast(this.$t("codeNotCorrect"), this.$t("Error"), "danger");
          return false;
        }
        if (
          self.SportEventType.shortname === 0 ||
          self.SportEventType.shortname === null ||
          self.SportEventType.shortname === undefined ||
          self.SportEventType.shortname === ""
        ) {
          this.makeToast(
        ...
        if (
          self.SportEventType.fullname === 0 ||
          self.SportEventType.fullname === null ||
          self.SportEventType.fullname === undefined ||
          self.SportEventType.fullname === ""
        ) {
          this.makeToast(
        ...
        if (
          self.SportEventType.stateid === 0 ||
          self.SportEventType.stateid === null ||
          self.SportEventType.stateid === undefined ||
          self.SportEventType.stateid === ""
        ) {
      ...
      SaveData() {
        ...
        SportEventTypeService.Update(this.SportEventType)
          .then((res) => {
            this.makeToast(
            ...
            this.$router.push({ name: "SportEventType" });
          })
          .catch((error) => {
            this.makeToast(
      ...
      AddItem() {
        if (this.TranslatebyName == "shortname") {
          this.ItemsPush();
          this.SportEventTypeTranslatePush();
      ...
      SportEventTypeTranslatePush() {
        this.SportEventType.Translates.push({
          languageid: this.Translate.languageid,
          languagename: this.LanguageList.filter(
            (item) => item.id == this.Translate.languageid
          )[0].name,
