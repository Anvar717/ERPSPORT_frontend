  ...
  <CRow>
    <CCol sm="12" md="12">
      <CCard accent-color="primary">
        <h3 class="text-center mt-3">{{ $t("SportLevel") }}</h3>
        <CCardHeader>
          <CRow class="form-group justify-content-between">
            ...
            <CCol lg="7" md="6" sm="12" class="text-lg-right mt-2" style="margin-right: -7px;">
              <CButton
               v-if="$can('SportLevelDelete', 'permissions')"
                @click="Create"
                color="primary"
                variant="outline"
                class="mr-2"
        ...
        <CCardBody>
          ...
          >
            ...
            <template v-slot:cell(ismultiplesports)="{ item }">
                    <div>
                      <b-badge :variant="getItemColor(item.ismultiplesports)">{{
                        item.ismultiplesports ? "Да" : "Нет"
                      }}</b-badge>
                    </div>
                  </template>
            ...
            <template v-slot:cell(actions)="{ item }">
              <div class="text-center">
                <b-link
                  v-if="$can('SportLevelEdit', 'permissions')"
                  @click="Edit(item)"
                  class="mr-2"
...
export default {
  data() {
    return {
      fields: [
        ...
        {
          key: "ismultiplesports",
          label: this.$t("ismultiplesports"),
          sortable: false,
          thClass: "text-center",
          tdClass: "text-center",
        },
  ...
  methods: {
    ...
    Delete(item) {
      SportLevelService.Delete(item.id)
        .then((res) => {
          this.makeToast(
    ...
    Approve(item) {
      var self = this;
      SportLevelService.Approve(item.id)
        .then((res) => {
          self.makeToast(
    ...
    CancelApproval(item) {
      var self = this;
      SportLevelService.CancelApproval(item.id)
        .then((res) => {
          self.makeToast(
