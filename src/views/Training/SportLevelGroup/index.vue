<template>
    <CRow>
      <CCol sm="12" md="12">
        <CCard accent-color="primary">
          <h3 class="text-center mt-3">{{ $t("SportLevelGroup") }}</h3>
          <CCardHeader>
            ...
            >
              ...
              <template v-slot:cell(actions)="{ item }">
                <div class="text-center">
                  <b-link
                    @click="Edit(item)"
                    v-if="$can('SportLevelGroupEdit', 'permissions')"
                    class="mr-2"
                ...
                <b-modal :id="'DeleteModal' + item.id" hide-footer>
                  ...
                  <div class="d-block text-right">
                    ...
                    <CButton v-if="$can('SportLevelGroupDelete', 'permissions')" @click="Delete(item)" color="success">
  ...
  export default {
    ...
    methods: {
      ...
      Delete(item) {
        SportLevelGroupService.Delete(item.id)
          .then((res) => {
            this.makeToast(
      ...
      Approve(item) {
        var self = this;
        SportLevelGroupService.Approve(item.id)
          .then((res) => {
            self.makeToast(
      ...
      CancelApproval(item) {
        var self = this;
        SportLevelGroupService.CancelApproval(item.id)
          .then((res) => {
            self.makeToast(
      ...
      Refresh() {
        this.isBusy = true;
        SportLevelGroupService.GetList(
          this.filter.Search,
          this.filter.SortColumn,
          this.filter.OrderType,
          this.filter.PageNumber,
