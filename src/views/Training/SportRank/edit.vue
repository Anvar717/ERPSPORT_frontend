<template>
      ...
      <CCol>
        <CCard accent-color="primary" class="pb-4">
          <CCardHeader>
            <CRow>
              <CCol>
                <h4 class="text-center">{{ $t("SportRank") }}</h4>
              </CCol>
            </CRow>
          </CCardHeader>
          <CCardBody>
              ...
              <CCol>
                <div class="text-left">
                  <CButton
                    ...
                    size="sm"
                    color="danger"
                    @click="$router.push({ name: 'SportRank' })"
                  >
  ...
  export default {
    ...
    created() {
      ...
      SportRankService.Get(this.$route.params.id).then((res) => {
        this.SportRank = res.data;
        this.items1 =
    ...
    methods: {
      check() {
        ...
        if (
          self.SportRank.code === 0 ||
          self.SportRank.code === null ||
          self.SportRank.code === undefined ||
          self.SportRank.code === ""
        ) {
        ...
        if (
          self.SportRank.shortname === 0 ||
          self.SportRank.shortname === null ||
          self.SportRank.shortname === undefined ||
          self.SportRank.shortname === ""
        ) {
          this.makeToast(
        ...
        if (
          self.SportRank.fullname === 0 ||
          self.SportRank.fullname === null ||
          self.SportRank.fullname === undefined ||
          self.SportRank.fullname === ""
        ) {
          this.makeToast(
        ...
        if (
          self.SportRank.stateid === 0 ||
          self.SportRank.stateid === null ||
          self.SportRank.stateid === undefined ||
          self.SportRank.stateid === ""
        ) {
      ...
      SaveData() {
        ...
        SportRankService.Update(this.SportRank)
          .then((res) => {
            this.makeToast(
            ...
            this.$router.push({ name: "SportRank" });
          })
          .catch((error) => {
            this.makeToast(
      ...
      SportRankTranslatePush() {
        this.SportRank.Translates.push({
          languageid: this.Translate.languageid,
          languagename: this.LanguageList.filter(
            (item) => item.id == this.Translate.languageid
          )[0].name,
