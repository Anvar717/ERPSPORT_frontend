    ...
    <CCol>
      <CCard accent-color="primary" class="pb-4">
        <CCardHeader>
          <CRow>
            <CCol>
              <h4 class="text-center">{{ $t("SportSeason") }}</h4>
            </CCol>
          </CRow>
        </CCardHeader>
        <CCardBody>
            ...
            <CCol>
              <div class="text-left">
                <CButton
                  ...
                  size="sm"
                  color="danger"
                  @click="$router.push({ name: 'SportSeason' })"
                >
...
export default {
  ...
  created() {
    ...
    SportSeasonService.Get(this.$route.params.id).then((res) => {
      this.SportSeason = res.data;
      this.items1 =
  ...
  methods: {
    check() {
      ...
      if (
        self.SportSeason.code === 0 ||
        self.SportSeason.code === null ||
        self.SportSeason.code === undefined ||
        self.SportSeason.code === ""
      ) {
        this.makeToast(this.$t("codeNotCorrect"), this.$t("Error"), "danger");
        return false;
      }
      if (
        self.SportSeason.shortname === 0 ||
        self.SportSeason.shortname === null ||
        self.SportSeason.shortname === undefined ||
        self.SportSeason.shortname === ""
      ) {
        this.makeToast(
      ...
      if (
        self.SportSeason.fullname === 0 ||
        self.SportSeason.fullname === null ||
        self.SportSeason.fullname === undefined ||
        self.SportSeason.fullname === ""
      ) {
        this.makeToast(
      ...
      if (
        self.SportSeason.stateid === 0 ||
        self.SportSeason.stateid === null ||
        self.SportSeason.stateid === undefined ||
        self.SportSeason.stateid === ""
      ) {
    ...
    SaveData() {
      ...
      SportSeasonService.Update(this.SportSeason)
        .then((res) => {
          this.makeToast(
          ...
          this.$router.push({ name: "SportSeason" });
        })
        .catch((error) => {
          this.makeToast(
    ...
    SportSeasonTranslatePush() {
      this.SportSeason.Translates.push({
        languageid: this.Translate.languageid,
        languagename: this.LanguageList.filter(
          (item) => item.id == this.Translate.languageid
        )[0].name,
