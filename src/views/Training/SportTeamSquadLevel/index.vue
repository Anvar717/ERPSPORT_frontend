<template>
    <CRow>
      <CCol sm="12" md="12">
        <CCard accent-color="primary">
          <h3 class="text-center mt-3">{{ $t("SportTeamSquadLevel") }}</h3>
          <CCardHeader>
            ...
            >
              ...
              <template v-slot:cell(actions)="{ item }">
                <div class="text-center">
                  <b-link
                  v-if="$can('SportTeamSquadLevelEdit', 'permissions')"
                    @click="Edit(item)"
                    class="mr-2"
                ...
                <b-modal :id="'DeleteModal' + item.id" hide-footer>
                  ...
                  <div class="d-block text-right">
                    ...
                    <CButton
                    v-if="$can('SportTeamSquadLevelDelete', 'permissions')"
  ...
  export default {
    ...
    methods: {
      ...
      Delete(item) {
        SportTeamSquadLevelService.Delete(item.id)
          .then((res) => {
            this.makeToast(
      ...
      Refresh() {
        this.isBusy = true;
        SportTeamSquadLevelService.GetList(
          this.filter.Search,
