<template>
      ...
      <CCol>
        <CCard accent-color="primary" class="pb-4">
          <CCardHeader>
            <CRow>
              <CCol>
                <h4 class="text-center">{{ $t("SportTeamType") }}</h4>
              </CCol>
            </CRow>
          </CCardHeader>
          <CCardBody>
            ...
            <CRow>
              ...
              <CCol sm="6">
                <CInput
                  ...
                  horizontal
                  :label="$t('shortname')"
                  v-model="SportTeamType.shortname"
                >
                  <template #append>
                    <CButton
                      @click="ShortNameLang"
            ...
            <CRow>
              ...
              <CCol sm="6">
                <CInput
                  ...
                  horizontal
                  :label="$t('fullname')"
                  v-model="SportTeamType.fullname"
                >
                  <template #append>
                    <CButton
                      @click="FullNameLang"
              ...
              <CCol>
                <div class="text-left">
                  <CButton
                    ...
                    size="sm"
                    color="danger"
                    @click="$router.push({ name: 'SportTeamType' })"
                  >
  ...
  export default {
    ...
    created() {
      ...
      SportTeamTypeService.Get(this.$route.params.id).then((res) => {
        this.SportTeamType = res.data;
        this.items1 =
    ...
    methods: {
      check() {
        ...
        if (
          self.SportTeamType.code === 0 ||
          self.SportTeamType.code === null ||
          self.SportTeamType.code === undefined ||
          self.SportTeamType.code === ""
        ) {
          this.makeToast(this.$t("codeNotCorrect"), this.$t("Error"), "danger");
          return false;
        }
        if (
          self.SportTeamType.shortname === 0 ||
          self.SportTeamType.shortname === null ||
          self.SportTeamType.shortname === undefined ||
          self.SportTeamType.shortname === ""
        ) {
          this.makeToast(
        ...
        if (
          self.SportTeamType.fullname === 0 ||
          self.SportTeamType.fullname === null ||
          self.SportTeamType.fullname === undefined ||
          self.SportTeamType.fullname === ""
        ) {
          this.makeToast(
        ...
        if (
          self.SportTeamType.stateid === 0 ||
          self.SportTeamType.stateid === null ||
          self.SportTeamType.stateid === undefined ||
          self.SportTeamType.stateid === ""
        ) {
      ...
      SaveData() {
        ...
        SportTeamTypeService.Update(this.SportTeamType)
          .then((res) => {
            this.makeToast(
            ...
            this.$router.push({ name: "SportTeamType" });
          })
          .catch((error) => {
            this.makeToast(
      ...
      AddItem() {
        if (this.TranslatebyName == "shortname") {
          this.ItemsPush();
          this.SportTeamTypeTranslatePush();
      ...
      SportTeamTypeTranslatePush() {
        this.SportTeamType.Translates.push({
          languageid: this.Translate.languageid,
          languagename: this.LanguageList.filter(
            (item) => item.id == this.Translate.languageid
          )[0].name,
