      ...
      <CCol>
        <CCard accent-color="primary" class="pb-4">
          <CCardHeader>
            <CRow>
              <CCol>
                <h4 class="text-center">{{ $t("SportTitle") }}</h4>
              </CCol>
            </CRow>
          </CCardHeader>
          <CCardBody>
              ...
              <CCol sm="6">
                <div class="form-group form-row">
                  ...
                  <v-select
                    ...
                    v-model="SportTitle.organizationtypeid"
                    :reduce="(item) => item.id"
              ...
              <CCol>
                <div class="text-left">
                  <CButton
                    ...
                    size="sm"
                    color="danger"
                    @click="$router.push({ name: 'SportTitle' })"
                  >
...
import OrganizationTypeService from "@/services/OrganizationType.service";
  export default {
    ...
    created() {
      ...
      SportTitleService.Get(this.$route.params.id).then((res) => {
        this.SportTitle = res.data;
        this.items1 =
    ...
    methods: {
      check() {
        ...
        if (
          self.SportTitle.code === 0 ||
          self.SportTitle.code === null ||
          self.SportTitle.code === undefined ||
          self.SportTitle.code === ""
        ) {
          this.makeToast(this.$t("codeNotCorrect"), this.$t("Error"), "danger");
          return false;
        }
        if (
          self.SportTitle.shortname === 0 ||
          self.SportTitle.shortname === null ||
          self.SportTitle.shortname === undefined ||
          self.SportTitle.shortname === ""
        ) {
          this.makeToast(
        ...
        if (
          self.SportTitle.fullname === 0 ||
          self.SportTitle.fullname === null ||
          self.SportTitle.fullname === undefined ||
          self.SportTitle.fullname === ""
        ) {
          this.makeToast(
        ...
        if (
          self.SportTitle.stateid === 0 ||
          self.SportTitle.stateid === null ||
          self.SportTitle.stateid === undefined ||
          self.SportTitle.stateid === ""
        ) {
      ...
      SaveData() {
        ...
        SportTitleService.Update(this.SportTitle)
          .then((res) => {
            this.makeToast(
            ...
            this.$router.push({ name: "SportTitle" });
          })
          .catch((error) => {
            this.makeToast(
      ...
      SportTitleTranslatePush() {
        this.SportTitle.Translates.push({
          languageid: this.Translate.languageid,
          languagename: this.LanguageList.filter(
            (item) => item.id == this.Translate.languageid
          )[0].name,
