  ...
  <CRow>
    <CCol sm="12" md="12">
      <CCard accent-color="primary">
        <h3 class="text-center mt-3">{{ $t("SportTypeDependence") }}</h3>
        <CCardHeader>
          ...
          >
            ...
            <template v-slot:cell(actions)="{ item }">
              <div class="text-center">
                <b-link
                v-if="$can('SportTypeDependenceEdit', 'permissions')"
                  @click="Edit(item)"
                  class="mr-2"
                ...
                <b-link
                v-if="$can('SportTypeDependenceDelete', 'permissions')"
...
export default {
  ...
  methods: {
    ...
    Delete(item) {
      SportTypeDependenceService.Delete(item.id)
        .then((res) => {
          this.makeToast(
    ...
    Refresh() {
      this.isBusy = true;
      SportTypeDependenceService.GetList(
        this.filter.Search,
