  ...
  <CRow>
    <CCol sm="12" md="12">
      <CCard accent-color="primary">
        <h3 class="text-center mt-3">{{ $t("SportTypeGender") }}</h3>
        <CCardHeader>
          ...
          >
            ...
            <template v-slot:cell(actions)="{ item }">
              <div class="text-center">
                <b-link
                v-if="$can('SportTypeGenderEdit', 'permissions')"
                  @click="Edit(item)"
                  class="mr-2"
                ...
                <b-link
                v-if="$can('SportTypeGenderDelete', 'permissions')"
...
export default {
  ...
  methods: {
    ...
    Delete(item) {
      SportTypeGenderService.Delete(item.id)
        .then((res) => {
          this.makeToast(
    ...
    Refresh() {
      this.isBusy = true;
      SportTypeGenderService.GetList(
        this.filter.Search,
