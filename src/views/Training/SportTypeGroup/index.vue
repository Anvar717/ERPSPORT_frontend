<template>
    <CRow>
      <CCol sm="12" md="12">
        <CCard accent-color="primary">
          <h3 class="text-center mt-3">{{ $t("SportTypeGroup") }}</h3>
          <CCardHeader>
            ...
            >
              ...
              <template v-slot:cell(actions)="{ item }">
                <div class="text-center">
                  <b-link
                    @click="Edit(item)"
                    v-if="$can('SportTypeGroupEdit', 'permissions')"
                    class="mr-2"
                ...
                <b-modal :id="'DeleteModal' + item.id" hide-footer>
                  ...
                  <div class="d-block text-right">
                    ...
                    <CButton
                     @click="Delete(item)" 
                     v-if="$can('SportTypeGroupDelete', 'permissions')"
                     color="success">
  ...
  export default {
      ...
      return {
        fields: [
          ...
          },
          {
            ...
            label: this.$t("sporttypedivisionname1"),
            sortable: false,
            thClass: "text-center",
            tdClass: "text-center",
          },
    ...
    methods: {
      ...
      Delete(item) {
        SportTypeGroupService.Delete(item.id)
          .then((res) => {
            this.makeToast(
      ...
      Approve(item) {
        var self = this;
        SportTypeGroupService.Approve(item.id)
          .then((res) => {
            self.makeToast(
      ...
      CancelApproval(item) {
        var self = this;
        SportTypeGroupService.CancelApproval(item.id)
          .then((res) => {
            self.makeToast(
